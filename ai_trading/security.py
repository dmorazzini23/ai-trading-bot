...
class SecureConfig:
    ...
    def _generate_master_key(self) -> str:
        ...
        self.logger.warning(
            "Generated new master encryption key - save MASTER_ENCRYPTION_KEY to environment"
        )
        return key
    ...
    def _init_encryption(self) -> Fernet | None:
        ...
        try:
            ...
            salt = (
                b"ai_trading_salt_2024"  # Fixed salt for deterministic key derivation
            )
    ...
    def get_secure_config(self, key: str, default: Any = None) -> Any:
        ...
        self.log_audit_event(
            event_type=AuditEventType.CONFIG_ACCESS,
            action="get_config",
            resource=key,
            details={"key": key, "has_default": default is not None},
        )
        ...
        # Decrypt if sensitive and encrypted
    ...
    def set_secure_config(self, key: str, value: Any, encrypt: bool = None) -> None:
        ...
        self.log_audit_event(
            ...
            action="set_config",
            resource=key,
            details={"key": key, "encrypted": should_encrypt},
        )
    ...
    def _mask_string(self, value: str) -> str:
        ...
        if any(
            pattern in value.upper()
            for pattern in ["KEY", "SECRET", "TOKEN", "PASSWORD"]
        ):
            return f"{value[:4]}***{value[-4:]}"

        # Check for long alphanumeric strings (likely keys)
    ...
    def log_audit_event(
        ...
        event = AuditEvent(
            ...
            action=action,
            resource=resource,
            details=self.mask_sensitive_data(details or {}),
            result=result,
        )
        ...
        if event.severity in [SecurityLevel.ERROR, SecurityLevel.CRITICAL]:
            self.logger.warning(
                f"Security event: {event.action} on {event.resource} - {event.result}"
            )
...
class SafeLogger:
    ...
    def __init__(
        self, logger: logging.Logger, secure_config: SecureConfig | None = None
    ):
        self.logger = logger
        ...
        self._sensitive_patterns = [
            ...
            r"[A-Za-z0-9+/]{32,}={0,2}",  # Base64 encoded
        ]
...
class SecurityManager:
    ...
    def get_api_key(self, service: str) -> str | None:
        ...
        self.secure_config.log_audit_event(
            ...
            action="get_api_key",
            resource=service,
            result="success" if api_key else "failure",
            details={"has_key": bool(api_key)},
        )
    ...
    def validate_api_key(self, service: str, key: str) -> bool:
        ...
        validation_rules = {
            "alpaca": lambda k: len(k) >= 20 and k.replace("-", "").isalnum(),
            "news": lambda k: len(k) >= 32,
            "finnhub": lambda k: len(k) >= 20,
        }
    ...
    def check_security_health(self) -> dict[str, Any]:
        ...
        health_status = {
            "encryption_available": _CRYPTOGRAPHY_AVAILABLE,
            "audit_logging_active": bool(self.secure_config.audit_logger.handlers),
            "failed_access_attempts": self._failed_access_attempts,
            ...
            "last_security_check": self._last_security_check.isoformat(),
        }
        ...
        health_status["overall_health"] = (
            "healthy" if not critical_issues else "degraded"
        )
    ...
    def rotate_encryption_key(self) -> bool:
        ...
        try:
            ...
            # Create new secure config with new key
            self.secure_config = SecureConfig(new_key)

            # Re-encrypt all cached sensitive values
...
def log_security_event(
    event_type: AuditEventType, action: str, resource: str, **kwargs
) -> None:
    """Convenience function to log security events."""
    security_manager = get_security_manager()
    security_manager.secure_config.log_audit_event(
        event_type=event_type, action=action, resource=resource, **kwargs
    )
