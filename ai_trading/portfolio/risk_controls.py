...
class AdaptiveRiskController:
    ...
    def calculate_volatilities(
        self, returns_data: pd.DataFrame, lookback_days: int | None = None
    ) -> dict[str, float]:
        """
    ...
    def calculate_correlation_clusters(
        ...
        try:
            ...
            self.logger.info(
                f"Created {num_clusters} correlation clusters for {num_symbols} symbols"
            )
    ...
    def update_drawdown_governor(
        self, portfolio_return: float, is_positive_day: bool
    ) -> None:
        """
        ...
        if portfolio_return < -self.risk_budget.drawdown_threshold:
            ...
            self.green_days_count = 0
            self.logger.warning(
                f"Drawdown detected, reducing risk multiplier to {self.drawdown_multiplier}"
            )
            ...
            if self.green_days_count >= 5 and self.drawdown_multiplier < 1.0:
                ...
                self.logger.info(
                    f"Risk recovery: multiplier increased to {self.drawdown_multiplier}"
                )
    ...
    def check_cluster_limits(
        self, positions: dict[str, PositionRisk], cluster_assignments: dict[str, int]
    ) -> list[ClusterRisk]:
        """
    ...
    def calculate_position_sizes(
        ...
        expected_returns = {
            symbol: signal * 0.01 for symbol, signal in signals.items()
        }  # 1% per signal unit
        ...
        for symbol, position_risk in position_risks.items():
            ...
            if trade_value > 0 and not self.turnover_budget.add_trade(
                trade_value, portfolio_value
            ):
                # Scale down trade to fit budget
                available_turnover = (
                    self.turnover_budget.remaining_turnover * portfolio_value
                )
                if available_turnover > 0:
                    scale_factor = available_turnover / trade_value
                    target_value = (
                        current_value + (target_value - current_value) * scale_factor
                    )
                    ...
                    self.logger.warning(
                        f"No turnover budget for {symbol}, keeping current position"
                    )
    ...
    def reset_daily_budget(self) -> None:
        ...
        if self.turnover_budget.date != today:
            self.turnover_budget = TurnoverBudget(
                date=today, total_budget=self.risk_budget.max_turnover_daily
            )
            self.logger.info(
                f"Reset daily turnover budget: {self.risk_budget.max_turnover_daily:.1%}"
            )
    ...
    def get_risk_summary(self) -> dict:
        ...
        return {
            ...
            "num_clusters": (
                len(set(self._cluster_assignments.values()))
