...
class ProductionTradingSystem:
    ...
    def __init__(
        ...
        self.execution_coordinator = ProductionExecutionCoordinator(
            account_equity, risk_level
        )
        ...
        logger.info(
            f"ProductionTradingSystem initialized with equity=${account_equity:,.2f}, "
    ...
    async def analyze_trading_opportunity(
        ...
        try:
            ...
            mtf_analysis = self.mtf_analyzer.analyze_symbol(
                symbol, market_data.get("timeframe_data", {})
            )
    ...
    async def execute_trade(
        ...
        try:
            ...
            if market_data:
                opportunity_analysis = await self.analyze_trading_opportunity(
                    symbol, market_data
                )
    ...
    async def get_system_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Calculate uptime
            uptime_seconds = (
                (datetime.now(UTC) - self.session_start_time).total_seconds()
    ...
    async def _integrate_analyses(
        ...
        try:
            ...
            regime_multiplier = regime_recommendations.get(
            ...
            if mtf_action in ["BUY", "WEAK_BUY"] and regime_multiplier > 0.5:
                ...
                integrated["reasoning"].append(
                    f"Regime allows {regime_multiplier:.0%} position size"
                )
                ...
                integrated["reasoning"].append(
                    "No clear signal or regime restricts trading"
                )

            # Apply liquidity constraints
            if (
                liquidity_level
                and hasattr(liquidity_level, "value")
                and liquidity_level.value in ["very_low", "low"]
            ):
                integrated["confidence"] *= 0.5
                integrated["reasoning"].append(
                    "Reduced confidence due to low liquidity"
                )
    ...
    async def _generate_final_recommendation(
        self, integrated_rec: dict, risk_assessment: dict, liquidity_analysis: dict
    ) -> dict[str, Any]:
        ...
        try:
            ...
            # Apply liquidity constraints
            execution_recs = liquidity_analysis.get("execution_recommendations", {})
            final_rec["order_type"] = execution_recs.get(
                "recommended_order_type", OrderType.MARKET
            )
            final_rec["execution_strategy"] = execution_recs.get(
                "execution_strategy", "standard"
            )
    ...
    async def _update_performance_tracking(self, execution_result: dict):
        ...
        try:
            ...
            self.performance_dashboard.update_position(
                symbol, quantity, fill_price, fill_price
            )
