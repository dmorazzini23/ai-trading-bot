...
def _load_pandas():
    """Return pandas module or ``None`` if unavailable."""
    try:
...
def ensure_datetime(value: Any) -> _dt.datetime:
    ...
    """
    pd_mod = _load_pandas()
    oob_exc = getattr(getattr(pd_mod, "errors", None), "OutOfBoundsDatetime", ValueError)  # type: ignore[attr-defined]
    if callable(value):
        try:
            value = value()
        except (TypeError, ValueError, AttributeError, oob_exc) as e:  # type: ignore[arg-type]
...
def warmup_cache() -> None:
    """Compatibility stub for legacy cache warmup."""
    return None
...
def _flatten_and_normalize_ohlcv(df: pd.DataFrame, symbol: str | None = None) -> pd.DataFrame:
    ...
    pd = _load_pandas()
    if pd is None:
        raise DataFetchError("pandas is required")
...
def _fetch_bars(
    ...
    try:  # optional requests import
        ...
        ConnectionError = HTTPError = RequestException = Timeout = Exception
