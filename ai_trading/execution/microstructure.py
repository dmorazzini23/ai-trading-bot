...
class BidAskSpreadAnalyzer:
    ...
    def analyze_spread_features(
        self, market_data: dict, trade_history: list[dict]
    ) -> dict[str, float]:
        """
        ...
        try:
            ...
            if bid_price > 0 and ask_price > 0:
                ...
                features["spread_bps"] = (
                    (bid_ask_spread / mid_price) * 10000 if mid_price > 0 else 0
                )

                # Quoted spread percentage
                features["quoted_spread_pct"] = (
                    (bid_ask_spread / mid_price) * 100 if mid_price > 0 else 0
                )

                # Relative spread position
                if last_price > 0:
                    relative_position = (
                        (last_price - mid_price) / bid_ask_spread
                        if bid_ask_spread > 0
                        else 0
                    )
            ...
            if trade_history:
                effective_spread, realized_spread = self._calculate_trade_spreads(
                    trade_history
                )
            ...
            if bid_size > 0 and ask_size > 0:
                ...
                features["depth_imbalance"] = (bid_size - ask_size) / (
                    bid_size + ask_size
                )
                features["bid_ask_size_ratio"] = (
                    bid_size / ask_size if ask_size > 0 else 0
                )
    ...
    def classify_spread_regime(
        self, spread_features: dict[str, float]
    ) -> MarketRegimeFeature:
        """
    ...
    def _calculate_trade_spreads(
        ...
        try:
            ...
            avg_effective = (
                statistics.mean(effective_spreads) if effective_spreads else 0.0
            )
            avg_realized = (
                statistics.mean(realized_spreads) if realized_spreads else 0.0
            )
    ...
    def _assess_spread_quality(self, spread_features: dict[str, float]) -> float:
        ...
        try:
            ...
            # Weighted average
            quality_score = (
                spread_score * 0.5 + depth_score * 0.3 + imbalance_score * 0.2
            )
...
class OrderFlowAnalyzer:
    ...
    def analyze_order_flow(
        self, trade_data: list[dict], quote_data: list[dict]
    ) -> dict[str, float]:
        """
        ...
        try:
            ...
            # Flow toxicity
            features["order_flow_toxicity"] = self._calculate_flow_toxicity(
                trade_data, quote_data
            )

            # Information content
            features["information_content"] = self._estimate_information_content(
                trade_data
            )
            ...
            trade_sizes = [
                trade.get("size", 0) for trade in trade_data[-self.flow_window :]
            ]
            if trade_sizes:
                features["avg_trade_size"] = statistics.mean(trade_sizes)
                features["trade_size_volatility"] = (
                    statistics.stdev(trade_sizes) if len(trade_sizes) > 1 else 0
                )
                features["large_trade_ratio"] = sum(
                    1 for size in trade_sizes if size > 1000
                ) / len(trade_sizes)
    ...
    def detect_toxic_flow(self, flow_features: dict[str, float]) -> dict[str, Any]:
        ...
        try:
            ...
            is_toxic = (
                ...
                or trade_intensity > 5.0  # Very high intensity
                or order_imbalance > 0.8  # Severe imbalance
            )
    ...
    def _calculate_flow_toxicity(
        ...
        try:
            ...
            for _i, trade in enumerate(trade_data[-10:]):  # Last 10 trades
                trade_price = trade.get("price", 0)
                trade_side = trade.get("side", "unknown")
    ...
    def _estimate_information_content(self, trade_data: list[dict]) -> float:
        ...
        try:
            ...
            information_content = (
                price_variance / (total_volume / len(volumes))
                if total_volume > 0
                else 0
            )
...
class MarketMicrostructureEngine:
    ...
    def analyze_market_microstructure(
        ...
        try:
            ...
            # Spread analysis
            spread_features = self.spread_analyzer.analyze_spread_features(
                market_data, trade_history
            )

            # Order flow analysis
            flow_features = self.flow_analyzer.analyze_order_flow(
                trade_history, quote_history
            )
            ...
            microstructure_data = MarketMicrostructureData(
                ...
                price_variance_ratio=volatility_features.get(
                    "price_variance_ratio", 0.0
                ),
                microstructure_noise=volatility_features.get(
    ...
    def estimate_execution_impact(
        self, order_size: int, microstructure_data: MarketMicrostructureData
    ) -> dict[str, float]:
        """
        ...
        try:
            ...
            spread_impact_bps = (
                microstructure_data.spread_bps * 0.5
            )  # Half spread crossing

            # Market impact based on participation
            market_impact_bps = self._calculate_market_impact(
                participation_rate, microstructure_data
            )

            # Temporary impact (recovers quickly)
            temporary_impact_bps = (
                market_impact_bps * self.impact_parameters["temporary_impact_factor"]
            )

            # Permanent impact (persists)
            permanent_impact_bps = (
                market_impact_bps * self.impact_parameters["permanent_impact_factor"]
            )

            # Total expected impact
            total_impact_bps = (
                spread_impact_bps + temporary_impact_bps + permanent_impact_bps
            )

            # Timing risk based on volatility
            timing_risk_bps = (
                microstructure_data.realized_volatility
                * 100
    ...
    def _estimate_price_impact(
        ...
        try:
            ...
            # Average trade size
            recent_trades = (
                trade_history[-20:] if len(trade_history) >= 20 else trade_history
            )
            trade_sizes = [
                trade.get("size", 0)
                for trade in recent_trades
            ...
            if depth > 0 and avg_trade_size > 0:
                impact_per_1000_shares = (
                    avg_trade_size / depth
                ) * 0.1  # 10% of ratio as impact
    ...
    def _analyze_microstructure_volatility(
        self, trade_history: list[dict]
    ) -> dict[str, float]:
        ...
        try:
            ...
            # Realized volatility
            if len(returns) > 1:
                features["realized_volatility"] = statistics.stdev(returns) * math.sqrt(
                    252 * 390
                )  # Annualized
            else:
            ...
            if len(prices) >= 20:
                variance_1 = (
                    statistics.variance(prices[-10:]) if len(prices[-10:]) > 1 else 0
                )
                variance_2 = (
                    statistics.variance(prices[-20:-10])
                    if len(prices[-20:-10]) > 1
                    else 0
                )
                features["price_variance_ratio"] = (
                    variance_1 / variance_2 if variance_2 > 0 else 1.0
                )
            else:
    ...
    def _calculate_market_impact(
        self, participation_rate: float, microstructure_data: MarketMicrostructureData
    ) -> float:
        """Calculate market impact in basis points."""
        try:
            # Base impact from square-root law
            base_impact = 100 * math.sqrt(
                participation_rate
            )  # 100 bps at 100% participation

            # Adjust for market conditions
            spread_adjustment = (
                microstructure_data.spread_bps / 10
            )  # Higher spreads = higher impact
            volatility_adjustment = (
                microstructure_data.realized_volatility * 100
            )  # Higher vol = higher impact
            toxicity_adjustment = (
                microstructure_data.order_flow_toxicity * 50
    ...
    def _assess_impact_confidence(
        self, microstructure_data: MarketMicrostructureData
    ) -> float:
    ...
    def _calculate_autocorrelation(self, data: list[float], lag: int = 1) -> float:
        ...
        try:
            ...
            autocovariance = sum(
                (data[i] - mean_val) * (data[i - lag] - mean_val) for i in range(lag, n)
            ) / (n - lag)
    ...
    def _create_default_microstructure_data(
        self, symbol: str
    ) -> MarketMicrostructureData:
