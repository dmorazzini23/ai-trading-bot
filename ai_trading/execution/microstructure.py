...
class BidAskSpreadAnalyzer:
        ...
        try:
            ...
            for i, trade in enumerate(trade_history[-self.lookback_periods:]):
                ...
                if quote_mid > 0:
                    ...
                    effective_spreads.append(effective_spread * 100)
                if i < len(trade_history) - 10:
    ...
    def _assess_spread_quality(self, spread_features: dict[str, float]) -> float:
        ...
        try:
            ...
            depth_score = min(1, market_depth / 10000)
...
class OrderFlowAnalyzer:
    ...
    def detect_toxic_flow(self, flow_features: dict[str, float]) -> dict[str, Any]:
        ...
        try:
            ...
            is_toxic = toxicity_score > self.toxicity_threshold or trade_intensity > 5.0 or order_imbalance > 0.8
            risk_level = 'high' if is_toxic else 'normal'
            if toxicity_score > 0.7 or trade_intensity > 10:
                risk_level = 'extreme'
            return {'is_toxic': is_toxic, 'risk_level': risk_level, 'toxicity_score': toxicity_score, 'primary_concern': self._identify_primary_concern(flow_features), 'recommendations': self._generate_flow_recommendations(flow_features)}
        except (ValueError, TypeError) as e:
    ...
    def _calculate_flow_toxicity(self, trade_data: list[dict], quote_data: list[dict]) -> float:
        ...
        try:
            if len(trade_data) < 5 or len(quote_data) < 5:
                return 0.0
            toxicity_measures = []
            for _i, trade in enumerate(trade_data[-10:]):
                trade_price = trade.get('price', 0)
                trade_side = trade.get('side', 'unknown')
                trade_time = trade.get('timestamp')
                if not all([trade_price, trade_side, trade_time]):
                    continue
                future_quotes = [q for q in quote_data if q.get('timestamp', datetime.min) > trade_time][:3]
                if not future_quotes:
                    continue
                pre_trade_mid = trade.get('quote_mid_before', trade_price)
                post_trade_mid = statistics.mean([(q.get('bid_price', 0) + q.get('ask_price', 0)) / 2 for q in future_quotes if q.get('bid_price', 0) > 0 and q.get('ask_price', 0) > 0])
...
class MarketMicrostructureEngine:
    ...
    def __init__(self):
        ...
        self.impact_parameters = {'temporary_impact_factor': 0.5, 'permanent_impact_factor': 0.3, 'participation_threshold': 0.1}
        logger.info('MarketMicrostructureEngine initialized')

    def analyze_market_microstructure(self, symbol: str, market_data: dict, trade_history: list[dict], quote_history: list[dict]) -> MarketMicrostructureData:
        """
        ...
        try:
            ...
            microstructure_data = MarketMicrostructureData(symbol=symbol, timestamp=datetime.now(UTC), bid_price=bid_price, ask_price=ask_price, last_price=last_price, bid_size=bid_size, ask_size=ask_size, volume=volume, bid_ask_spread=spread_features.get('bid_ask_spread', 0.0), spread_bps=spread_features.get('spread_bps', 0.0), quoted_spread_pct=spread_features.get('quoted_spread_pct', 0.0), effective_spread_pct=spread_features.get('effective_spread_pct', 0.0), realized_spread_pct=spread_features.get('realized_spread_pct', 0.0), market_depth=spread_features.get('market_depth', 0.0), order_imbalance=flow_features.get('order_imbalance', 0.0), price_impact_estimate=impact_features.get('estimated_impact', 0.0), trade_intensity=flow_features.get('trade_intensity', 0.0), order_flow_toxicity=flow_features.get('order_flow_toxicity', 0.0), information_content=flow_features.get('information_content', 0.0), realized_volatility=volatility_features.get('realized_volatility', 0.0), price_variance_ratio=volatility_features.get('price_variance_ratio', 0.0), microstructure_noise=volatility_features.get('microstructure_noise', 0.0))
    ...
    def estimate_execution_impact(self, order_size: int, microstructure_data: MarketMicrostructureData) -> dict[str, float]:
        ...
        try:
            ...
            spread_impact_bps = microstructure_data.spread_bps * 0.5
            ...
            return {'spread_impact_bps': spread_impact_bps, 'market_impact_bps': market_impact_bps, 'temporary_impact_bps': temporary_impact_bps, 'permanent_impact_bps': permanent_impact_bps, 'total_impact_bps': total_impact_bps, 'timing_risk_bps': timing_risk_bps, 'participation_rate': participation_rate, 'confidence_level': self._assess_impact_confidence(microstructure_data)}
        except (ValueError, TypeError) as e:
    ...
    def _estimate_price_impact(self, market_data: dict, trade_history: list[dict]) -> dict[str, float]:
        ...
        try:
            ...
            if depth > 0 and avg_trade_size > 0:
                impact_per_1000_shares = avg_trade_size / depth * 0.1
                features['estimated_impact'] = impact_per_1000_shares
            else:
                features['estimated_impact'] = 0.05
            return features
        except (KeyError, ValueError, TypeError, ZeroDivisionError):
    ...
    def _analyze_microstructure_volatility(self, trade_history: list[dict]) -> dict[str, float]:
        ...
        try:
            ...
            if len(returns) > 1:
                features['realized_volatility'] = statistics.stdev(returns) * math.sqrt(252 * 390)
            else:
    ...
    def _calculate_market_impact(self, participation_rate: float, microstructure_data: MarketMicrostructureData) -> float:
        ...
        try:
            base_impact = 100 * math.sqrt(participation_rate)
            spread_adjustment = microstructure_data.spread_bps / 10
            volatility_adjustment = microstructure_data.realized_volatility * 100
            toxicity_adjustment = microstructure_data.order_flow_toxicity * 50
            total_impact = base_impact + spread_adjustment + volatility_adjustment + toxicity_adjustment
            return min(200, max(1, total_impact))
        except (KeyError, ValueError, TypeError, ZeroDivisionError):
            return 50.0
    ...
    def _calculate_autocorrelation(self, data: list[float], lag: int=1) -> float:
        ...
        try:
            ...
            autocovariance = sum(((data[i] - mean_val) * (data[i - lag] - mean_val) for i in range(lag, n))) / (n - lag)
    ...
    def _create_default_microstructure_data(self, symbol: str) -> MarketMicrostructureData:
        ...
        return MarketMicrostructureData(symbol=symbol, timestamp=datetime.now(UTC), bid_price=0.0, ask_price=0.0, last_price=0.0, bid_size=0, ask_size=0, volume=0, bid_ask_spread=0.0, spread_bps=0.0, quoted_spread_pct=0.0, effective_spread_pct=0.0, realized_spread_pct=0.0, market_depth=0.0, order_imbalance=0.0, price_impact_estimate=0.0, trade_intensity=0.0, order_flow_toxicity=0.0, information_content=0.0, realized_volatility=0.0, price_variance_ratio=1.0, microstructure_noise=0.0)