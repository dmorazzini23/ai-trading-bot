...
class PositionDiscrepancy:
    ...
    def __init__(self, symbol: str, bot_qty: float, broker_qty: float, discrepancy_type: str, severity: str='medium'):
        self.symbol = symbol
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'symbol': self.symbol, 'bot_qty': self.bot_qty, 'broker_qty': self.broker_qty, 'difference': self.difference, 'discrepancy_type': self.discrepancy_type, 'severity': self.severity, 'timestamp': self.timestamp.isoformat()}
...
class PositionReconciler:
    ...
    def __init__(self, api_client=None):
        ...
        self.reconciliation_interval = 300
        self.position_tolerance = 1e-06
        self.large_discrepancy_threshold = 10
        self.running = False
        self._reconciliation_thread: Thread | None = None

    def update_bot_position(self, symbol: str, quantity: float, reason: str='live_trading') -> None:
    ...
    def adjust_bot_position(self, symbol: str, quantity_change: float, reason: str='live_trading') -> None:
    ...
    def reconcile_positions(self) -> list[PositionDiscrepancy]:
        ...
        for symbol in all_symbols:
            ...
            if abs(bot_qty - broker_qty) > self.position_tolerance:
                ...
                discrepancy = PositionDiscrepancy(symbol=symbol, bot_qty=bot_qty, broker_qty=broker_qty, discrepancy_type=discrepancy_type, severity=severity)
        ...
        if discrepancies:
            self.logger.error('RECONCILIATION_DISCREPANCIES_FOUND', extra={'discrepancies_count': len(discrepancies), 'symbols_affected': [d.symbol for d in discrepancies], 'severe_discrepancies': len([d for d in discrepancies if d.severity == 'high'])})
        else:
            self.logger.info('RECONCILIATION_SUCCESS', extra={'message': 'All positions reconciled successfully', 'positions_checked': len(all_symbols)})
    ...
    def auto_resolve_discrepancies(self, discrepancies: list[PositionDiscrepancy]) -> int:
        ...
        for discrepancy in discrepancies:
            if discrepancy.severity in ['low', 'medium']:
                self.logger.info('AUTO_RESOLVE_DISCREPANCY', extra={'symbol': discrepancy.symbol, 'action': 'update_bot_position', 'old_bot_qty': discrepancy.bot_qty, 'new_bot_qty': discrepancy.broker_qty, 'reason': 'auto_reconciliation'})
                self.update_bot_position(discrepancy.symbol, discrepancy.broker_qty, reason='auto_reconciliation')
                resolved_count += 1
            else:
                self.logger.error('HIGH_SEVERITY_DISCREPANCY_REQUIRES_MANUAL_REVIEW', extra=discrepancy.to_dict())
    ...
    def get_discrepancy_history(self, symbol: str | None=None, limit: int=50) -> list[PositionDiscrepancy]:
    ...
    def force_sync_from_broker(self) -> dict[str, float]:
        ...
        self.logger.info('FORCE_SYNC_COMPLETED', extra={'old_positions': old_positions, 'new_positions': broker_positions, 'positions_changed': len(set(old_positions.keys()) | set(broker_positions.keys()))})
        return broker_positions
    ...
    def get_reconciliation_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'running': self.running, 'interval_seconds': self.reconciliation_interval, 'current_discrepancies': current_discrepancies, 'total_historical_discrepancies': total_historical_discrepancies, 'severity_breakdown': dict(severity_counts), 'avg_discrepancies_per_reconciliation': avg_discrepancies_per_reconciliation, 'total_reconciliations': len(self._reconciliation_history), 'bot_positions_count': len(self._bot_positions), 'broker_positions_count': len(self._broker_positions)}
_position_reconciler: PositionReconciler | None = None
...
def update_bot_position(symbol: str, quantity: float, reason: str='live_trading') -> None:
    """Update bot position tracking."""
    reconciler = get_position_reconciler()
    reconciler.update_bot_position(symbol, quantity, reason)

def adjust_bot_position(symbol: str, quantity_change: float, reason: str='live_trading') -> None:
