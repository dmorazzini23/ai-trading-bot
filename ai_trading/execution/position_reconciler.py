...
class PositionReconciler:
    ...
    def update_bot_position(
        self, symbol: str, quantity: float, reason: str = "trade_execution"
    ) -> None:
    ...
    def adjust_bot_position(
        self, symbol: str, quantity_change: float, reason: str = "trade_execution"
    ) -> None:
    ...
    def get_broker_positions(self) -> dict[str, float]:
        ...
        if not self.api_client:
            self.logger.warning(
                "NO_API_CLIENT",
                extra={"message": "Cannot fetch broker positions without API client"},
            )
            return {}
    ...
    def reconcile_positions(self) -> list[PositionDiscrepancy]:
        ...
        self.logger.info(
            "RECONCILIATION_START", extra={"timestamp": datetime.now(UTC).isoformat()}
        )
    ...
    def _determine_severity(
        self, symbol: str, bot_qty: float, broker_qty: float
    ) -> str:
    ...
    def auto_resolve_discrepancies(
        ...
        for discrepancy in discrepancies:
            if discrepancy.severity in ["low", "medium"]:
                ...
                self.logger.error(
                    "HIGH_SEVERITY_DISCREPANCY_REQUIRES_MANUAL_REVIEW",
    ...
    def start_periodic_reconciliation(self, interval: int | None = None) -> None:
        ...
        self._reconciliation_thread = Thread(
            target=self._reconciliation_loop, daemon=True, name="PositionReconciler"
        )
        self._reconciliation_thread.start()

        self.logger.info(
            "PERIODIC_RECONCILIATION_STARTED",
            extra={"interval_seconds": self.reconciliation_interval},
        )
    ...
    def _reconciliation_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                time.sleep(
                    min(self.reconciliation_interval, 60)
        ...
        self.logger.info(
            ...
            extra={
                ...
                "positions_changed": len(
                    set(old_positions.keys()) | set(broker_positions.keys())
                ),
            },
        )
        ...
        with self._lock:
            ...
            if recent_reconciliations:
                total_discrepancies = sum(
                    r["discrepancies_count"] for r in recent_reconciliations
                )
                avg_discrepancies_per_reconciliation = total_discrepancies / len(
...
def update_bot_position(
    symbol: str, quantity: float, reason: str = "trade_execution"
) -> None:
...
def adjust_bot_position(
    symbol: str, quantity_change: float, reason: str = "trade_execution"
) -> None:
