...
class ExecutionDebugTracker:
    ...
    def log_position_update(self, symbol: str, old_qty: float, new_qty: float, 
        ...
        if correlation_id:
            self.log_execution_event(
                correlation_id,
                ExecutionPhase.POSITION_UPDATED,
                position_update
            )
    ...
    def get_position_updates(self, symbol: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
...
_debug_tracker: Optional[ExecutionDebugTracker] = None
_tracker_lock = Lock()
...
def log_signal_to_execution(symbol: str, side: str, qty: int, 
                           signal_data: Optional[Dict] = None) -> str:
...
def log_execution_phase(correlation_id: str, phase: ExecutionPhase, 
...
def log_order_outcome(correlation_id: str, success: bool, 
                     order_data: Optional[Dict] = None, error: Optional[str] = None) -> None:
...
def log_position_change(symbol: str, old_qty: float, new_qty: float, 
                       correlation_id: Optional[str] = None) -> None:
