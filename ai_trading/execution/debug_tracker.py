...
class ExecutionDebugTracker:
    ...
    def start_execution_tracking(self, correlation_id: str, symbol: str, qty: int, side: str, signal_data: dict | None=None) -> None:
        """Start tracking a new order execution."""
        execution_start = {'correlation_id': correlation_id, 'symbol': symbol, 'qty': qty, 'side': side, 'start_time': datetime.now(UTC).isoformat(), 'signal_data': signal_data or {}, 'status': OrderStatus.PENDING.value, 'phases': []}
        lock_acquired = False
        try:
            ...
            self.logger.error('START_TRACKING_ERROR', extra={'correlation_id': correlation_id, 'error': str(e)})
        finally:
            if lock_acquired:
                self._lock.release()
        self.log_execution_event(correlation_id, ExecutionPhase.SIGNAL_GENERATED, {'symbol': symbol, 'qty': qty, 'side': side, 'signal_data': signal_data})

    def log_execution_event(self, correlation_id: str, phase: ExecutionPhase, data: dict[str, Any] | None=None) -> None:
        """Log an execution phase event with correlation ID."""
        timestamp = datetime.now(UTC).isoformat()
        event = {'timestamp': timestamp, 'correlation_id': correlation_id, 'phase': phase.value, 'data': data or {}}
        lock_acquired = False
        try:
            lock_acquired = self._lock.acquire(timeout=5.0)
            if not lock_acquired:
                self.logger.warning('LOCK_TIMEOUT_EXECUTION_EVENT', extra={'correlation_id': correlation_id, 'phase': phase.value, 'message': 'Failed to acquire lock within timeout, logging without state update'})
                return
            ...
            if correlation_id in self._active_orders:
                ...
                if phase == ExecutionPhase.ORDER_SUBMITTED:
                    self._active_orders[correlation_id]['status'] = OrderStatus.SUBMITTED.value
                elif phase == ExecutionPhase.ORDER_ACKNOWLEDGED:
                    self._active_orders[correlation_id]['status'] = OrderStatus.ACKNOWLEDGED.value
                elif phase == ExecutionPhase.ORDER_FILLED:
                    self._active_orders[correlation_id]['status'] = OrderStatus.FILLED.value
                elif phase == ExecutionPhase.ORDER_PARTIALLY_FILLED:
                    self._active_orders[correlation_id]['status'] = OrderStatus.PARTIALLY_FILLED.value
            ...
            self.logger.error('EXECUTION_EVENT_ERROR', extra={'correlation_id': correlation_id, 'phase': phase.value, 'error': str(e), 'message': 'Error updating execution event state'})
        finally:
            if lock_acquired:
                self._lock.release()
        log_data = {'correlation_id': correlation_id, 'phase': phase.value, 'timestamp': timestamp}
        ...
        try:
            ...
            elif phase in [ExecutionPhase.SIGNAL_GENERATED, ExecutionPhase.ORDER_SUBMITTED, ExecutionPhase.ORDER_FILLED, ExecutionPhase.ORDER_REJECTED]:
                self.logger.info(f'EXEC_EVENT_{phase.value.upper()}', extra=log_data)
        except (ValueError, TypeError, AttributeError) as e:
            self.logger.exception('debug_tracker: logging error during execution event', exc_info=e)

    def log_order_result(self, correlation_id: str, success: bool, order_data: dict | None=None, error: str | None=None) -> None:
        """Log the final result of an order execution."""
        timestamp = datetime.now(UTC).isoformat()
        result_data = {'correlation_id': correlation_id, 'success': success, 'timestamp': timestamp, 'order_data': order_data or {}, 'error': error}
        lock_acquired = False
        order_info = None
        found_order = False
        try:
            ...
            self.logger.error('ORDER_RESULT_ERROR', extra={'correlation_id': correlation_id, 'error': str(e)})
        finally:
            if lock_acquired:
                self._lock.release()
        try:
            if found_order:
                ...
                self.logger.warning('UNKNOWN_CORRELATION_ID', extra={'correlation_id': correlation_id, 'message': 'Attempted to log result for unknown correlation ID'})
        except (ValueError, TypeError, AttributeError) as e:
            self.logger.exception('debug_tracker: logging error during order result', exc_info=e)

    def log_position_update(self, symbol: str, old_qty: float, new_qty: float, correlation_id: str | None=None) -> None:
        """Log position updates with optional correlation to order."""
        position_update = {'timestamp': datetime.now(UTC).isoformat(), 'symbol': symbol, 'old_qty': old_qty, 'new_qty': new_qty, 'qty_change': new_qty - old_qty, 'correlation_id': correlation_id}
        with self._lock:
            self._position_updates.append(position_update)
        if correlation_id:
            self.log_execution_event(correlation_id, ExecutionPhase.POSITION_UPDATED, position_update)
    ...
    def get_position_updates(self, symbol: str | None=None, limit: int=50) -> list[dict[str, Any]]:
    ...
    def get_execution_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'active_orders': active_count, 'recent_successes': recent_success_count, 'recent_failures': recent_failure_count, 'success_rate': success_rate, 'status_breakdown': status_breakdown, 'position_updates_count': len(self._position_updates)}
...
def log_signal_to_execution(symbol: str, side: str, qty: int, signal_data: dict | None=None) -> str:
...
def log_execution_phase(correlation_id: str, phase: ExecutionPhase, data: dict | None=None) -> None:
    """Log an execution phase with correlation ID."""
    tracker = get_debug_tracker()
    tracker.log_execution_event(correlation_id, phase, data)

def log_order_outcome(correlation_id: str, success: bool, order_data: dict | None=None, error: str | None=None) -> None:
    """Log the final outcome of an order execution."""
    tracker = get_debug_tracker()
    tracker.log_order_result(correlation_id, success, order_data, error)

def log_position_change(symbol: str, old_qty: float, new_qty: float, correlation_id: str | None=None) -> None:
