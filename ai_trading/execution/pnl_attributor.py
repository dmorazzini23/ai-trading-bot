...
class PnLEvent:
    ...
    def __init__(self, symbol: str, pnl_amount: float, source: PnLSource,
                 description: str, position_qty: Optional[float] = None,
                 price: Optional[float] = None, correlation_id: Optional[str] = None):
        self.symbol = symbol
...
class PositionSnapshot:
    ...
    def __init__(self, symbol: str, quantity: float, avg_cost: float, 
                 market_price: float, market_value: Optional[float] = None):
        self.symbol = symbol
...
class PnLAttributor:
    ...
    def update_position_snapshot(self, symbol: str, quantity: float, avg_cost: float,
                                market_price: float, correlation_id: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            self.logger.info("POSITION_SNAPSHOT_UPDATE", extra={
                'symbol': symbol,
    ...
    def _calculate_pnl_attribution(self, old_snapshot: PositionSnapshot, 
        ...
        if abs(price_change) > 1e-6 and old_snapshot.quantity != 0:  # Price changed and we had position
    ...
    def add_trade_pnl(self, symbol: str, trade_qty: float, execution_price: float,
                     avg_cost: float, fees: float = 0, slippage: float = 0,
                     correlation_id: Optional[str] = None) -> None:
    ...
    def add_dividend_pnl(self, symbol: str, dividend_amount: float, 
                        shares: float, correlation_id: Optional[str] = None) -> None:
    ...
    def add_manual_adjustment(self, symbol: str, adjustment_amount: float,
                            reason: str, correlation_id: Optional[str] = None) -> None:
    ...
    def _add_pnl_event(self, symbol: str, pnl_amount: float, source: PnLSource,
                      description: str, position_qty: Optional[float] = None,
                      price: Optional[float] = None, correlation_id: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if source in [PnLSource.POSITION_CHANGE, PnLSource.FEES, 
                         PnLSource.SLIPPAGE, PnLSource.DIVIDEND, PnLSource.ADJUSTMENT]:
    ...
    def get_pnl_by_symbol(self, symbol: str, sources: Optional[List[PnLSource]] = None) -> Dict[str, float]:
    ...
    def get_daily_pnl(self, date: Optional[str] = None) -> Dict[str, float]:
    ...
    def get_total_pnl_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'pnl_by_source': {source.value: amount for source, amount in total_by_source.items()},
                'today_pnl': today_pnl,
                'positions_count': len(self._position_snapshots),
                'events_count': len(self._pnl_events)
            }
    
    def get_recent_pnl_events(self, symbol: Optional[str] = None, 
                             sources: Optional[List[PnLSource]] = None,
    ...
    def explain_pnl_change(self, symbol: str, time_window_minutes: int = 60) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_events = [
                e for e in self._pnl_events 
                if (e.symbol == symbol and 
                    e.timestamp.timestamp() > cutoff_time)
            ]
        ...
        return {
            ...
            'explanation': '; '.join(explanations) if explanations else 'No significant PnL changes',
    ...
    def calculate_attribution_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'events_by_source': {source.value: count for source, count in source_counts.items()},
                'total_pnl_by_source': {source.value: amount for source, amount in source_amounts.items()},
                ...
                'symbols_tracked': len(set(e.symbol for e in self._pnl_events))
            }


# Global PnL attributor instance
_pnl_attributor: Optional[PnLAttributor] = None
...
def update_position_for_pnl(symbol: str, quantity: float, avg_cost: float,
                           market_price: float, correlation_id: Optional[str] = None) -> None:
...
def record_trade_pnl(symbol: str, trade_qty: float, execution_price: float,
                    avg_cost: float, fees: float = 0, slippage: float = 0,
                    correlation_id: Optional[str] = None) -> None:
...
def record_dividend_income(symbol: str, dividend_per_share: float, 
                          shares: float, correlation_id: Optional[str] = None) -> None:
