...
class PnLEvent:
    ...
    def __init__(self, symbol: str, pnl_amount: float, source: PnLSource, description: str, position_qty: float | None=None, price: float | None=None, correlation_id: str | None=None):
        self.symbol = symbol
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'symbol': self.symbol, 'pnl_amount': self.pnl_amount, 'source': self.source.value, 'description': self.description, 'position_qty': self.position_qty, 'price': self.price, 'correlation_id': self.correlation_id, 'timestamp': self.timestamp.isoformat()}

class PositionSnapshot:
    """Snapshot of a position at a point in time."""

    def __init__(self, symbol: str, quantity: float, avg_cost: float, market_price: float, market_value: float | None=None):
        self.symbol = symbol
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'symbol': self.symbol, 'quantity': self.quantity, 'avg_cost': self.avg_cost, 'market_price': self.market_price, 'market_value': self.market_value, 'cost_basis': self.cost_basis, 'unrealized_pnl': self.unrealized_pnl, 'timestamp': self.timestamp.isoformat()}
...
class PnLAttributor:
    ...
    def __init__(self):
        ...
        self._cumulative_pnl: dict[str, dict[PnLSource, float]] = defaultdict(lambda: defaultdict(float))
        self._total_realized_pnl = 0.0
        self._total_unrealized_pnl = 0.0
        self.max_events_history = 5000

    def update_position_snapshot(self, symbol: str, quantity: float, avg_cost: float, market_price: float, correlation_id: str | None=None) -> None:
        ...
        with self._lock:
            ...
            self.logger.info('POSITION_SNAPSHOT_UPDATE', extra={'symbol': symbol, 'new_snapshot': new_snapshot.to_dict(), 'correlation_id': correlation_id})
            if old_snapshot:
                self._calculate_pnl_attribution(old_snapshot, new_snapshot, correlation_id)

    def _calculate_pnl_attribution(self, old_snapshot: PositionSnapshot, new_snapshot: PositionSnapshot, correlation_id: str | None=None) -> None:
        ...
        if abs(qty_change) > 1e-06:
            if qty_change < 0:
                avg_exit_price = new_snapshot.market_price
                position_change_pnl = abs(qty_change) * (avg_exit_price - old_snapshot.avg_cost)
                self._add_pnl_event(symbol=symbol, pnl_amount=position_change_pnl, source=PnLSource.POSITION_CHANGE, description=f'Realized PnL from selling {abs(qty_change)} shares at ~${avg_exit_price:.2f}', position_qty=qty_change, price=avg_exit_price, correlation_id=correlation_id)
        price_change = new_snapshot.market_price - old_snapshot.market_price
        if abs(price_change) > 1e-06 and old_snapshot.quantity != 0:
            base_quantity = old_snapshot.quantity
            market_movement_pnl = base_quantity * price_change
            self._add_pnl_event(symbol=symbol, pnl_amount=market_movement_pnl, source=PnLSource.MARKET_MOVEMENT, description=f'Market movement PnL: ${price_change:.2f} price change on {base_quantity} shares', position_qty=base_quantity, price=price_change, correlation_id=correlation_id)

    def add_trade_pnl(self, symbol: str, trade_qty: float, execution_price: float, avg_cost: float, fees: float=0, slippage: float=0, correlation_id: str | None=None) -> None:
        """Add PnL from a completed trade with detailed attribution."""
        if trade_qty != 0:
            realized_pnl = trade_qty * (execution_price - avg_cost)
            self._add_pnl_event(symbol=symbol, pnl_amount=realized_pnl, source=PnLSource.POSITION_CHANGE, description=f'Trade PnL: {trade_qty} shares @ ${execution_price:.2f} (cost basis ${avg_cost:.2f})', position_qty=trade_qty, price=execution_price, correlation_id=correlation_id)
        if fees != 0:
            self._add_pnl_event(symbol=symbol, pnl_amount=-abs(fees), source=PnLSource.FEES, description=f'Transaction fees: ${fees:.2f}', position_qty=trade_qty, price=execution_price, correlation_id=correlation_id)
        if slippage != 0:
            self._add_pnl_event(symbol=symbol, pnl_amount=slippage, source=PnLSource.SLIPPAGE, description=f'Execution slippage: ${slippage:.2f}', position_qty=trade_qty, price=execution_price, correlation_id=correlation_id)

    def add_dividend_pnl(self, symbol: str, dividend_amount: float, shares: float, correlation_id: str | None=None) -> None:
        """Add dividend income to PnL attribution."""
        total_dividend = dividend_amount * shares
        self._add_pnl_event(symbol=symbol, pnl_amount=total_dividend, source=PnLSource.DIVIDEND, description=f'Dividend income: ${dividend_amount:.4f} per share on {shares} shares', position_qty=shares, price=dividend_amount, correlation_id=correlation_id)

    def add_manual_adjustment(self, symbol: str, adjustment_amount: float, reason: str, correlation_id: str | None=None) -> None:
        """Add manual PnL adjustment with explanation."""
        self._add_pnl_event(symbol=symbol, pnl_amount=adjustment_amount, source=PnLSource.ADJUSTMENT, description=f'Manual adjustment: {reason}', correlation_id=correlation_id)

    def _add_pnl_event(self, symbol: str, pnl_amount: float, source: PnLSource, description: str, position_qty: float | None=None, price: float | None=None, correlation_id: str | None=None) -> None:
        """Add a PnL event and update tracking."""
        event = PnLEvent(symbol=symbol, pnl_amount=pnl_amount, source=source, description=description, position_qty=position_qty, price=price, correlation_id=correlation_id)
        with self._lock:
            ...
            if source in [PnLSource.POSITION_CHANGE, PnLSource.FEES, PnLSource.SLIPPAGE, PnLSource.DIVIDEND, PnLSource.ADJUSTMENT]:
    ...
    def get_total_pnl_summary(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'total_realized_pnl': self._total_realized_pnl, 'total_unrealized_pnl': total_unrealized, 'total_pnl': self._total_realized_pnl + total_unrealized, 'pnl_by_source': {source.value: amount for source, amount in total_by_source.items()}, 'today_pnl': today_pnl, 'positions_count': len(self._position_snapshots), 'events_count': len(self._pnl_events)}

    def get_recent_pnl_events(self, symbol: str | None=None, sources: list[PnLSource] | None=None, limit: int=50) -> list[dict[str, Any]]:
    ...
    def explain_pnl_change(self, symbol: str, time_window_minutes: int=60) -> dict[str, Any]:
        ...
        for source, amount in pnl_by_source.items():
            if abs(amount) > 0.01:
                ...
                explanations.append(f"{sign} ${abs(amount):.2f} from {source.value.replace('_', ' ')}")
        return {'symbol': symbol, 'time_window_minutes': time_window_minutes, 'total_change': total_change, 'pnl_by_source': {source.value: amount for source, amount in pnl_by_source.items()}, 'explanation': '; '.join(explanations) if explanations else 'No significant PnL changes', 'events_count': len(recent_events), 'events': [event.to_dict() for event in recent_events]}

    def get_position_snapshots(self) -> dict[str, dict[str, Any]]:
        """Get current position snapshots."""
        with self._lock:
            return {symbol: snapshot.to_dict() for symbol, snapshot in self._position_snapshots.items()}
    ...
    def calculate_attribution_statistics(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_pnl_by_source = {source.value: source_amounts[source] / source_counts[source] for source in source_counts}
            ...
            return {'total_events': len(self._pnl_events), 'events_by_source': {source.value: count for source, count in source_counts.items()}, 'total_pnl_by_source': {source.value: amount for source, amount in source_amounts.items()}, 'avg_pnl_by_source': avg_pnl_by_source, 'biggest_gain': biggest_gain.to_dict() if biggest_gain else None, 'biggest_loss': biggest_loss.to_dict() if biggest_loss else None, 'symbols_tracked': len({e.symbol for e in self._pnl_events})}
...
def update_position_for_pnl(symbol: str, quantity: float, avg_cost: float, market_price: float, correlation_id: str | None=None) -> None:
...
def record_trade_pnl(symbol: str, trade_qty: float, execution_price: float, avg_cost: float, fees: float=0, slippage: float=0, correlation_id: str | None=None) -> None:
    """Record PnL from a completed trade."""
    attributor = get_pnl_attributor()
    attributor.add_trade_pnl(symbol, trade_qty, execution_price, avg_cost, fees, slippage, correlation_id)

def record_dividend_income(symbol: str, dividend_per_share: float, shares: float, correlation_id: str | None=None) -> None:
