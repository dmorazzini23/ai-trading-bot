...
class SymbolCosts:
    ...
    def total_holding_cost_bps(
        self, days_held: float = 1.0, is_short: bool = False
    ) -> float:
        """
...
class SymbolCostModel:
        ...
        if cost_file.exists():
            try:
                ...
                for symbol, cost_data in data.items():
                    ...
                    if "updated_at" in cost_data and isinstance(
                        cost_data["updated_at"], str
                    ):
                        cost_data["updated_at"] = datetime.fromisoformat(
                            cost_data["updated_at"]
                        )
    ...
    def calculate_position_impact(
        self, symbol: str, position_value: float, volume_ratio: float = 1.0
    ) -> dict[str, float]:
        """
    ...
    def adjust_position_size(
        ...
        if total_cost_bps <= max_cost_bps:
            cost_info = self.calculate_position_impact(
                symbol, abs(target_size), volume_ratio
            )
        ...
        cost_info = self.calculate_position_impact(
            symbol, abs(adjusted_size), volume_ratio
        )
...
def calculate_execution_cost(
    symbol: str, position_value: float, volume_ratio: float = 1.0
) -> dict[str, float]:
    """
