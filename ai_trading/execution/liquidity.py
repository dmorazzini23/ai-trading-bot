...
class LiquidityAnalyzer:
    ...
    def analyze_liquidity(
        self, symbol: str, market_data: dict, current_price: float = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            dollar_volume_analysis = self._analyze_dollar_volume(
    ...
    def _analyze_volume_patterns(self, market_data: dict) -> dict[str, Any]:
        ...
        try:
            ...
            volume_std = (
                statistics.stdev(recent_volume) if len(recent_volume) > 1 else 0
            )
    ...
    def _analyze_bid_ask_spread(
        self, market_data: dict, current_price: float = None
    ) -> dict[str, Any]:
            ...
            if not bid_data or not ask_data:
                ...
                if price_data and len(price_data) >= 10:
                    ...
                    return {
                        ...
                        "spread_basis_points": (
                            (estimated_spread / current_price * 10000)
                            if current_price
                            else 0
                        ),
    ...
    def _analyze_dollar_volume(
        ...
        try:
            ...
            volume_data = (
                volume_data[-min_length:]
                if hasattr(volume_data, "__getitem__")
                else [volume_data]
            )
            price_data = (
                price_data[-min_length:]
                if hasattr(price_data, "__getitem__")
                else [price_data]
            )
            ...
            volume_relative_to_monthly = (
                current_dollar_volume / monthly_avg if monthly_avg > 0 else 0
            )
    ...
    def _determine_liquidity_level(
        self, volume_analysis: dict, dollar_volume_analysis: dict, spread_analysis: dict
    ) -> LiquidityLevel:
        """Determine overall liquidity level."""
        try:
            # Score components
            ...
            weighted_score = (
                dollar_volume_score * 0.5 + spread_score * 0.3 + volume_score * 0.2
            )
    ...
    def _generate_execution_recommendations(
        ...
        try:
                ...
                recommendations.update(
                    {
                        ...
                        "timing_recommendations": [
                            "Can execute large orders immediately"
                        ],
                    }
                )
                ...
                recommendations.update(
                    {
                        ...
                        "timing_recommendations": [
                            "Good conditions for immediate execution"
                        ],
                    }
                )
            ...
            market_session = market_hours_analysis.get(
                "market_session", MarketHours.REGULAR_HOURS
            )
            ...
            if market_session in [MarketHours.PRE_MARKET, MarketHours.AFTER_HOURS]:
                ...
                recommendations["risk_warnings"].append(
                    "Extended hours trading - reduced liquidity"
                )

            if liquidity_impact == "very_low":
                recommendations["timing_recommendations"].append(
                    "Consider waiting for regular market hours"
                )
    ...
    def _update_liquidity_history(self, symbol: str, analysis_result: dict):
        ...
        try:
            ...
            history_entry = {
                ...
                "avg_dollar_volume": analysis_result["dollar_volume_analysis"].get(
                    "avg_dollar_volume", 0
                ),
                "volume_ratio": analysis_result["volume_analysis"].get(
                    "volume_ratio", 0
                ),
            }
    ...
    def get_liquidity_trend(
        ...
        try:
            if (
                symbol not in self.liquidity_history
                or len(self.liquidity_history[symbol]) < 2
            ):
            ...
            if len(dollar_volumes) >= 2:
                dollar_volume_trend = (
                    (dollar_volumes[-1] - dollar_volumes[0]) / dollar_volumes[0]
                    if dollar_volumes[0] > 0
                    else 0
                )
            ...
            level_values = [
                self._liquidity_level_to_numeric(level) for level in liquidity_levels
            ]
            ...
            return {
                ...
                "current_level": (
                    liquidity_levels[-1] if liquidity_levels else "unknown"
                ),
            }
...
class LiquidityManager:
    ...
    def update_symbol_liquidity(
        self, symbol: str, market_data: dict, current_price: float = None
    ) -> dict[str, Any]:
        """Update liquidity analysis for a symbol."""
        try:
            analysis = self.analyzer.analyze_liquidity(
    ...
    def get_portfolio_liquidity_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            for _symbol, analysis in self.symbol_liquidity.items():
                ...
                liquidity_distribution[level_str] = (
                    liquidity_distribution.get(level_str, 0) + 1
                )
            ...
            high_liquidity_pct = liquidity_percentages.get(
                "high", 0
            ) + liquidity_percentages.get("very_high", 0)
            low_liquidity_pct = liquidity_percentages.get(
                "low", 0
    ...
    def get_illiquid_positions(
        self, threshold: LiquidityLevel = LiquidityLevel.LOW
    ) -> list[dict]:
        ...
        try:
            ...
            for symbol, analysis in self.symbol_liquidity.items():
                ...
                level_numeric = self.analyzer._liquidity_level_to_numeric(
                    liquidity_level
                )
                    ...
                    illiquid_positions.append(
                        {
                            ...
                            "liquidity_level": (
                                ...
                                if hasattr(liquidity_level, "value")
                                else str(liquidity_level)
                            ),
                            "avg_dollar_volume": analysis.get(
                                "dollar_volume_analysis", {}
                            ).get("avg_dollar_volume", 0),
                            "spread_bps": analysis.get("spread_analysis", {}).get(
    ...
    def _update_portfolio_liquidity_score(self):
        ...
        try:
            ...
            self.portfolio_liquidity_score = (
