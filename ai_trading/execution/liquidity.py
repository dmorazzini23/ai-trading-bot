...
class LiquidityAnalyzer:
    ...
    def analyze_liquidity(self, symbol: str, market_data: dict, current_price: float=None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            liquidity_level = self._determine_liquidity_level(volume_analysis, dollar_volume_analysis, spread_analysis)
            execution_recommendations = self._generate_execution_recommendations(liquidity_level, volume_analysis, market_hours_analysis)
            analysis_result = {'symbol': symbol, 'timestamp': datetime.now(UTC), 'liquidity_level': liquidity_level, 'volume_analysis': volume_analysis, 'spread_analysis': spread_analysis, 'dollar_volume_analysis': dollar_volume_analysis, 'market_hours_analysis': market_hours_analysis, 'execution_recommendations': execution_recommendations, 'analysis_time_seconds': (datetime.now(UTC) - analysis_start).total_seconds()}
    ...
    def _analyze_volume_patterns(self, market_data: dict) -> dict[str, Any]:
        ...
        try:
            ...
            return {'avg_volume': avg_volume, 'current_volume': current_volume, 'volume_ratio': volume_ratio, 'volume_trend': volume_trend, 'volume_volatility': volume_cv, 'volume_pattern': volume_pattern, 'data_points': len(volume_data)}
        except COMMON_EXC as e:
        ...
        try:
            ...
            if not bid_data or not ask_data:
                ...
                if price_data and len(price_data) >= 10:
                    ...
                    estimated_spread = price_std * 2
                    return {'estimated_spread': estimated_spread, 'spread_basis_points': estimated_spread / current_price * 10000 if current_price else 0, 'spread_quality': 'estimated'}
                else:
                    return {'error': 'Insufficient price data for spread estimation'}
            spreads = []
            for bid, ask in zip(bid_data, ask_data, strict=False):
            ...
            if spread_bps < 5:
                ...
                spread_quality = 'poor'
            return {'avg_spread': avg_spread, 'current_spread': current_spread, 'spread_std': spread_std, 'spread_basis_points': spread_bps, 'current_spread_bps': current_spread_bps, 'spread_quality': spread_quality, 'data_points': len(spreads)}
        except COMMON_EXC as e:
    ...
    def _analyze_dollar_volume(self, market_data: dict, current_price: float=None) -> dict[str, Any]:
        ...
        try:
            ...
            dollar_volumes = []
            for vol, price in zip(volume_data, price_data, strict=False):
            ...
            return {'avg_dollar_volume': avg_dollar_volume, 'current_dollar_volume': current_dollar_volume, 'monthly_avg_dollar_volume': monthly_avg, 'volume_relative_to_monthly': volume_relative_to_monthly, 'data_points': len(dollar_volumes)}
        except COMMON_EXC as e:
    ...
    def _analyze_market_hours_liquidity(self) -> dict[str, Any]:
        ...
        try:
            ...
            if is_friday and hour >= 15:
                ...
                liquidity_impact = 'elevated'
            return {'market_session': market_session, 'liquidity_impact': liquidity_impact, 'is_weekend': weekday >= 5, 'is_friday': is_friday, 'is_monday': is_monday, 'is_option_expiry_week': is_option_expiry, 'current_hour': hour}
        except COMMON_EXC as e:
    ...
    def _determine_liquidity_level(self, volume_analysis: dict, dollar_volume_analysis: dict, spread_analysis: dict) -> LiquidityLevel:
    ...
    def _generate_execution_recommendations(self, liquidity_level: LiquidityLevel, volume_analysis: dict, market_hours_analysis: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if liquidity_level == LiquidityLevel.VERY_HIGH:
                ...
                recommendations.update({'recommended_order_type': OrderType.MARKET, 'max_participation_rate': 0.15, 'execution_strategy': 'normal', 'timing_recommendations': ['Good conditions for immediate execution']})
            elif liquidity_level == LiquidityLevel.NORMAL:
                recommendations.update({'recommended_order_type': OrderType.LIMIT, 'max_participation_rate': 0.1, 'execution_strategy': 'patient', 'timing_recommendations': ['Use limit orders for better pricing']})
            elif liquidity_level == LiquidityLevel.LOW:
                recommendations.update({'recommended_order_type': OrderType.LIMIT, 'max_participation_rate': 0.05, 'execution_strategy': 'cautious', 'timing_recommendations': ['Break up large orders', 'Use TWAP strategy'], 'risk_warnings': ['Low liquidity may cause slippage']})
            else:
                recommendations.update({'recommended_order_type': OrderType.LIMIT, 'max_participation_rate': 0.02, 'execution_strategy': 'very_cautious', 'timing_recommendations': ['Avoid large orders', 'Consider waiting for better liquidity'], 'risk_warnings': ['Very low liquidity', 'High slippage risk', 'Consider alternative timing']})
    ...
    def _update_liquidity_history(self, symbol: str, analysis_result: dict):
        ...
        try:
            ...
            history_entry = {'timestamp': analysis_result['timestamp'], 'liquidity_level': analysis_result['liquidity_level'], 'avg_dollar_volume': analysis_result['dollar_volume_analysis'].get('avg_dollar_volume', 0), 'volume_ratio': analysis_result['volume_analysis'].get('volume_ratio', 0)}
    ...
    def get_liquidity_trend(self, symbol: str, lookback_periods: int=10) -> dict[str, Any]:
        ...
        try:
            ...
            return {'trend_direction': trend_direction, 'dollar_volume_trend': dollar_volume_trend, 'avg_volume_ratio': avg_volume_ratio, 'periods_analyzed': len(recent_history), 'current_level': liquidity_levels[-1] if liquidity_levels else 'unknown'}
        except COMMON_EXC as e:
    ...
    def _liquidity_level_to_numeric(self, level: LiquidityLevel) -> int:
        ...
        level_map = {LiquidityLevel.VERY_LOW: 1, LiquidityLevel.LOW: 2, LiquidityLevel.NORMAL: 3, LiquidityLevel.HIGH: 4, LiquidityLevel.VERY_HIGH: 5}
        return level_map.get(level, 3)

    def calculate_optimal_order_size(self, target_size: int, liquidity_level: LiquidityLevel, avg_volume: float, participation_rate: float=None) -> dict[str, Any]:
        ...
        try:
            ...
            if target_size <= max_size_from_volume:
                return {'recommended_size': target_size, 'execution_method': 'single_order', 'estimated_slices': 1, 'size_adjustment': 'none'}
            else:
                recommended_size = max_size_from_volume
                estimated_slices = max(2, int(target_size / max_size_from_volume))
                return {'recommended_size': recommended_size, 'execution_method': 'sliced_orders', 'estimated_slices': estimated_slices, 'size_adjustment': 'reduced_for_liquidity', 'original_size': target_size, 'reduction_ratio': recommended_size / target_size}
        except COMMON_EXC as e:
...
class LiquidityManager:
    ...
    def get_portfolio_liquidity_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            liquidity_percentages = {level: count / total_symbols * 100 for level, count in liquidity_distribution.items()}
            high_liquidity_pct = liquidity_percentages.get('high', 0) + liquidity_percentages.get('very_high', 0)
            low_liquidity_pct = liquidity_percentages.get('low', 0) + liquidity_percentages.get('very_low', 0)
            ...
            return {'total_symbols': total_symbols, 'liquidity_distribution': liquidity_distribution, 'liquidity_percentages': liquidity_percentages, 'portfolio_liquidity_score': self.portfolio_liquidity_score, 'portfolio_assessment': portfolio_assessment, 'high_liquidity_percentage': high_liquidity_pct, 'low_liquidity_percentage': low_liquidity_pct, 'last_updated': datetime.now(UTC)}
        except COMMON_EXC as e:
        ...
        try:
            ...
            for symbol, analysis in self.symbol_liquidity.items():
                ...
                if level_numeric <= threshold_numeric:
                    illiquid_positions.append({'symbol': symbol, 'liquidity_level': liquidity_level.value if hasattr(liquidity_level, 'value') else str(liquidity_level), 'avg_dollar_volume': analysis.get('dollar_volume_analysis', {}).get('avg_dollar_volume', 0), 'spread_bps': analysis.get('spread_analysis', {}).get('spread_basis_points', 0), 'last_updated': analysis.get('timestamp')})
