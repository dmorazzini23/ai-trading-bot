...
class HealthMonitor:
    ...
    async def run_all_checks(self) -> List[HealthCheckResult]:
        ...
        for checker in self.checkers.values():
            ...
            if (checker.last_check is None or 
                datetime.now(timezone.utc) - checker.last_check >= timedelta(seconds=checker.interval_seconds)):
    ...
    def _collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            return SystemMetrics(
                ...
                memory_used_gb=0, memory_available_gb=0,
                disk_used_gb=0, disk_available_gb=0,
    ...
    def _process_alerts(self) -> None:
        ...
        recent_results = [
            ...
            if result.timestamp > datetime.now(timezone.utc) - timedelta(minutes=5)
        ]
        ...
        for component, results in component_results.items():
            ...
            if latest_result.status == HealthStatus.CRITICAL:
                self._send_alert(
                    f"CRITICAL: {component} health check failed",
                    latest_result
                )
            ...
            if (checker and 
                checker.consecutive_failures >= self.thresholds['consecutive_failures_critical']):
                self._send_alert(
