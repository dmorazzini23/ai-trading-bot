        ...
        if mp_mode == 'AUTO':
            ...
            try:
                if hasattr(tcfg, 'max_position_size'):
                    ...
                    logger.warning('CONFIG_AUTOFIX_FALLBACK_APPLIED_VIA_ENV', extra={'field': 'max_position_size', 'fallback': float(fallback)})
            except (ValueError, OSError, TypeError, ZeroDivisionError, OverflowError, KeyError) as e:
                logger.warning('CONFIG_AUTOFIX_FALLBACK_APPLIED_VIA_ENV', extra={'field': 'max_position_size', 'fallback': float(fallback), 'error': repr(e)})
...
def main(argv: list[str] | None=None) -> None:
    ...
    config = get_settings()
    S = get_settings()
    logger.info('DATA_CONFIG feed=%s adjustment=%s timeframe=1Day/1Min provider=alpaca', S.alpaca_data_feed, S.alpaca_adjustment)
    try:
        _validate_runtime_config(config, S)
    except ValueError as e:
        logger.critical('RUNTIME_CONFIG_INVALID', extra={'error': str(e)})
        raise
    session = build_retrying_session(pool_maxsize=int(getattr(config, 'http_pool_maxsize', 32)), total_retries=int(getattr(config, 'http_total_retries', 3)), backoff_factor=float(getattr(config, 'http_backoff_factor', 0.3)), connect_timeout=float(getattr(config, 'http_connect_timeout', 5.0)), read_timeout=float(getattr(config, 'http_read_timeout', 10.0)))
    set_global_session(session)
    logger.info('REQUESTS_POOL_STATS', extra={'transport': 'requests', 'pool_maxsize': getattr(config, 'http_pool_maxsize', 32), 'retries': getattr(config, 'http_total_retries', 3), 'backoff_factor': getattr(config, 'http_backoff_factor', 0.3), 'connect_timeout': getattr(config, 'http_connect_timeout', 5.0), 'read_timeout': getattr(config, 'http_read_timeout', 10.0)})
    try:
    ...
    banner = {'mode': getattr(config, 'trading_mode', 'balanced'), 'paper': getattr(config, 'paper', True), 'alpaca_base_url': getattr(config, 'alpaca_base_url', ''), 'capital_cap': _as_float(getattr(S, 'capital_cap', 0.0), 0.0), 'dollar_risk_limit': _as_float(getattr(S, 'dollar_risk_limit', 0.0), 0.0), 'max_position_mode': str(getattr(S, 'max_position_mode', getattr(config, 'max_position_mode', 'STATIC'))).upper(), 'max_position_size': _as_float(getattr(S, 'max_position_size', None), 0.0)}
    ...
    raw_iter = args.iterations if args.iterations is not None else env_iter if env_iter is not None else getattr(S, 'iterations', None) or getattr(S, 'scheduler_iterations', 0)
    try:
