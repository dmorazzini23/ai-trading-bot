...
class RateLimiter:
    ...
    def _setup_default_routes(self) -> None:
        ...
        default_routes = {
            "orders": RateLimitConfig(
                capacity=50, refill_rate=10.0
            ),  # 10 orders/sec, burst 50
            "bars": RateLimitConfig(
                capacity=200, refill_rate=50.0
            ),  # 50 requests/sec, burst 200
            "quotes": RateLimitConfig(
                capacity=100, refill_rate=20.0
            ),  # 20 quotes/sec, burst 100
            "positions": RateLimitConfig(
                capacity=20, refill_rate=5.0
            ),  # 5 requests/sec, burst 20
            "account": RateLimitConfig(
                capacity=10, refill_rate=2.0
            ),  # 2 requests/sec, burst 10
        }
    ...
    def configure_route(self, route: str, config: RateLimitConfig) -> None:
        ...
        self.logger.info(
            f"Configured rate limit for {route}: "
    ...
    async def acquire(
        self, route: str, tokens: int = 1, timeout: float | None = None
    ) -> bool:
        """
        ...
        async with self._async_lock:
            ...
            if can_proceed:
                ...
                self._metrics[route]["denials"] += 1
                self.logger.warning(
                    f"Rate limit still exceeded for {route} after waiting"
                )
                return False
...
async def rate_limit_async(
    route: str, tokens: int = 1, timeout: float | None = None
) -> bool:
