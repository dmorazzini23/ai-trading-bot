...
class RateLimiter:
    ...
    def _setup_default_routes(self) -> None:
        ...
        default_routes = {'orders': RateLimitConfig(capacity=50, refill_rate=10.0), 'bars': RateLimitConfig(capacity=200, refill_rate=50.0), 'quotes': RateLimitConfig(capacity=100, refill_rate=20.0), 'positions': RateLimitConfig(capacity=20, refill_rate=5.0), 'account': RateLimitConfig(capacity=10, refill_rate=2.0)}
    ...
    def configure_route(self, route: str, config: RateLimitConfig) -> None:
        ...
        self.logger.info(f'Configured rate limit for {route}: {config.capacity} capacity, {config.refill_rate}/sec refill')
    ...
    def get_status(self, route: str | None=None) -> dict[str, Any]:
        ...
        with self._lock:
            if route is not None:
                ...
                return {'route': route, 'available_tokens': bucket.available_tokens(), 'capacity': config.capacity if config else 0, 'refill_rate': config.refill_rate if config else 0, 'enabled': config.enabled if config else False, 'metrics': dict(metrics)}
            status = {'global': {'available_tokens': self._global_bucket.available_tokens(), 'capacity': self._global_bucket.capacity, 'refill_rate': self._global_bucket.refill_rate}, 'routes': {}}
