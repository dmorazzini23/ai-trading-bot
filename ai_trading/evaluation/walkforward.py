...
class WalkForwardEvaluator:
    ...
    def __init__(
        self,
        mode: str = "rolling",
        train_span: int | timedelta = 252,  # 1 year
        test_span: int | timedelta = 21,  # 1 month
    ...
    def run_walkforward(
        ...
        try:
            ...
            for fold_idx, split_info in enumerate(splits):
                ...
                if "predictions" in fold_result and "actual" in fold_result:
                    ...
                    if len(fold_result["predictions"]) > 0:
                        fold_return = (
                            np.mean(fold_result["predictions"]) * 0.01
                        )  # Convert to return
            ...
            # Create equity curve
            equity_dates = [split["test_start"] for split in splits] + [
                splits[-1]["test_end"]
            ]
            self.equity_curve = pd.Series(
                equity_values, index=equity_dates[: len(equity_values)]
            )
            ...
            logger.info(
                f"Walk-forward analysis completed. Final return: {results['total_return']:.2%}"
            )
            return results
    ...
    def _calculate_fold_metrics(
        ...
        try:
            ...
            hit_rate_long = (
                np.mean(y_true[long_signals] > 0) if np.any(long_signals) else 0.0
            )
            hit_rate_short = (
                np.mean(y_true[short_signals] < 0) if np.any(short_signals) else 0.0
            )
    ...
    def _calculate_aggregate_metrics(
        ...
        try:
            ...
            correlation = (
                np.corrcoef(actual, predictions)[0, 1] if len(predictions) > 1 else 0.0
            )
            ...
            if len(downside_returns) > 0:
                sortino_ratio = (
                    np.mean(predictions) / np.std(downside_returns) * np.sqrt(252)
                )
            else:
            ...
            max_drawdown = (
                self.drawdown_series.max() if len(self.drawdown_series) > 0 else 0.0
            )
            ...
            total_days = sum(
                split["period_days"]
                for split in self.fold_results
                if "metrics" in split
            )
        ...
        try:
            ...
            if self.fold_results:
                ...
                fold_file = os.path.join(
                    self.output_dir, f"walkforward_folds_{timestamp}.csv"
                )
            ...
            if self.aggregate_results:
                agg_file = os.path.join(
                    self.output_dir, f"walkforward_aggregate_{timestamp}.json"
                )
            ...
            if len(self.equity_curve) > 0:
                equity_file = os.path.join(
                    self.output_dir, f"equity_curve_{timestamp}.csv"
                )
    ...
    def _create_plots(self, timestamp: str) -> None:
        ...
        try:
            ...
            fig.suptitle(
                f"Walk-Forward Analysis Results ({self.mode.title()})", fontsize=16
            )

            # Equity curve
            if len(self.equity_curve) > 0:
                ax1.plot(
                    self.equity_curve.index, self.equity_curve.values, "b-", linewidth=2
                )
            ...
            if len(self.drawdown_series) > 0:
                ax2.fill_between(
                    self.drawdown_series.index,
                    0,
                    -self.drawdown_series.values,
                    color="red",
                    alpha=0.6,
                )
            ...
            if self.fold_results:
                fold_metrics = [
                    r.get("metrics", {}) for r in self.fold_results if "metrics" in r
                ]
                if fold_metrics:
                    correlations = [m.get("correlation", 0) for m in fold_metrics]
                    ax3.plot(
                        range(len(correlations)), correlations, "g-o", markersize=4
                    )
                    ...
                    ax4.plot(
                        range(len(dir_acc)), dir_acc, "orange", marker="s", markersize=4
                    )
                    ax4.axhline(
                        y=0.5, color="r", linestyle="--", alpha=0.5, label="Random"
                    )
            ...
            plot_file = os.path.join(
                self.output_dir, f"walkforward_plots_{timestamp}.png"
            )
...
def run_walkforward_smoke_test() -> None:
    ...
    try:
        ...
        data = pd.DataFrame(
            {
                ...
                "feature_3": prices * 0.01
                + np.random.randn(n_samples) * 0.1,  # Somewhat predictive
        ...
        logger.info(
            f"  - Net Sharpe: {results['aggregate_metrics'].get('net_sharpe', 'N/A')}"
        )
