...
class HyperparametersSchema(BaseModel):
    ...
    stop_loss_pct: float = Field(
        default=0.02, ge=0.001, le=0.5, description="Stop loss percentage"
    )
    take_profit_pct: float = Field(
        default=0.04, ge=0.001, le=1.0, description="Take profit percentage"
    )

    # Signal parameters
    signal_lookback_days: int = Field(
        default=30, ge=1, le=365, description="Lookback period for signal calculation"
    )
    signal_confirmation_bars: int = Field(
        default=2, ge=1, le=10, description="Number of bars for signal confirmation"
    )
    ...
    rebalance_frequency: str = Field(
        default="daily", description="Portfolio rebalancing frequency"
    )

    # Model parameters
    model_retrain_days: int = Field(
        default=7, ge=1, le=30, description="Days between model retraining"
    )
    model_confidence_threshold: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Minimum model confidence for trading"
    )

    # Execution parameters
    order_timeout_seconds: int = Field(
        default=300, ge=30, le=3600, description="Order timeout in seconds"
    )
    ...
    use_sentiment_analysis: bool = Field(
        default=True, description="Enable sentiment analysis features"
    )
    use_technical_indicators: bool = Field(
        default=True, description="Enable technical indicator features"
    )
    use_fundamental_data: bool = Field(
        default=False, description="Enable fundamental data features"
    )

    # Metadata
    created_at: str | None = Field(
        default=None, description="Timestamp when hyperparameters were created"
    )
    updated_at: str | None = Field(
        default=None, description="Timestamp when hyperparameters were last updated"
    )
...
def save_hyperparams(
    hyperparams: HyperparametersSchema, file_path: str = "hyperparams.json"
) -> bool:
    """
...
def validate_hyperparams_file(file_path: str = "hyperparams.json") -> dict[str, Any]:
    ...
    if os.path.exists(file_path):
        ...
        try:
            ...
            report["version_compatible"] = _is_compatible_version(
                file_version, HYPERPARAMS_SCHEMA_VERSION
            )
