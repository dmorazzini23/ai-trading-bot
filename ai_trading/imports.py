    ...
    class MockNumpy:
        ...
        class _MockRandom:
            def seed(self, seed: int) -> None:
                pass
            def rand(self, *shape) -> Union[float, List]:
                if not shape:
                    return 0.5
                return [0.5] * shape[0] if len(shape) == 1 else [[0.5]]
            def randn(self, *shape) -> Union[float, List]:
        ...
        def full(self, shape: Union[int, Tuple], fill_value: float) -> List:
            if isinstance(shape, int):
                return [fill_value] * shape
            return [[fill_value] * shape[1] for _ in range(shape[0])]
        
        # Mathematical operations
        def mean(self, data: Any, axis: Optional[int] = None) -> float:
            if isinstance(data, (list, tuple)) and data:
                return sum(data) / len(data)
            return 0.0
        
        def std(self, data: Any, axis: Optional[int] = None) -> float:
            return 1.0
        
        def sum(self, data: Any, axis: Optional[int] = None) -> float:
            if isinstance(data, (list, tuple)):
                return sum(data)
            return 0.0
        
        def max(self, data: Any, axis: Optional[int] = None) -> float:
            if isinstance(data, (list, tuple)) and data:
                return max(data)
            return 0.0
        
        def min(self, data: Any, axis: Optional[int] = None) -> float:
            if isinstance(data, (list, tuple)) and data:
                return min(data)
            return 0.0
...
try:
    ...
    class MockSeries:
        ...
        def __init__(self, data: Any = None, index: Any = None, name: Any = None):
            self.data = data if isinstance(data, (list, tuple)) else [data] if data is not None else []
        ...
        def fillna(self, value: Any) -> 'MockSeries':
            return MockSeries([value if x is None else x for x in self.data], name=self.name)
    ...
    class MockRolling:
        ...
        def std(self) -> MockSeries:
            return MockSeries([1.0] * len(self.series.data), self.series.index, self.series.name)
    ...
    class MockDataFrame:
        ...
        def __init__(self, data: Any = None, index: Any = None, columns: Any = None):
            if isinstance(data, dict):
                ...
                self.data = {col: [row[i] if i < len(row) else None for row in data] 
                           for i, col in enumerate(self.columns)} if data else {}
            else:
                self.data = {}
        ...
        def __getitem__(self, key):
            if isinstance(key, str):
                ...
                return MockDataFrame({k: self.data.get(k, []) for k in key}, self.index, key)
        ...
        def mean(self) -> MockSeries:
            ...
            for col, values in self.data.items():
                ...
                means[col] = sum(numeric_values) / len(numeric_values) if numeric_values else 0.0
        ...
        def values(self) -> List[List]:
            return [[self.data[col][i] for col in self.columns] for i in range(len(self.index))]
