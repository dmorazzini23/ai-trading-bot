...
from ai_trading.alpaca_api import get_api_error_cls
...
from ai_trading.data.finnhub import FinnhubAPIException, fh_fetcher
...
def _resolve_consecutive_failure_threshold() -> int:
    """Return the minimum consecutive Alpaca failures required before fallback."""
    ...
    if settings is not None:
        for attr in (
            "alpaca_consecutive_failure_threshold",
            "alpaca_failure_consecutive_threshold",
        ):
    ...
    if candidate is None:
        try:
            candidate = get_env(
                "AI_TRADING_ALPACA_CONSECUTIVE_FAILURE_THRESHOLD",
                None,
                cast=int,
            )
        except Exception:
...
_ALPACA_CONSECUTIVE_FAILURE_THRESHOLD = _resolve_consecutive_failure_threshold()
_ALPACA_CONSECUTIVE_FAILURES: dict[tuple[str, str], int] = {}


def _consecutive_failure_key(symbol: str, timeframe: str | None) -> tuple[str, str]:
    return (str(symbol or "").upper(), str(timeframe or "").lower())


def _record_consecutive_failure(symbol: str, timeframe: str | None) -> int:
    key = _consecutive_failure_key(symbol, timeframe)
    count = _ALPACA_CONSECUTIVE_FAILURES.get(key, 0) + 1
    _ALPACA_CONSECUTIVE_FAILURES[key] = count
    return count


def _consecutive_failure_count(symbol: str, timeframe: str | None) -> int:
    return _ALPACA_CONSECUTIVE_FAILURES.get(_consecutive_failure_key(symbol, timeframe), 0)


def _clear_consecutive_failures(symbol: str, timeframe: str | None = None) -> None:
    symbol_key = str(symbol or "").upper()
    if timeframe is None:
        to_clear = [key for key in _ALPACA_CONSECUTIVE_FAILURES if key[0] == symbol_key]
    else:
        to_clear = [_consecutive_failure_key(symbol, timeframe)]
    for key in to_clear:
        _ALPACA_CONSECUTIVE_FAILURES.pop(key, None)
...
def _record_alpaca_failure_event(
    symbol: str,
    ...
    *,
    ...
    while queue and queue[0] < cutoff:
        queue.popleft()
    _record_consecutive_failure(symbol, timeframe)
    return len(queue)
...
def _clear_alpaca_failure_events(symbol: str, timeframe: str | None = None) -> None:
    _ALPACA_FAILURE_EVENTS.pop(symbol, None)
    _clear_consecutive_failures(symbol, timeframe)


def _yahoo_fallback_allowed(symbol: str, timeframe: str | None = None, *, force: bool = False) -> bool:
    if force:
        return True
    ...
    if fail_count >= max(threshold, 1):
        return True
    recent_failures = _alpaca_failure_count(symbol)
    return recent_failures >= _yahoo_failure_threshold() or consecutive >= required
...
def _safe_exception_message(exc: Exception | None, *, limit: int = 200) -> str | None:
    ...
    if isinstance(message, Mapping):
        message = message.get("message") or message.get("detail") or message.get("error")
    if not message:
        message = str(exc)
...
def _set_global_backup_skip(timeframe: str, *, until: datetime | float | None = None) -> None:
    ...
    if until_dt is None:
        return
    current_until = _GLOBAL_BACKUP_SKIP_UNTIL.get(key)
...
def _clear_global_backup_skip(timeframe: str) -> None:
    ...
    if key:
        _GLOBAL_BACKUP_SKIP_UNTIL.pop(key, None)
        _clear_backup_probe_watch(key)


def _timeframe_has_active_backup_skip(timeframe: str) -> bool:
    """Return True when any symbol retains an active backup skip for ``timeframe``."""

    tf = str(timeframe or "").strip()
    if not tf:
        return False
    now_dt = datetime.now(tz=UTC)
    for (symbol_key, timeframe_key), until_raw in list(_BACKUP_SKIP_UNTIL.items()):
        if str(timeframe_key or "").strip() != tf:
            continue
...
def _backup_probe_watch_enabled(timeframe: str) -> bool:
...
def _record_primary_probe_seen(
    timeframe: str,
    *,
    symbol: str | None = None,
    route: str | None = None,
) -> None:
    """Record that a primary recovery probe was attempted for ``timeframe``."""

    tf_key = str(timeframe or "").strip()
    if not tf_key:
        return
    now_dt = datetime.now(tz=UTC)
    _BACKUP_PRIMARY_PROBE_LAST_SEEN_AT[tf_key] = now_dt
    _BACKUP_PRIMARY_PROBE_ALERT_AT.pop(tf_key, None)
    if (
        _get_global_backup_skip_until(tf_key) is None
        and not _timeframe_has_active_backup_skip(tf_key)
    ):
        _BACKUP_SKIP_ACTIVE_SINCE.pop(tf_key, None)


def _clear_backup_probe_watch(timeframe: str) -> None:
    tf_key = str(timeframe or "").strip()
    if not tf_key:
        return
    _BACKUP_SKIP_ACTIVE_SINCE.pop(tf_key, None)
...
def _note_backup_skip_activity(
    timeframe: str,
    *,
    symbol: str | None = None,
    ...
    """Emit an alert when backup skip remains active without recent primary probes."""

    if not _backup_probe_watch_enabled(timeframe):
        return
    tf_key = str(timeframe or "").strip()
    if not tf_key:
        return
    now_dt = datetime.now(tz=UTC)
    active_since = _BACKUP_SKIP_ACTIVE_SINCE.setdefault(tf_key, now_dt)
    threshold = _backup_probe_missing_threshold()
    if threshold <= timedelta(0):
        return
    last_probe_dt = _BACKUP_PRIMARY_PROBE_LAST_SEEN_AT.get(tf_key)
    reference_dt = last_probe_dt if isinstance(last_probe_dt, datetime) else active_since
    missing_duration = now_dt - reference_dt
    if missing_duration < threshold:
        return

    cooldown = _backup_probe_alert_cooldown()
    last_alert_dt = _BACKUP_PRIMARY_PROBE_ALERT_AT.get(tf_key)
    if (
        isinstance(last_alert_dt, datetime)
        and cooldown > timedelta(0)
        and (now_dt - last_alert_dt) < cooldown
    ):
        return

    probe_interval = _backup_primary_probe_interval()
    payload = {
        "provider": "alpaca",
        "timeframe": tf_key,
        "symbol": symbol,
        "global_skip": bool(global_skip),
        "skip_until": skip_until.isoformat() if isinstance(skip_until, datetime) else None,
        "skip_active_since": active_since.isoformat(),
        "last_probe_at": last_probe_dt.isoformat() if isinstance(last_probe_dt, datetime) else None,
        "missing_duration_s": round(max(0.0, missing_duration.total_seconds()), 3),
        "threshold_s": round(max(0.0, threshold.total_seconds()), 3),
        "probe_interval_s": round(max(0.0, probe_interval.total_seconds()), 3),
    }
    _incr(
        "data.fetch.primary_probe_missing",
        value=1.0,
        tags={"provider": "alpaca", "timeframe": tf_key},
    )
    logger.error(
        "ALERT_PRIMARY_RECOVERY_PROBE_MISSING",
        extra=_norm_extra(payload),
    )
    try:
...
def _mark_fallback(
    ...
    * Appends ``(provider, symbol)`` to the shared registries in
    ...
    if not reason_value and using_backup_flag:
        reason_value = "upstream_unavailable"
    ...
    runtime_state.update_data_provider_state(
        ...
        status="degraded" if using_backup_flag else "healthy",
        data_status="degraded" if using_backup_flag else "healthy",
        timeframe=timeframe,
        safe_mode=is_safe_mode_active(),
...
def _set_backup_skip(symbol: str, timeframe: str, *, until: datetime | float | None = None) -> None:
    key = (symbol, timeframe)
    now_dt = datetime.now(tz=UTC)
    existing_until = _BACKUP_SKIP_UNTIL.get(key)
    ...
    if until is not None:
        ...
        _SKIPPED_SYMBOLS.add(key)
        effective_until = existing_until if active_existing_skip else until_dt
        if _get_global_backup_skip_until(timeframe) is None:
            _set_global_backup_skip(timeframe, until=effective_until)
        return
    _SKIPPED_SYMBOLS.add(key)
    if active_existing_skip:
        if key not in _BACKUP_PRIMARY_PROBE_AT:
            _schedule_backup_primary_probe(key)
        if _get_global_backup_skip_until(timeframe) is None:
...
def _clear_backup_skip(symbol: str, timeframe: str) -> None:
    ...
    tf_key = str(timeframe or "").strip()
    if tf_key and _get_global_backup_skip_until(tf_key) is None and not _timeframe_has_active_backup_skip(tf_key):
        _clear_backup_probe_watch(tf_key)
    ...
    *,
    ...
    if cleared:
        ...
        if not _timeframe_has_active_backup_skip(timeframe):
            _clear_global_backup_skip(timeframe)
        runtime_state.update_data_provider_state(
            primary=primary_canon,
...
def ensure_ohlcv_schema(
    ...
    *,
    ...
    warn_prefix = f"{source or 'unknown'!s}:{frequency or 'unknown'!s}"
...
def _flatten_and_normalize_ohlcv(
    ...
    for canonical in canonical_primary:
    ...
    if close_series is not None:
        ...
        if all_nan:
            ...
            err.fetch_reason = "close_column_all_nan"
            err.symbol = symbol
...
def _verify_minute_continuity(df: pd.DataFrame | None, symbol: str, backfill: str | None = None) -> pd.DataFrame | None:
    ...
    _record_gap_ratio_state(
        ...
        metadata={
            ...
            "missing_after": missing_minutes,
            "gap_ratio": gap_ratio,
            "detector": "verify_minute_continuity",
        },
    )
    if not backfill:
        return df
...
def _repair_rth_minute_gaps(
    ...
    *,
    ...
    _record_gap_ratio_state(gap_ratio, metadata=metadata)
    emit_gap_event = (
        not tolerated
        and (
    ...
    *,
    ...
    def _run_backup_fetch(interval_code: str, *, from_provider: str | None = None) -> Any:
        ...
        if (
            ...
            and int(_ALPACA_CONSECUTIVE_FAILURE_THRESHOLD) > 1
            and not _yahoo_fallback_allowed(
                symbol,
                _interval,
                force=force_yahoo_flag,
            )
        ):
    ...
    if isinstance(global_skip_until_dt, datetime) and skip_until_dt is None:
    ...
    if skip_window_active and bypass_backup_skip:
        skip_window_active = False
        logger.info(
            "ALPACA_PRIMARY_RECOVERY_PROBE_BYPASS_SKIP",
            extra=_norm_extra(
                {
                    "symbol": symbol,
                    "timeframe": _interval,
                    ...
                    "global_skip": skip_from_global,
                }
            ),
        )
        _record_primary_probe_seen(
            _interval,
            symbol=symbol,
            route="bypass_backup_skip",
        )
    if skip_window_active and tf_key in _SKIPPED_SYMBOLS:
        ...
        if fb_interval:
            ...
            _note_backup_skip_activity(
                _interval,
                symbol=symbol,
                skip_until=skip_until_dt if isinstance(skip_until_dt, datetime) else None,
                global_skip=bool(skip_from_global),
            )
            try:
                _state["empty_data_logged"] = True
            except Exception:
    ...
    if explicit_feed_request and _feed == "sip":
        override = globals().get("_ALLOW_SIP")
        if override is True or (override is None and _pytest_active()):
    ...
    def _req(
        ...
        *,
        ...
        def _current_fetch_attempt() -> int:
            retries_val = 0
            try:
        ...
        def _emit_fetch_attempt_log(
            *,
            ...
            payload = _build_fetch_log_extra(
                extra,
                symbol=symbol,
                timeframe=_interval,
                attempt=_current_fetch_attempt(),
                status=status,
                exception=exception,
        ...
        def _report_provider_health_failure(reason_text: str, *, severity: str = "hard_fail") -> None:
            ...
            backup_val = _state.get("backup_label")
            if not primary_val or not backup_val:
                return
            try:
                provider_monitor.update_data_health(
                    primary_val,
        ...
        try:
            ...
            _report_provider_health_failure("connection_error")
            _record_alpaca_failure_event(symbol, timeframe=_interval)
            fallback_target = fallback or _select_fallback_target(_interval, _feed, _start, _end)
            if fallback_target:
                result = _attempt_fallback(fallback_target, skip_check=True)
            ...
            if response_obj is not None:
                status_hint = getattr(response_obj, "status_code", None)
                try:
                    if status_hint is not None:
                        status_hint = int(status_hint)
                except (TypeError, ValueError):
                    status_hint = None
            ...
            if isinstance(e, ValueError) and str(e) == "rate_limited":
                ...
                report_failure = True
            elif status_hint == 429:
                error_reason = "rate_limited"
                report_failure = True
            elif status_hint in (401, 403):
                error_reason = "unauthorized"
            elif status_hint is not None and 500 <= status_hint < 600:
            ...
            if report_failure:
                provider_monitor.record_failure("alpaca", error_reason, str(e))
                _report_provider_health_failure(error_reason)
            _record_alpaca_failure_event(symbol, timeframe=_interval)
            fallback_target = fallback or _select_fallback_target(_interval, _feed, _start, _end)
            if fallback_target:
                result = _attempt_fallback(fallback_target, skip_check=True)
        ...
        if status != 400:
            ...
            if "json" in ctype or (not text and hasattr(resp, "json")):
        ...
        if df.empty:
            ...
            if (
                ...
                _emit_fetch_attempt_log(status=status, error="empty", extra=log_payload)
                logged_attempt = True
                time.sleep(retry_delay)
        ...
        _emit_fetch_attempt_log(status=status, extra=log_extra_success)
        provider_monitor.record_success("alpaca")
        _ALPACA_DISABLED_ALERTED = False
        _alpaca_disable_count = 0
        success_tags = _tags()
    ...
    if df is not None and not getattr(df, "empty", True):
        ...
        last_fallback_feed = _state.get("last_fallback_feed")
        if last_fallback_feed in (None, "sip", "iex"):
            _clear_alpaca_failure_events(symbol, timeframe=_interval)
    ...
    *,
    ...
    if skip_window_active:
        _note_backup_skip_activity(
            ...
            symbol=symbol,
            skip_until=skip_until_dt if isinstance(skip_until_dt, datetime) else None,
            global_skip=bool(global_skip_window_active),
        )
    ...
    backup_label = (backup_provider_normalized or (backup_provider_str.lower() if backup_provider_str else "")) or ""
    backup_label = backup_label.strip()
    primary_label = forced_provider_label if forced_provider_label else f"alpaca_{normalized_feed or _DEFAULT_FEED}"
    ...
    if forced_provider_label:
        ...
        fallback_allowed_flag = True
        backup_label = forced_provider_label
    ...
    def _record_minute_fallback(
        *,
        ...
        coverage_meta_state = _state.get("coverage_meta")
        if isinstance(coverage_meta_state, Mapping):
            gap_ratio_value = coverage_meta_state.get("gap_ratio")
            try:
                ratio_float = float(gap_ratio_value)
            except (TypeError, ValueError):
                ratio_float = None
    ...
    def _primary_probe_allowed() -> bool:
        allow_probe_when_disabled = bool(
            get_env("AI_TRADING_BACKUP_PROBE_ALLOW_WHEN_DISABLED", True, cast=bool)
        )
        return bool(
            ...
            and (allow_probe_when_disabled or (not _disable_signal_active(primary_label)))
        )

    def _mark_primary_probe_due(*, route: str) -> None:
        _state["backup_probe_due"] = True
        _state["fallback_reason"] = "primary_probe_recheck"
        _record_primary_probe_seen("1Min", symbol=symbol, route=route)
        logger.info(
            "ALPACA_PRIMARY_RECOVERY_PROBE",
            extra={"symbol": symbol, "timeframe": "1Min", "route": route},
        )

    if (
        not force_primary_fetch
        and skip_primary_due_to_fallback
        and _primary_probe_allowed()
        and _backup_primary_probe_due(symbol, "1Min")
    ):
        skip_primary_due_to_fallback = False
        force_primary_fetch = True
        _mark_primary_probe_due(route="backup_skip_window")
    if backup_label:
        prefer_primary_first = bool(get_env("PYTEST_RUNNING")) or not _disable_signal_active(primary_label)
        if _state.get("backup_probe_due"):
            # A scheduled probe must force a primary attempt before any
            # provider-monitor backup routing is applied for this request.
            active_provider = primary_label
        elif skip_primary_due_to_fallback:
        ...
        if (
            active_provider == backup_label
            and (not _state.get("backup_probe_due"))
            and _primary_probe_allowed()
            and _backup_primary_probe_due(symbol, "1Min")
        ):
            active_provider = primary_label
    ...
    backup_probe_due = bool(_state.get("backup_probe_due"))

    def _fetch_primary_with_probe(
        symbol_arg: str,
        start_arg: Any,
        end_arg: Any,
        timeframe_arg: str,
        *,
        ...
        if backup_probe_due:
            try:
                ...
                # Test stubs frequently monkeypatch ``_fetch_bars`` without new kwargs.
                if "bypass_backup_skip" not in str(exc):
                    raise
        return _fetch_bars(symbol_arg, start_arg, end_arg, timeframe_arg, feed=feed_arg)
    ...
    if _has_alpaca_keys() and force_primary_fetch:
        try:
            ...
            df = _fetch_primary_with_probe(
                symbol,
                start_dt,
                end_dt,
                "1Min",
                feed_arg=feed_to_use,
            )
            ...
            if isinstance(e, EmptyBarsError):
                ...
                if end_dt > now or start_dt > now:
                    ...
                    if (
                        ...
                        if not switch_recorded:
                            ...
                            switch_recorded = True
                        try:
                            df_alt = _fetch_primary_with_probe(
                                symbol,
                                start_dt,
                                end_dt,
                                "1Min",
                ...
                if (not cap_exhausted) and cnt >= _EMPTY_BAR_THRESHOLD:
                    ...
                    sip_locked_backoff = _is_sip_unauthorized()
                    sip_ready = _sip_configured()
                    if not sip_ready and _pytest_active() and not sip_locked_backoff:
                        sip_ready = True
                    force_sip_allowed = (
                        current_feed == "iex"
                        and sip_ready
                    ...
                    if force_sip_allowed and not sip_force_consumed:
                        ...
                        df_sip_forced = None
                        try:
                            df_sip_forced = _fetch_primary_with_probe(
                                symbol,
                                start_dt,
                                end_dt,
                                "1Min",
                        ...
                        if df_sip_forced is not None and getattr(df_sip_forced, "empty", True):
                            last_empty_error = None
                    ...
                    if end_dt - start_dt > _dt.timedelta(days=1):
                        ...
                        try:
                            df_short = _fetch_primary_with_probe(
                                symbol,
                                short_start,
                                end_dt,
                                "1Min",
                                feed_arg=normalized_feed or _DEFAULT_FEED,
                            )
    ...
    try:
        ...
        if used_backup:
            ...
            err.fetch_reason = getattr(exc, "fetch_reason", "ohlcv_columns_missing")
            raise err from exc
        raise
    if df is None:
        if allow_empty_return:
...
def get_bars(
    ...
    *,
    ...
    y_int = interval_map.get(tf_norm)
    if y_int is None:
...
def _record_gap_ratio_state(ratio: float | None, *, metadata: Mapping[str, Any] | None = None) -> None:
    ...
    if ratio is None:
        ...
        try:
            state["gap_ratio"] = float(ratio)
        except (TypeError, ValueError):
            state["gap_ratio"] = None
...
def _current_gap_ratio(default: float | None = None) -> float | None:
    ...
    if ratio is None:
        coverage = state.get("coverage_meta")
        if isinstance(coverage, Mapping):
            ratio = coverage.get("gap_ratio")
    if ratio is None:
        return default
    try:
        return float(ratio)
...
def _build_fetch_log_extra(
    ...
    *,
    ...
    if attempt is not None:
        payload["attempt"] = max(int(attempt), 1)
    gap_ratio = _current_gap_ratio()
    if gap_ratio is not None:
    ...
    if exception is not None:
        payload["exc_type"] = exception.__class__.__name__
        message = _safe_exception_message(exception)
        if message:
            payload["message"] = message
        if isinstance(exception, APIError):
            status_attr = getattr(exception, "status_code", None)
            try:
                if status_attr is not None:
                    payload.setdefault("http_status", int(status_attr))
            except (TypeError, ValueError):
                pass
            code_value = getattr(exception, "code", None) or getattr(exception, "error_code", None)
            if code_value is not None:
                payload["error_code"] = str(code_value)
    message_value = payload.get("message")
    if isinstance(message_value, str) and len(message_value) > 200:
        payload["message"] = message_value[:200]
    return payload
