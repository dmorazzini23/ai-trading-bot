...
class MarketDataValidator:
    """Minimal market data validator used in tests."""  # AI-AGENT-REF
...
def monitor_real_time_data_quality(prices: dict[str, float]) -> dict[str, Any]:
    """Check price mapping for non-positive values."""  # AI-AGENT-REF
    bad = [sym for sym, price in prices.items() if price <= 0]
    return {
        "data_quality_ok": not bad,
...
def validate_trade_log_integrity(path: str | Path) -> dict[str, Any]:
    """Validate CSV trade log and return integrity report."""  # AI-AGENT-REF
    p = Path(path)
    report: dict[str, Any] = {
    ...
    report["integrity_score"] = (
        1.0 - len(corrupted) / len(df) if len(df) else 0.0
    )
    return report
