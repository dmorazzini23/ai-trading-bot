...
class MarketDataValidator:
            ...
            issues.append(
                {
                    ...
                    "severity": (
                        "error" if total_violations > len(df) * 0.05 else "warning"
                    ),
    ...
    def _detect_price_spikes(
        ...
        if total_spikes > 0:
            max_spike = (
                max(close_pct_change.max(), high_pct_change.max(), low_pct_change.max())
                * 100
            )
        ...
        score = max(
            0.0, 1.0 - spike_frequency * 20
        )  # Heavy penalty for frequent spikes
    ...
    def _validate_volume_patterns(self, df: pd.DataFrame) -> tuple[float, list[dict]]:
        ...
        volume_quality = (
            1.0 - zero_volume_pct - (negative_volume / len(df) if len(df) > 0 else 0)
        )
    ...
    def _check_completeness(self, df: pd.DataFrame) -> tuple[float, list[dict]]:
        ...
        if missing_data:
            total_missing_pct = sum(
                data["percentage"] for data in missing_data.values()
            ) / len(missing_data)
...
_market_data_validator: MarketDataValidator | None = None
