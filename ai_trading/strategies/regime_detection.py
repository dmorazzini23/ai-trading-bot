...
class RegimeDetector:
    ...
    def detect_regime(self, market_data: 'pd.DataFrame', supplementary_data: dict=None) -> dict[str, Any]:
        ...
        try:
            ...
            primary_regime = self._determine_primary_regime(trend_analysis, volatility_analysis, momentum_analysis)
            secondary_characteristics = self._detect_secondary_characteristics(market_data, volume_analysis, sentiment_analysis)
            confidence_score = self._calculate_regime_confidence(trend_analysis, volatility_analysis, momentum_analysis)
            transition_analysis = self._analyze_regime_transitions(market_data)
            regime_result = {'timestamp': datetime.now(UTC), 'primary_regime': primary_regime, 'secondary_characteristics': secondary_characteristics, 'confidence_score': confidence_score, 'trend_analysis': trend_analysis, 'volatility_analysis': volatility_analysis, 'momentum_analysis': momentum_analysis, 'volume_analysis': volume_analysis, 'sentiment_analysis': sentiment_analysis, 'transition_analysis': transition_analysis, 'analysis_time_seconds': (datetime.now(UTC) - analysis_start).total_seconds()}
    ...
    def _analyze_trend(self, data: 'pd.DataFrame') -> dict[str, Any]:
        ...
        try:
            ...
            returns_12m = data['close'].iloc[-1] / data['close'].iloc[-252] - 1 if len(data) >= 252 else 0
            data['SMA_50'] = data['close'].rolling(50).mean()
            data['SMA_200'] = data['close'].rolling(200).mean()
            current_price = data['close'].iloc[-1]
            sma_50 = data['SMA_50'].iloc[-1] if not pd.isna(data['SMA_50'].iloc[-1]) else current_price
            sma_200 = data['SMA_200'].iloc[-1] if not pd.isna(data['SMA_200'].iloc[-1]) else current_price
            ...
            trend_strength = self._calculate_trend_strength(returns_1m, returns_3m, returns_6m, price_vs_sma50, price_vs_sma200)
            ...
            return {'direction': trend_direction, 'strength': trend_strength, 'returns_1m': returns_1m, 'returns_3m': returns_3m, 'returns_6m': returns_6m, 'returns_12m': returns_12m, 'price_vs_sma50': price_vs_sma50, 'price_vs_sma200': price_vs_sma200, 'sma_trend': sma_trend, 'above_sma200': current_price > sma_200}
        except COMMON_EXC as e:
    ...
    def _analyze_volatility(self, data: 'pd.DataFrame') -> dict[str, Any]:
        ...
        try:
            ...
            vol_10d = data['returns'].tail(10).std() * np.sqrt(252) if len(data) >= 10 else 0
            vol_30d = data['returns'].tail(30).std() * np.sqrt(252) if len(data) >= 30 else 0
            vol_60d = data['returns'].tail(60).std() * np.sqrt(252) if len(data) >= 60 else 0
            vol_252d = data['returns'].tail(252).std() * np.sqrt(252) if len(data) >= 252 else 0
            data['high_low'] = data['high'] - data['low']
            data['high_close'] = abs(data['high'] - data['close'].shift(1))
            data['low_close'] = abs(data['low'] - data['close'].shift(1))
            data['true_range'] = data[['high_low', 'high_close', 'low_close']].max(axis=1)
            atr = data['true_range'].rolling(14).mean().iloc[-1] if len(data) >= 14 else 0
            current_vol = vol_30d
            ...
            vol_clustering = returns_squared.tail(20).autocorr(lag=1) if len(data) >= 21 else 0
            return {'regime': vol_regime, 'trend': vol_trend, 'current_vol': current_vol, 'vol_10d': vol_10d, 'vol_30d': vol_30d, 'vol_60d': vol_60d, 'vol_252d': vol_252d, 'atr': atr, 'volatility_clustering': vol_clustering, 'vol_percentile': self._calculate_volatility_percentile(current_vol, vol_252d)}
        except COMMON_EXC as e:
    ...
    def _analyze_momentum(self, data: 'pd.DataFrame') -> dict[str, Any]:
        ...
        try:
            ...
            current_histogram = macd_histogram.iloc[-1] if not pd.isna(macd_histogram.iloc[-1]) else 0
            ...
            roc_10 = data['close'].iloc[-1] / data['close'].iloc[-11] - 1 if len(data) >= 11 else 0
            roc_20 = data['close'].iloc[-1] / data['close'].iloc[-21] - 1 if len(data) >= 21 else 0
            return {'state': momentum_state, 'rsi': current_rsi, 'macd': current_macd, 'macd_signal': current_signal, 'macd_histogram': current_histogram, 'roc_10d': roc_10, 'roc_20d': roc_20, 'momentum_strength': self._calculate_momentum_strength(current_rsi, current_macd, roc_20)}
        except COMMON_EXC as e:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                if obv_current > obv_20_ago * 1.05:
                    ...
                    obv_trend = 'bearish'
            return {'trend': volume_trend, 'current_volume': current_volume, 'volume_ratio_20': volume_ratio_20, 'volume_ratio_50': volume_ratio_50, 'obv_trend': obv_trend, 'volume_strength': 'high' if volume_ratio_20 > 1.5 else 'normal' if volume_ratio_20 > 0.8 else 'low'}
        except COMMON_EXC as e:
    ...
    def _determine_primary_regime(self, trend_analysis: dict, volatility_analysis: dict, momentum_analysis: dict) -> MarketRegime:
        ...
        try:
            ...
            if trend_direction == 'bullish' and returns_6m > self.trend_thresholds['bull_threshold'] and (momentum_state in ['bullish', 'overbought_bullish']):
                return MarketRegime.BULL_MARKET
            if trend_direction == 'bearish' and returns_6m < self.trend_thresholds['bear_threshold'] and (momentum_state in ['bearish', 'oversold_bearish']):
                return MarketRegime.BEAR_MARKET
            if trend_direction == 'bullish' and momentum_state == 'bullish' and (returns_6m > -0.05) and (returns_6m < 0.1):
                return MarketRegime.RECOVERY
            if trend_direction == 'sideways' and abs(returns_6m) < self.trend_thresholds['sideways_threshold']:
    ...
    def _detect_secondary_characteristics(self, data: 'pd.DataFrame', volume_analysis: dict, sentiment_analysis: dict) -> list[str]:
        ...
        try:
            ...
            price_trend = 'up' if data['close'].iloc[-1] > data['close'].iloc[-21] else 'down'
    ...
    def _calculate_regime_confidence(self, trend_analysis: dict, volatility_analysis: dict, momentum_analysis: dict) -> float:
    ...
    def _analyze_regime_transitions(self, data: 'pd.DataFrame') -> dict[str, Any]:
        ...
        try:
            ...
            recent_regimes = [entry['primary_regime'] for entry in self.regime_history[-10:]]
            ...
            next_likely_regime = current_regime
            if current_regime == MarketRegime.BULL_MARKET:
                ...
                if len(data) >= 20:
                    recent_returns = data['close'].iloc[-1] / data['close'].iloc[-21] - 1
            ...
            return {'transition_probability': transition_prob, 'next_likely_regime': next_likely_regime, 'regime_stability': 1.0 - transition_prob, 'recent_regime_changes': regime_changes}
        except COMMON_EXC as e:
            logger.error(f'Error analyzing regime transitions: {e}')
            return {'transition_probability': 0.1}

    def _calculate_trend_strength(self, returns_1m: float, returns_3m: float, returns_6m: float, price_vs_sma50: float, price_vs_sma200: float) -> TrendStrength:
    ...
    def _calculate_volatility_percentile(self, current_vol: float, historical_vol: float) -> float:
    ...
    def get_regime_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'current_regime': latest['primary_regime'].value if hasattr(latest['primary_regime'], 'value') else str(latest['primary_regime']), 'confidence': latest['confidence_score'], 'stability': stability, 'secondary_characteristics': latest.get('secondary_characteristics', []), 'volatility_regime': latest['volatility_analysis']['regime'].value if hasattr(latest['volatility_analysis']['regime'], 'value') else str(latest['volatility_analysis']['regime']), 'trend_direction': latest['trend_analysis']['direction'], 'momentum_state': latest['momentum_analysis']['state'], 'last_updated': latest['timestamp'], 'regime_history_length': len(self.regime_history)}
        except COMMON_EXC as e:
    ...
    def get_regime_recommendations(self) -> dict[str, Any]:
        ...
        try:
            ...
            recommendations = {'strategy_type': 'conservative', 'position_size_multiplier': 1.0, 'risk_level': 'medium', 'recommended_timeframes': ['1h', '4h', '1d'], 'avoid_strategies': [], 'preferred_strategies': []}
            if self.current_regime == MarketRegime.BULL_MARKET:
                recommendations.update({'strategy_type': 'aggressive_long', 'position_size_multiplier': 1.2, 'risk_level': 'medium_low', 'preferred_strategies': ['momentum', 'breakout', 'trend_following'], 'avoid_strategies': ['short_selling', 'contrarian']})
            elif self.current_regime == MarketRegime.BEAR_MARKET:
                recommendations.update({'strategy_type': 'defensive', 'position_size_multiplier': 0.7, 'risk_level': 'high', 'preferred_strategies': ['short_selling', 'mean_reversion', 'defensive'], 'avoid_strategies': ['momentum_long', 'growth']})
            elif self.current_regime == MarketRegime.CRISIS:
                recommendations.update({'strategy_type': 'crisis_mode', 'position_size_multiplier': 0.3, 'risk_level': 'very_high', 'preferred_strategies': ['cash', 'defensive', 'volatility'], 'avoid_strategies': ['momentum', 'leverage', 'growth']})
            elif self.current_regime == MarketRegime.HIGH_VOLATILITY:
                recommendations.update({'strategy_type': 'volatility_adjusted', 'position_size_multiplier': 0.6, 'risk_level': 'high', 'preferred_strategies': ['mean_reversion', 'volatility_trading'], 'avoid_strategies': ['trend_following', 'momentum']})
