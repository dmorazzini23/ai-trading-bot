...
class SignalAggregator:
    ...
    def aggregate_signals(
        ...
        try:
            ...
            if method == "stacking" and self.enable_stacking:
                aggregated_signal = self._stacking_aggregation(
                    resolved_signals, market_data, timestamp
                )
                ...
                logger.warning(
                    f"Unknown aggregation method: {method}, using weighted average"
                )
                aggregated_signal = self._weighted_average_aggregation(resolved_signals)

            # Apply turnover penalty
            if aggregated_signal:
                aggregated_signal = self._apply_turnover_penalty(
                    aggregated_signal, timestamp
                )
    ...
    def _weighted_average_aggregation(
        ...
        for (symbol, side), group_signals in signal_groups.items():
            ...
            weighted_strength = (
                sum(s.strength * s.confidence for s in group_signals) / total_weight
            )
    ...
    def _stacking_aggregation(
        ...
        try:
            if not sklearn_available:
                logger.warning(
                    "sklearn not available, falling back to weighted average"
                )
            ...
            if signals:
                ...
                for signal in signals:
                    ...
                    recent_performance = self.signal_metrics.get(signal_id, {}).get(
                ...
                stacked_signal = StrategySignal(
                    ...
                    side=(
                        base_signal.side if combined_strength > 0 else base_signal.side
                    ),
    ...
    def _apply_signal_decay(
        self, signals: list[StrategySignal], timestamp: datetime | None = None
    ) -> list[StrategySignal]:
                ...
                if signal_time_str:
                    try:
                        signal_time = datetime.fromisoformat(
                            signal_time_str.replace("Z", "+00:00")
                        )
        ...
        try:
            ...
            for symbol, sides in signal_groups.items():
                ...
                if self.conflict_resolution == "majority":
                    ...
                    if buy_signals:
                        strongest_buy = max(
                            buy_signals, key=lambda s: s.weighted_strength
                        )
                        resolved_signals.append(strongest_buy)
                    if sell_signals:
                        strongest_sell = max(
                            sell_signals, key=lambda s: s.weighted_strength
                        )
    ...
    def _apply_turnover_penalty(
        self, signal: StrategySignal, timestamp: datetime | None = None
    ) -> StrategySignal:
        ...
        try:
            ...
            recent_signals = [
                entry
                for entry in self.ensemble_history[-10:]  # Last 10 signals
            ...
            if len(recent_signals) > 3:  # If trading too frequently
                penalty_factor = 1.0 - (
                    self.turnover_penalty * (len(recent_signals) - 3)
                )
    ...
    def _prepare_meta_features(
        self, signals: list[StrategySignal], market_data: dict | None = None
    ) -> list[float] | None:
    ...
    def _train_meta_model(self) -> None:
        ...
        try:
            if (
                ...
                or len(self.signal_performance_history) < self.min_performance_window
            ):
                return
    ...
    def update_signal_performance(
        self, signal_id: str, actual_return: float, prediction_horizon: int = 1
    ) -> None:
        """
        ...
        try:
            ...
            if len(self.signal_metrics[signal_id]["returns"]) > 100:
                self.signal_metrics[signal_id]["returns"] = self.signal_metrics[
                    signal_id
                ]["returns"][-50:]
            ...
            if len(returns) > 5:
                ...
                recent_perf = max(
                    0.0, min(1.0, (recent_perf + 0.02) / 0.04)
                )  # Assume ±2% range
    ...
    def _add_performance_observation(
        self, signal_id: str, actual_return: float
    ) -> None:
            ...
            for entry in reversed(self.ensemble_history[-50:]):  # Search recent history
                if entry.get("signal_id") == signal_id:
                    ...
                    if len(self.signal_performance_history) > 500:
                        self.signal_performance_history = (
                            self.signal_performance_history[-250:]
                        )
...
class SignalProcessor:
    ...
    def process_signals(self, signals: list[StrategySignal]) -> list[StrategySignal]:
        ...
        try:
            ...
            logger.debug(
                f"Processed {len(processed_signals)} signals from {len(signals)} input signals"
            )
