...
class SignalAggregator:
    ...
    def __init__(self, enable_stacking: bool=True, decay_window: int=30, min_performance_window: int=10, turnover_penalty: float=0.1, conflict_resolution: str='majority'):
        """
    ...
    def _weighted_average_aggregation(self, signals: list[StrategySignal]) -> StrategySignal:
        ...
        for (symbol, side), group_signals in signal_groups.items():
            ...
            aggregated_signal = StrategySignal(symbol=symbol, side=side, strength=weighted_strength, confidence=avg_confidence, signal_type='aggregated_weighted', metadata={'source_signals': len(group_signals), 'aggregation_method': 'weighted_average'})
    ...
    def _consensus_aggregation(self, signals: list[StrategySignal]) -> StrategySignal | None:
        ...
        consensus_threshold = len(signals) * 0.6
        for (symbol, side), group_signals in signal_counts.items():
            if len(group_signals) >= consensus_threshold:
                ...
                avg_confidence = statistics.mean([s.confidence for s in group_signals])
                return StrategySignal(symbol=symbol, side=side, strength=avg_strength, confidence=avg_confidence, signal_type='consensus', metadata={'consensus_count': len(group_signals), 'total_signals': len(signals)})
        return None
    ...
    def _stacking_aggregation(self, signals: list[StrategySignal], market_data: dict | None=None, timestamp: datetime | None=None) -> StrategySignal | None:
        """
        ...
        try:
            ...
            if signals:
                ...
                stacked_signal = StrategySignal(symbol=base_signal.symbol, side=base_signal.side if combined_strength > 0 else base_signal.side, strength=abs(combined_strength), confidence=combined_confidence, signal_type='stacked_meta', metadata={'source_signals': len(signals), 'meta_prediction': predicted_weight, 'aggregation_method': 'stacking', 'timestamp': timestamp.isoformat() if timestamp else datetime.now(UTC).isoformat()})
    ...
    def _apply_signal_decay(self, signals: list[StrategySignal], timestamp: datetime | None=None) -> list[StrategySignal]:
            ...
            for signal in signals:
                ...
                if signal_time_str:
                    try:
                        ...
                        decayed_signal = StrategySignal(symbol=signal.symbol, side=signal.side, strength=signal.strength * decay_factor, confidence=signal.confidence * decay_factor, signal_type=signal.signal_type, metadata={**signal.metadata, 'decay_factor': decay_factor})
    ...
    def _apply_turnover_penalty(self, signal: StrategySignal, timestamp: datetime | None=None) -> StrategySignal:
        ...
        try:
            ...
            if len(recent_signals) > 3:
                ...
                penalized_signal = StrategySignal(symbol=signal.symbol, side=signal.side, strength=signal.strength * penalty_factor, confidence=signal.confidence, signal_type=signal.signal_type, metadata={**signal.metadata, 'turnover_penalty': penalty_factor, 'recent_signals_count': len(recent_signals)})
    ...
    def _prepare_training_data(self) -> tuple[list[list[float]], list[float]]:
        ...
        try:
            X, y = ([], [])
            for entry in self.signal_performance_history[-100:]:
    ...
    def update_signal_performance(self, signal_id: str, actual_return: float, prediction_horizon: int=1) -> None:
        """
