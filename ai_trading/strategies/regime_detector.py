...
class RegimeMetrics:
    ...
    vix_level: Optional[float]  # VIX level if available
...
class RegimeDetector:
    ...
    def __init__(self, 
                 ...
                 volatility_window: int = 21):  # 1 month volatility window
        """
        ...
        logger.info(f"RegimeDetector initialized with lookback={lookback_periods}, "
                   f"trend_threshold={trend_threshold:.1%}")

    def detect_current_regime(self, 
                            market_data: Dict[str, Any],
                            index_symbol: str = "SPY") -> Tuple[MarketRegime, RegimeMetrics]:
        """
    ...
    def calculate_dynamic_thresholds(self, 
                                   regime: MarketRegime,
                                   metrics: RegimeMetrics,
                                   base_thresholds: Optional[Dict[str, float]] = None) -> TradingThresholds:
        """
        ...
        try:
            ...
            adjusted_thresholds = TradingThresholds(
                rebalance_drift_threshold=base_thresholds['rebalance_drift_threshold'] * adjustments['drift_multiplier'],
                trade_frequency_multiplier=base_thresholds['trade_frequency_multiplier'] * adjustments['frequency_multiplier'],
                correlation_penalty_adjustment=base_thresholds['correlation_penalty_adjustment'] * adjustments['correlation_multiplier'],
                minimum_improvement_threshold=base_thresholds['minimum_improvement_threshold'] * adjustments['improvement_multiplier'],
                safety_margin_multiplier=base_thresholds['safety_margin_multiplier'] * adjustments['safety_multiplier']
            )
            
            logger.info(f"Dynamic thresholds for {regime.value}: "
        ...
        try:
            ...
            for i in range(self.volatility_window, len(returns) + 1):
                ...
                if len(window_returns) > 1:
                    vol = statistics.stdev(window_returns) * math.sqrt(252)  # Annualized
    ...
    def _calculate_momentum(self, returns: List[float]) -> float:
        ...
        try:
            ...
            momentum = max(-1.0, min(1.0, momentum / 0.1))  # Assume 10% is extreme momentum
    ...
    def _get_vix_level(self, market_data: Dict[str, Any]) -> Optional[float]:
        """Get VIX level if available."""
        try:
            prices = market_data.get('prices', {})
    ...
    def _classify_market_regime(self, metrics: RegimeMetrics) -> MarketRegime:
        ...
        try:
            ...
            if (metrics.volatility_regime in [VolatilityRegime.EXTREMELY_HIGH, VolatilityRegime.HIGH] and
            ...
            if (metrics.trend_strength < 0.3 and 
                metrics.volatility_regime == VolatilityRegime.NORMAL):
