...
The functions in this module provide minimal trading-session utilities
without requiring :mod:`pandas_market_calendars`.  When the optional
package is installed we defer to it for accurate open/close times.  When
it is unavailable a small table of known sessions acts as a fallback.

The fallback data intentionally covers:
...
def rth_session_utc(d: date) -> tuple[datetime, datetime]:
    """Return the Regular Trading Hours session window in UTC."""
    cal = _get_calendar()
    if cal is not None:
