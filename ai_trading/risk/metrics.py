...
class DrawdownAnalyzer:
    ...
    def calculate_drawdowns(self, values: list[float]) -> dict:
        ...
        try:
            ...
            for i, value in enumerate(values):
                if value > peak:
                    if current_dd > 0:
                        drawdown_periods.append({'start_idx': current_dd_start, 'end_idx': i - 1, 'duration': i - current_dd_start, 'magnitude': current_dd})
                    peak = value
                    current_dd_start = i
                    current_dd = 0
                else:
                    ...
                    if current_dd > max_dd:
                        ...
                        dd_end_idx = i
            if current_dd > 0:
                drawdown_periods.append({'start_idx': current_dd_start, 'end_idx': len(values) - 1, 'duration': len(values) - current_dd_start, 'magnitude': current_dd})
            stats = {'max_drawdown': max_dd, 'max_drawdown_start': dd_start_idx, 'max_drawdown_end': dd_end_idx, 'max_drawdown_duration': dd_end_idx - dd_start_idx, 'drawdown_periods': drawdown_periods, 'num_drawdown_periods': len(drawdown_periods), 'average_drawdown': statistics.mean([dd['magnitude'] for dd in drawdown_periods]) if drawdown_periods else 0, 'average_recovery_time': statistics.mean([dd['duration'] for dd in drawdown_periods]) if drawdown_periods else 0}
            return stats
        except (ValueError, TypeError, ZeroDivisionError, OverflowError, statistics.StatisticsError) as e:
            logger.error(f'Error calculating drawdowns: {e}')
            return {}

    def is_in_drawdown(self, current_value: float, peak_value: float) -> tuple[bool, float]:
        """
    ...
    def calculate_recovery_time(self, values: list[float], drawdown_start: int, drawdown_end: int) -> int | None:
        """
