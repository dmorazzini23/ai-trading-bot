...
class ATRPositionSizer:
    ...
    def __init__(self, risk_per_trade: float=None):
        ...
        logger.info(f'ATRPositionSizer initialized with risk_per_trade={self.risk_per_trade}, atr_multiplier={self.atr_multiplier}')

    def calculate_position_size(self, account_equity: float, entry_price: float, atr_value: float, stop_distance_multiplier: float=None) -> int:
        """
    ...
    def calculate_stop_levels(self, entry_price: float, atr_value: float, side: str='long') -> dict[str, float]:
        """
        ...
        try:
            ...
            if side.lower() == 'long':
                ...
                stop_loss = entry_price + stop_distance
                take_profit = entry_price - profit_distance
            return {'stop_loss': max(0.01, stop_loss), 'take_profit': max(0.01, take_profit)}
        except COMMON_EXC as e:
...
class VolatilityPositionSizer:
    ...
    def __init__(self, target_volatility: float=0.15, lookback_days: int=20):
        ...
        logger.info(f'VolatilityPositionSizer initialized with target_volatility={target_volatility}, lookback_days={lookback_days}')
    ...
    def calculate_volatility_multiplier(self, returns: list[float]) -> float:
        ...
        try:
            ...
            realized_volatility = returns_std * math.sqrt(252)
            ...
            logger.debug(f'Volatility calculation: realized={realized_volatility:.3f}, target={self.target_volatility:.3f}, multiplier={volatility_multiplier:.3f}')
...
class DynamicPositionSizer:
    ...
    def calculate_optimal_position(self, symbol: str, account_equity: float, entry_price: float, market_data: dict, historical_data: dict) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if result['recommended_size'] > 0:
                ...
                result['risk_metrics']['estimated_risk'] = self._estimate_position_risk(notional_value, atr_value)
            if result['recommended_size'] * entry_price > account_equity * 0.05:
                result['warnings'].append('Large position size relative to account')
            logger.info(f"Position sizing for {symbol}: recommended={result['recommended_size']}, methods={result['sizing_methods']}")
            return result
        except COMMON_EXC as e:
    ...
    def calculate_scaling_orders(self, base_position: int, entry_price: float, scaling_levels: int=3) -> list[dict]:
        ...
        try:
            ...
            for level in range(scaling_levels):
                ...
                if order_size > 0:
                    price_adjustment = entry_price * 0.001 * level
                    scaled_orders.append({'level': level + 1, 'size': order_size, 'price': entry_price - price_adjustment, 'percentage': order_size / base_position * 100})
...
class PortfolioPositionManager:
    ...
    def assess_new_position(self, symbol: str, proposed_size: int, entry_price: float, account_equity: float) -> dict[str, Any]:
        """
