...
class ATRPositionSizer:
    ...
    def __init__(self, risk_per_trade: float = None):
        ...
        logger.info(
            f"ATRPositionSizer initialized with risk_per_trade={self.risk_per_trade}, "
    ...
    def calculate_position_size(
        ...
        entry_price: float,
        atr_value: float,
        stop_distance_multiplier: float = None,
    ) -> int:
        """
        ...
        try:
            if account_equity <= 0 or entry_price <= 0 or atr_value <= 0:
                logger.warning(
                    f"Invalid inputs: equity={account_equity}, price={entry_price}, atr={atr_value}"
                )
                return 0
            ...
            logger.debug(
                f"ATR position sizing: equity=${account_equity:,.2f}, "
...
class VolatilityPositionSizer:
    ...
    def __init__(self, target_volatility: float = 0.15, lookback_days: int = 20):
        ...
        logger.info(
            f"VolatilityPositionSizer initialized with target_volatility={target_volatility}, "
    ...
    def calculate_volatility_multiplier(self, returns: list[float]) -> float:
        ...
        try:
            if len(returns) < 10:
                logger.warning(
                    f"Insufficient data for volatility calculation: {len(returns)} returns"
                )
                return 1.0
            ...
            logger.debug(
                f"Volatility calculation: realized={realized_volatility:.3f}, "
    ...
    def calculate_position_size(
        self, base_position_size: int, returns: list[float]
    ) -> int:
        """
        ...
        try:
            ...
            logger.debug(
                f"Volatility position sizing: base={base_position_size}, "
...
class DynamicPositionSizer:
        ...
        try:
            ...
            if atr_value <= 0:
                result["warnings"].append(
                    "Invalid ATR value, using conservative sizing"
                )
            ...
            if returns:
                vol_adjusted_size = self.volatility_sizer.calculate_position_size(
                    atr_size, returns
                )
            ...
            if (
                trade_history
                and len(trade_history) >= self.kelly_criterion.min_sample_size
            ):
                kelly_fraction, kelly_stats = (
                    self.kelly_criterion.calculate_from_returns(
            ...
            logger.info(
                f"Position sizing for {symbol}: recommended={result['recommended_size']}, "
    ...
    def calculate_scaling_orders(
        self, base_position: int, entry_price: float, scaling_levels: int = 3
    ) -> list[dict]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Scaled orders for position {base_position}: {len(scaled_orders)} levels"
            )
    ...
    def _estimate_position_risk(self, notional_value: float, atr_value: float) -> float:
        ...
        try:
            ...
            risk_estimate = (atr_value * self.atr_sizer.atr_multiplier) / (
                notional_value / 100
            )
...
class PortfolioPositionManager:
    ...
    def __init__(self, max_portfolio_risk: float = None):
        ...
        self.max_portfolio_risk = (
            max_portfolio_risk or RISK_PARAMETERS["MAX_PORTFOLIO_RISK"]
        )
        ...
        logger.info(
            f"PortfolioPositionManager initialized with max_portfolio_risk={self.max_portfolio_risk}"
        )

    def assess_new_position(
        self, symbol: str, proposed_size: int, entry_price: float, account_equity: float
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if position_pct > self.dynamic_sizer.max_position_pct:
                ...
                assessment["warnings"].append(
                    f"Position size adjusted from {proposed_size} to {adjusted_size}"
                )
            else:
            ...
            if new_total_risk > self.max_portfolio_risk:
                ...
                assessment["warnings"].append(
                    f"Portfolio risk limit requires size reduction to {final_size}"
                )

            # Final approval check
            assessment["approved"] = (
                assessment["adjusted_size"] > 0 and len(assessment["warnings"]) == 0
            )

            if assessment["adjusted_size"] != proposed_size:
                assessment["recommendations"].append(
                    "Consider splitting order across multiple sessions"
                )
