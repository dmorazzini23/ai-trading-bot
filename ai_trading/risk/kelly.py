...
class KellyCriterion:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_fraction = (
            max_fraction if max_fraction is not None else self.config.kelly_fraction_max
        )
        ...
        logger.info(
            f"KellyCriterion initialized with min_sample_size={self.min_sample_size}, "
    ...
    def calculate_kelly_fraction(
        ...
        try:
            if win_rate <= 0 or win_rate >= 1:
                logger.warning(
                    f"Invalid win_rate: {win_rate}. Must be between 0 and 1."
                )
                return 0.0

            if avg_win <= 0 or avg_loss <= 0:
                logger.warning(
                    f"Invalid win/loss values: avg_win={avg_win}, avg_loss={avg_loss}"
                )
                return 0.0
            ...
            logger.debug(
                f"Kelly calculation: win_rate={win_rate:.3f}, avg_win={avg_win:.3f}, "
    ...
    def calculate_from_returns(self, returns: list[float]) -> tuple[float, dict]:
        ...
        try:
            if len(returns) < self.min_sample_size:
                logger.warning(
                    f"Insufficient sample size: {len(returns)} < {self.min_sample_size}"
                )
            ...
            stats = {
                ...
                "avg_loss": avg_loss,
                "profit_factor": (
                    (sum(wins) / sum(losses)) if sum(losses) > 0 else float("inf")
                ),
    ...
    def kelly_with_confidence(
        self, returns: list[float], confidence: float = None
    ) -> tuple[float, float]:
        """
...
class KellyCalculator:
    ...
    def calculate_portfolio_kelly(
        ...
        try:
            ...
            for symbol, returns in asset_returns.items():
                kelly_fraction, stats = self.kelly_criterion.calculate_from_returns(
                    returns
                )
                ...
                logger.debug(
                    f"Kelly for {symbol}: {kelly_fraction:.3f} "
            ...
            if total_kelly > self.kelly_criterion.max_fraction:
                ...
                logger.info(
                    f"Normalized portfolio Kelly fractions by factor {normalization_factor:.3f}"
                )
    ...
    def dynamic_kelly_adjustment(
        self, base_kelly: float, market_conditions: dict
    ) -> float:
        """
        ...
        try:
            ...
            # Ensure bounds
            adjusted_kelly = max(
                0.0, min(adjusted_kelly, self.kelly_criterion.max_fraction)
            )
    ...
    def kelly_with_correlation(
        ...
        try:
            ...
            if not correlation_matrix:
                logger.debug(
                    "No correlation matrix provided, using individual Kelly fractions"
                )
