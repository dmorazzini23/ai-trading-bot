...
class MarketHoursValidator:
    ...
    def validate_market_hours(
        ...
        try:
            ...
            if (
                pre_market_start <= current_time < market_open
                or market_close < current_time <= after_hours_end
            ):
                return ValidationResult(
                    category=ValidationCategory.MARKET_HOURS,
                    ...
                    score=0.7,
                    recommendations=[
                        "Consider smaller position sizes during extended hours",
...
class LiquidityValidator:
    ...
    def validate_liquidity(
        self, symbol: str, quantity: int, market_data: dict
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            if participation_rate > self.max_participation:
                return ValidationResult(
                    ...
                    score=0.5,
                    recommendations=[
                        "Split order across multiple sessions",
            ...
            return ValidationResult(
                ...
                recommendations=(
                    [] if liquidity_score > 0.8 else ["Monitor execution quality"]
                ),
            )
    ...
    def _calculate_liquidity_score(
        ...
        try:
            ...
            participation_score = max(
                0.0, 1.0 - (participation_rate / self.max_participation)
            )
            ...
            # Weighted average
            overall_score = (
                participation_score * 0.4 + slippage_score * 0.4 + volume_score * 0.2
            )
...
class RiskValidator:
    ...
    def validate_position_risk(
        ...
        try:
            ...
            details.update(
                {"position_value": position_value, "position_percentage": position_pct}
            )
            ...
            if position_pct > self.max_position_size:
                return ValidationResult(
                    ...
                    score=0.0,
                    recommendations=[
                        f"Reduce quantity to max {int(account_equity * self.max_position_size / price)}"
                    ],
                )
            ...
            if portfolio_utilization > 0.8:
                warnings.append(
                    f"High portfolio utilization: {portfolio_utilization:.1%}"
                )
            ...
            return ValidationResult(
                ...
                status=status,
                message=(
                    "Position risk within acceptable limits"
                    if not warnings
                    else "Position risk warnings"
                ),
                details=details,
                score=risk_score,
    ...
    def validate_portfolio_risk(
        self, proposed_trade: dict, portfolio_data: dict
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            position_count = len(current_positions) + (
                1 if symbol not in current_positions else 0
            )
            ...
            return ValidationResult(
                ...
                recommendations=(
                    [] if portfolio_score > 0.7 else ["Monitor portfolio risk closely"]
                ),
            )
    ...
    def _calculate_risk_score(
        self, position_pct: float, portfolio_utilization: float
    ) -> float:
        ...
        try:
            ...
            utilization_score = max(
                0.0, 1.0 - max(0.0, portfolio_utilization - 0.5) * 2
            )
    ...
    def _calculate_correlation_exposure(
        ...
        try:
            ...
            for other_symbol, position_info in current_positions.items():
                ...
                other_weight = (
                    position_info.get("notional_value", 0) / total_portfolio_value
                )
    ...
    def _assess_concentration_risk(
        self, current_positions: dict, account_equity: float
    ) -> float:
    ...
    def _calculate_portfolio_risk_score(
        ...
        try:
            ...
            correlation_score = max(
                0.0, 1.0 - (correlation_exposure / self.max_correlation_exposure)
            )
...
class PreTradeValidator:
    ...
    def validate_trade(
        self, trade_request: dict, market_data: dict, portfolio_data: dict
    ) -> PreTradeCheckResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Starting pre-trade validation for {symbol}: {quantity} @ {price}"
            )
            ...
            overall_status, overall_score = self._calculate_overall_result(
                validation_results
            )
            ...
            logger.info(
                f"Pre-trade validation complete for {symbol}: "
    ...
    def _calculate_overall_result(
        ...
        try:
            ...
            rejections = [
                r for r in validation_results if r.status == ValidationStatus.REJECTED
            ]
            if rejections:
                return ValidationStatus.REJECTED, 0.0
