...
def validate_trade_data_quality(trade_log_path: str) -> dict:
    ...
    if not isinstance(trade_log_path, str | os.PathLike):
        ...
        quality_report["recommendations"].append(
            "Pass a valid file path string instead of dictionary or other object"
        )
    ...
    try:
        ...
        if not Path(trade_log_path).exists():
            quality_report["issues"].append(
                f"Trade log file does not exist: {trade_log_path}"
            )
                ...
                if "price" in header_line.lower():
                    ...
                    if quality_report["row_count"] > 0:
                        quality_report["data_quality_score"] = (
                            quality_report["valid_price_rows"]
        ...
        try:
            ...
            for line_num, line in enumerate(lines):
                ...
                try:
                    ...
                    elif "price" in first_col.lower() or any(
                        "price" in str(col).lower() for col in row[:3]
                    ):
            ...
            if audit_format_rows > 0 and meta_format_rows > 0:
                ...
                quality_report["issues"].append(
                    "Only audit format detected - conversion needed for meta-learning"
                )
                quality_report["recommendations"].append(
                    "Convert audit format to meta-learning format"
                )
        ...
        if quality_report["row_count"] > 0:
            quality_score = (
                quality_report["valid_price_rows"] / quality_report["row_count"]
            )
            quality_report["data_quality_score"] = quality_score

            if quality_score < 0.5:
                quality_report["issues"].append(
                    f"Low data quality score: {quality_score:.2%}"
                )
                quality_report["recommendations"].append(
...
def adjust_confidence(
    confidence: float, volatility: float, threshold: float = 1.0
) -> float:
                    ...
                    try:
                        ...
                        if not df.empty:
                            ...
                            if isinstance(weights, np.ndarray):
                                logger.info(
                                    "Successfully loaded weights from CSV: %s", path
                                )
                                return weights
                ...
                try:
                    with open(p, "rb") as f:
                        ...
                        if isinstance(weights, np.ndarray):
                            ...
                            logger.warning(
                                "Invalid weights format in %s, using default", path
                            )
                except (pickle.PickleError, EOFError) as pickle_e:
                    logger.warning(
                        "Pickle loading also failed for %s: %s", path, pickle_e
                    )
        else:
...
def retrain_meta_learner(
    ...
    if valid.sum() < int(min_samples):
        logger.info(
            ...
            extra={"valid_rows": int(valid.sum()), "min_samples": int(min_samples)},
        )
        return False
    ...
    logger.debug(
        f"META_LEARNING_RAW_DATA: {original_rows} total rows loaded from {trade_log_path}"
    )
    ...
    if len(df) > 0 and len(df.columns) >= 3:
        ...
        has_meta_headers = any(
            col in ["symbol", "entry_price", "exit_price", "signal_tags"]
            for col in columns
        )
        ...
        if has_meta_headers:
            ...
            if audit_format_detected:
                logger.info(
                    "META_LEARNING_MIXED_FORMAT: Detected meta-learning headers with audit data, attempting conversion"
                )
                try:
                ...
                logger.debug(
                    "META_LEARNING_PURE_FORMAT: Detected pure meta-learning format"
                )
        else:
            # Pure audit format, try conversion
            logger.info(
                "META_LEARNING_AUDIT_CONVERSION: Attempting to convert pure audit format"
            )
            try:
                df = _convert_audit_to_meta_format(df)
                if df.empty:
                    logger.error(
                        "METALEARN_EMPTY_TRADE_LOG - No valid trades found: Failed to convert audit rows"
                    )
                    _implement_fallback_data_recovery(trade_log_path, 10)
                    return False
            except Exception as e:
    ...
    try:
        ...
        initial_rows = len(df)
        logger.debug(
            f"META_LEARNING_INITIAL_DATA: {initial_rows} rows before validation"
        )
        ...
        if after_numeric < initial_rows:
            logger.info(
                f"META_LEARNING_PRICE_CONVERSION: Removed {initial_rows - after_numeric} rows with invalid price formats"
            )
        ...
        if len(df) > 0:
            ...
            if price_issues.any():
                ...
                logger.warning(
                    f"META_LEARNING_PRICE_OUTLIERS: Found {len(problematic_rows)} rows with unrealistic prices"
                )
                logger.debug(
                    f"Price outliers range: entry({problematic_rows['entry_price'].min():.2f}-{problematic_rows['entry_price'].max():.2f}), exit({problematic_rows['exit_price'].min():.2f}-{problematic_rows['exit_price'].max():.2f})"
                )
        ...
        if len(df) > 0:
            ...
            if extreme_moves.any():
                ...
                logger.warning(
                    f"META_LEARNING_EXTREME_MOVES: Found {extreme_count} trades with >1000% price moves"
                )
        ...
        if len(df) > 0:
            ...
            if not positive_prices.all():
                logger.info(
                    f"META_LEARNING_NEGATIVE_PRICES: Filtered out {(~positive_prices).sum()} trades with non-positive prices"
                )
        ...
        logger.info(
            f"META_LEARNING_DATA_QUALITY: Retained {final_rows}/{initial_rows} trades ({retention_rate:.1f}%)"
        )
        ...
        if final_rows > 0:
            ...
            logger.debug(
                f"META_LEARNING_PRICE_STATS: Entry prices ${entry_stats['min']:.2f}-${entry_stats['max']:.2f} (avg: ${entry_stats['mean']:.2f})"
            )
            logger.debug(
...
def _implement_fallback_data_recovery(trade_log_path: str, min_samples: int) -> None:
    ...
    try:
        if not Path(trade_log_path).exists():
            logger.error(
                f"META_LEARNING_FALLBACK: Trade log file does not exist: {trade_log_path}"
            )
        ...
        if file_size == 0:
            logger.error(
                f"META_LEARNING_FALLBACK: Trade log file is empty: {trade_log_path}"
            )
            _create_emergency_trade_log(trade_log_path)
            return
        ...
        with open(trade_log_path, "r") as f:
            ...
            if not header_line:
                logger.error(
                    f"META_LEARNING_FALLBACK: Trade log file has no header: {trade_log_path}"
                )
                _create_emergency_trade_log(trade_log_path)
                return
            ...
            if missing_cols:
                logger.error(
                    f"META_LEARNING_FALLBACK: Missing required columns {missing_cols} in {trade_log_path}"
                )
                _backup_and_fix_trade_log(trade_log_path, header_cols, required_cols)
                return

        logger.info(
            f"META_LEARNING_FALLBACK: Trade log format appears valid, insufficient data for min_samples={min_samples}"
        )
...
def _backup_and_fix_trade_log(
    trade_log_path: str, current_cols: list, required_cols: list
) -> None:
    """Backup existing log and attempt to fix format issues."""
    try:
        # Create backup
        backup_path = (
            f"{trade_log_path}.backup.{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
        )
        import shutil

        shutil.copy2(trade_log_path, backup_path)
        logger.info(
            f"META_LEARNING_BACKUP: Backed up corrupted trade log to {backup_path}"
        )
...
def _generate_bootstrap_training_data(trade_log_path: str, target_samples: int) -> None:
    ...
    try:
        ...
        if len(existing_data) < 2:
            logger.warning(
                "META_LEARNING_BOOTSTRAP_INSUFFICIENT_PATTERNS: Need at least 2 trades for pattern analysis"
            )
            return
        ...
        avg_entry_price = np.mean(
            [
                float(trade.get("entry_price", 100))
...
def _append_bootstrap_trades_to_log(
    trade_log_path: str, bootstrap_trades: list
) -> None:
    ...
    try:
        ...
        if len(existing_data) < min_samples // 2:  # Only if we have very little data
            synthetic_trades = _generate_synthetic_trades(
                min_samples // 4, existing_data
            )
            ...
            if synthetic_trades:
                ...
                logger.info(
                    "META_LEARNING_SYNTHETIC: No synthetic data generated - insufficient patterns"
                )
        else:
            logger.info(
...
def _generate_synthetic_trades(num_trades: int, pattern_data: list) -> list:
    ...
    try:
        ...
        for i in range(
            min(num_trades, 10)
        ):  # Limit synthetic trades to prevent overfitting
            ...
            if is_winner:
                exit_price = entry_price * (
                    1.0 + random.uniform(0.005, 0.03)
                )  # 0.5-3% gain
            else:
                exit_price = entry_price * (
                    1.0 - random.uniform(0.01, 0.025)
                )  # 1-2.5% loss
...
def _append_synthetic_trades_to_log(
    trade_log_path: str, synthetic_trades: list
) -> None:
...
def _convert_mixed_format_to_meta(df: "pd.DataFrame") -> "pd.DataFrame":
    ...
    try:
        ...
        logger.debug(
            f"META_LEARNING_MIXED_CONVERSION: Processing {len(df)} rows with mixed format"
        )
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if len(row) >= 6:
                    ...
                    status = (
                        str(row.iloc[7]).strip().lower() if len(row) >= 8 else "filled"
                    )

                    # Skip invalid or unfilled orders
                    if status not in ["filled", "partially_filled"] or symbol in [
                        "",
                        "symbol",
                    ]:
                        continue
                    ...
                    if side == "buy":
                        ...
                        side == "sell"
                        and symbol in position_tracker
                        and position_tracker[symbol]
                    ):
        ...
        if meta_rows:
            ...
            logger.info(
                f"META_LEARNING_MIXED_CONVERSION: Successfully converted {len(result_df)} trades from mixed format"
            )
            return result_df
        else:
            logger.warning(
                "META_LEARNING_MIXED_CONVERSION: No valid trades found after conversion"
            )
            return pd.DataFrame()
...
def _convert_audit_to_meta_format(df: "pd.DataFrame") -> "pd.DataFrame":
    ...
    try:
        ...
        logger.debug(
            f"META_LEARNING_AUDIT_CONVERSION: Found {len(audit_rows)} audit format rows to convert"
        )
        ...
        for row in audit_rows:
            try:
                ...
                if len(row) >= 6:
                    ...
                    price = str(row.iloc[5]).strip()
                    status = (
                        str(row.iloc[7]).strip().lower() if len(row) >= 8 else "unknown"
                    )
                    ...
                    if status in invalid_statuses:
                        logger.debug(
                            f"Skipping audit row with invalid status: {status}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Invalid numeric values in audit row: qty={qty}, price={price}"
                        )
                        continue
                    ...
                    matching_positions = [
                        p
                        for p in position_tracker[symbol]
                        if p["side"] == opposite_side and not p.get("matched", False)
                    ]
                    ...
                    if matching_positions:
                        ...
                        for existing_row in meta_rows:
                            if (
                                ...
                                and existing_row["entry_price"] == match["entry_price"]
                                and existing_row["side"] == opposite_side
                                and not existing_row["exit_price"]
                            ):
                                existing_row["exit_time"] = timestamp
                                existing_row["exit_price"] = price_val
                                if side == "sell":
                                    existing_row["reward"] = (
                                        price_val - existing_row["entry_price"]
                                    ) * existing_row["qty"]
                                else:
                                    existing_row["reward"] = (
                                        existing_row["entry_price"] - price_val
                                    ) * existing_row["qty"]
                                break
        ...
        if meta_rows:
            ...
            logger.info(
                f"META_LEARNING_AUDIT_CONVERSION: Successfully converted {len(meta_rows)} audit rows to meta format"
            )
            return converted_df
        else:
            logger.warning(
                "META_LEARNING_AUDIT_CONVERSION: No valid audit rows found for conversion"
            )
            return pd.DataFrame()
...
def optimize_signals(
    signal_data: Any, cfg: Any, model: Any | None = None, *, volatility: float = 1.0
) -> Any:
    ...
    if signal_data is None:
        logger.warning(
            "optimize_signals received None signal_data, returning empty list"
        )
        return []
    ...
    if hasattr(signal_data, "__len__") and len(signal_data) == 0:
        logger.warning(
            "optimize_signals received empty signal_data, returning empty list"
        )
        return []
    ...
    if model is not None:
        try:
            ...
            if predictions is None:
                logger.warning(
                    "Model prediction returned None, falling back to original signal_data"
                )
    ...
    if model is None:
        logger.debug(
            "No model available for signal optimization, returning original data"
        )
        return signal_data if signal_data is not None else []
    ...
    try:
        ...
        RuntimeError,
        TypeError,
    ) as exc:  # pragma: no cover - model may fail
...
def trigger_meta_learning_conversion(trade_data: dict) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "METALEARN_INVALID_PATH | invalid path format: %s, error: %s",
                trade_log_path,
                e,
            )
            return False
        ...
        if not trade_log_path_obj.is_file():
            logger.warning(
                "METALEARN_NOT_A_FILE | %s is not a regular file", trade_log_path
            )
            return False
        ...
        if conversion_needed:
            ...
            if pd is not None:
                try:
                    ...
                    if not converted_df.empty:
                        ...
                        logger.info(
                            "METALEARN_CONVERSION_SUCCESS | symbol=%s converted_rows=%d",
                            symbol,
                            len(converted_df),
                        )
                        return True
                        ...
                        logger.warning(
                            "METALEARN_CONVERSION_EMPTY | no valid conversion result"
                        )
                        return False
                ...
                logger.warning(
                    "METALEARN_NO_PANDAS | pandas not available for conversion"
                )
                return False
        else:
            ...
            if not trade_log_path_obj.exists() or not trade_log_path_obj.is_file():
                logger.warning(
                    "METALEARN_FILE_DISAPPEARED | trade log file no longer exists"
                )
                return False
            logger.debug(
                "METALEARN_NO_CONVERSION_NEEDED | trade log format is already correct"
            )
            return True
...
def store_meta_learning_data(converted_data: dict) -> bool:
    ...
    try:
        ...
        meta_log_path = getattr(
            config, "META_LEARNING_LOG_FILE", "logs/meta_trades.csv"
        )
        ...
        if pd is not None:
            ...
            logger.info(
                "METALEARN_DATA_STORED | symbol=%s path=%s",
                converted_data.get("symbol", "UNKNOWN"),
                meta_log_path,
            )
            return True
        else:
...
def load_global_signal_performance(
    ...
    try:
        ...
        if "bot_engine" not in sys.modules:
            ...
            logger.info(
                "META_LEARNING_FALLBACK: bot_engine not available, using fallback implementation"
            )

            # Get trade log path from config
            trade_log_file = (
                getattr(config, "TRADE_LOG_FILE", "trades.csv")
                if config
                else "trades.csv"
            )
            ...
            if pd is None:
                logger.warning(
                    "METALEARN_NO_PANDAS: pandas not available for signal performance loading"
                )
                return None
            ...
            try:
                ...
                if df.empty:
                    logger.warning(
                        "METALEARN_NO_VALID_PRICES - No trades with valid prices"
                    )
                    return {}
                ...
                logger.info(
                    f"META_LEARNING_SIGNAL_PERFORMANCE: Loaded {len(result)} signals with {min_trades}+ trades"
                )
            ...
            if hasattr(bot_engine, "load_global_signal_performance"):
                ...
                logger.warning(
                    "META_LEARNING_FUNCTION_MISSING: load_global_signal_performance not found in bot_engine"
                )
                return None
