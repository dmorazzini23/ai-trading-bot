...
_pool_stats = {'workers': int(os.getenv('HTTP_POOL_WORKERS', os.getenv('HTTP_MAX_WORKERS', '8'))), 'per_host': int(os.getenv('HTTP_MAX_PER_HOST', '6')), 'pool_maxsize': 32, 'requests': 0, 'responses': 0, 'errors': 0}

def _build_session() -> requests.Session:
    _pool_stats['per_host'] = int(os.getenv('HTTP_MAX_PER_HOST', str(_pool_stats['per_host'])))
    _pool_stats['workers'] = int(os.getenv('HTTP_POOL_WORKERS', os.getenv('HTTP_MAX_WORKERS', str(_pool_stats['workers']))))
    s = requests.Session()
    retries = Retry(total=3, backoff_factor=0.3, status_forcelist=(429, 500, 502, 503, 504), allowed_methods=('GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'OPTIONS'), raise_on_status=False)
    adapter = HTTPAdapter(pool_connections=_pool_stats['per_host'], pool_maxsize=_pool_stats['pool_maxsize'], max_retries=retries)
...
def request(method: str, url: str, **kwargs) -> requests.Response:
    ...
    try:
        resp = retry_call(_do_request, exceptions=excs, retries=retries, backoff=backoff, max_backoff=max_backoff, jitter=jitter)
...
def request_json(method: str, url: str, *, timeout: tuple[float, float] | float | None=(3.0, 10.0), retries: int=3, backoff: float=0.5, status_forcelist: set[int] | None=None, headers: dict | None=None, params: dict | None=None) -> dict:
    """Perform HTTP request and return decoded JSON with bounded retries."""
    status_forcelist = status_forcelist or {429, 500, 502, 503, 504}
    if isinstance(timeout, tuple):
        to = (clamp_timeout(timeout[0], default_non_test=HTTP_TIMEOUT), clamp_timeout(timeout[1], default_non_test=HTTP_TIMEOUT))
    else:
