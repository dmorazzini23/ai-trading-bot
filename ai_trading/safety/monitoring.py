...
class SafetyMonitor:
    ...
    def update_metrics(self, **kwargs):
        ...
        if "new_order" in kwargs:
            self.metrics["last_order_time"] = datetime.now(timezone.utc)
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        uptime = datetime.now(timezone.utc) - self.metrics["system_start_time"]
    ...
    def _kill_switch_monitor(self):
        ...
        while self.is_monitoring:
            try:
                ...
                if self.auto_kill_time and datetime.now(timezone.utc) >= self.auto_kill_time:
...
class PerformanceMonitor:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "throughput_orders_per_hour": len(latencies) / (uptime.total_seconds() / 3600) if uptime.total_seconds() > 0 else 0,
            "error_breakdown": self.metrics["error_rates"].copy(),
            "performance_grade": self._calculate_performance_grade(avg_latency, total_errors, len(latencies))
        }
