...
class UnifiedRLInference:
    ...
    def preprocess_observation(self, observation: np.ndarray) -> np.ndarray:
        ...
        if len(obs.shape) == 1:
            ...
            while len(self._obs_buffer) < self.config.observation_window:
                self._obs_buffer.insert(
                    0, self._obs_buffer[0] if self._obs_buffer else np.zeros_like(obs)
                )
        ...
        if obs.shape[0] > self.config.observation_window:
            ...
            padding = np.tile(
                obs[0:1], (self.config.observation_window - obs.shape[0], 1)
            )
    ...
    def postprocess_action(
        self, raw_action: int | float | np.ndarray, observation: np.ndarray
    ) -> dict[str, Any]:
        """
        ...
        if self.config.action_config.action_type == "discrete":
            ...
            confidence = (
                0.8 if action_int != 0 else 0.2
            )  # Higher confidence for non-hold actions

        else:
            action_float = (
                float(raw_action) if np.isscalar(raw_action) else float(raw_action[0])
            )
            ...
            if action_float > self.config.confidence_threshold:
                ...
                confidence = 1.0 - abs(
                    action_float
                )  # High confidence for small actions (hold)
    ...
    def predict(
        self, observation: np.ndarray, symbol: str = "RL"
    ) -> TradeSignal | None:
        """
        ...
        try:
            ...
            raw_action, _ = self.agent.model.predict(
                processed_obs, deterministic=self.config.deterministic
            )
    ...
    def predict_batch(
        self, observations: list[np.ndarray], symbols: list[str]
    ...
    def _update_stats(self, action_details: dict[str, Any]) -> None:
        ...
        self._inference_stats["avg_confidence"] = (
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        stats["last_prediction"] = (
            str(self._last_prediction) if self._last_prediction else None
        )
...
def create_unified_inference(
    ...
    """
    action_config = ActionSpaceConfig(
        action_type=action_type, discrete_actions=discrete_actions
    )
