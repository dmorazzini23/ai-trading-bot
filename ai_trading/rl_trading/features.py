...
def bollinger_position(
    close: pd.Series, length: int = 20, nstd: float = 2.0
) -> pd.Series:
    close = pd.to_numeric(close, errors="coerce")
...
def compute_features(df: pd.DataFrame, cfg: FeatureConfig | None = None) -> np.ndarray:
    ...
    vwapb = (
        vwap_bias(close, high, low, _safe_series(vol, len(df), 0.0)).fillna(0.0).tail(w)
    )
    ...
    obv_s = (
        (
            ...
            / (obv_s.rolling(w, min_periods=1).std() + 1e-8)
        )
        .fillna(0.0)
        .clip(-1.0, 1.0)
        .tail(w)
