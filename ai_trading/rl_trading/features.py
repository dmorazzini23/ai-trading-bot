...
def _safe_series(x: Optional[pd.Series], size: int, fill: float = 0.0) -> pd.Series:
    if x is None:
...
def compute_features(df: pd.DataFrame, cfg: Optional[FeatureConfig] = None) -> np.ndarray:
    if cfg is None:
        cfg = FeatureConfig()
    w = cfg.window
    df = df.copy()
    ...
    obv_s = ((obv_s - obv_s.rolling(w, min_periods=1).mean()) /
             (obv_s.rolling(w, min_periods=1).std() + 1e-8)).fillna(0.0).clip(-1.0, 1.0).tail(w)
