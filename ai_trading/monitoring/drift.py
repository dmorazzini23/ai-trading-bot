...
class AlertThreshold:
    ...
    psi_high: float = 0.2
    hit_ratio_min: float = 0.45
    sharpe_ratio_min: float = 0.5
    drawdown_max: float = 0.15
...
class DriftMonitor:
    ...
    def __init__(self, baseline_data_path: str='artifacts/monitoring/baseline', alert_thresholds: AlertThreshold | None=None):
        """
    ...
    def monitor_feature_drift(self, current_features: pd.DataFrame) -> list[DriftMetrics]:
        ...
        for feature_name in current_features.columns:
            ...
            if psi_score < self.alert_thresholds.psi_low:
                ...
                drift_level = 'high'
            metrics = DriftMetrics(feature_name=feature_name, psi_score=psi_score, drift_level=drift_level, baseline_mean=baseline_mean, current_mean=float(current_values.mean()), baseline_std=baseline_std, current_std=float(current_values.std()), sample_size=len(current_values), calculated_at=datetime.now(UTC))
    ...
    def calculate_signal_attribution(self, signal_name: str, signal_returns: pd.Series, benchmark_returns: pd.Series | None=None, period_start: datetime | None=None, period_end: datetime | None=None) -> SignalAttribution:
        ...
        attribution = SignalAttribution(signal_name=signal_name, period_return=total_return, hit_ratio=hit_ratio, sharpe_ratio=sharpe_ratio, turnover=turnover, max_drawdown=max_drawdown, trade_count=trade_count, avg_hold_period=avg_hold_period, total_pnl=total_pnl, period_start=period_start, period_end=period_end)
    ...
    def _check_attribution_alerts(self, attribution: SignalAttribution) -> None:
        ...
        if alerts:
            self.logger.warning(f"Performance alerts for {attribution.signal_name}: {'; '.join(alerts)}")
...
_global_drift_monitor: DriftMonitor | None = None
_global_shadow_mode: ShadowMode | None = None
