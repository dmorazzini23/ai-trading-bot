...
class DriftMonitor:
    ...
    def __init__(
        ...
        alert_thresholds: AlertThreshold | None = None,
    ):
        """
    ...
    def calculate_psi(
        self, baseline_data: np.ndarray, current_data: np.ndarray, n_bins: int = 10
    ) -> float:
        """
    ...
    def save_attribution_history(self, attribution: SignalAttribution) -> None:
        ...
        try:
            attribution_dicts = [
                attr.to_dict() for attr in self._signal_history[signal_name]
            ]
    ...
    def get_drift_summary(self, drift_metrics: list[DriftMetrics]) -> dict[str, Any]:
        ...
        return {
            ...
            "features_with_alerts": [
                m.feature_name
                for m in drift_metrics
                if m.drift_level in ["medium", "high"]
            ],
        }
...
class ShadowMode:
    ...
    def evaluate_shadow_model(
        ...
        common_symbols = set(shadow_predictions.keys()) & set(
            production_predictions.keys()
        )

        if not common_symbols:
            self.logger.warning(
                "No common symbols between shadow and production predictions"
            )
            return {}
