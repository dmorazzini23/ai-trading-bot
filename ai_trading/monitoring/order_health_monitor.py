...
class OrderHealthMonitor:
    ...
    def __init__(self, execution_engine=None):
        ...
        self.logger.info(
            "OrderHealthMonitor initialized with timeout=%ds, cleanup_interval=%ds",
    ...
    def _check_stale_orders(self) -> None:
        ...
        for order_info in stale_orders:
            ...
            if self.execution_engine:
                try:
                    success = self.execution_engine._cancel_stale_order(
                        order_info.order_id
                    )
                    if success:
                        self.logger.info(
    ...
    def _update_partial_fill_tracking(self) -> None:
        ...
        with self._lock:
            for order_id, partial_info in list(self._partial_fills.items()):
                ...
                time_since_update = (
                    current_time - partial_info.last_update
                ).total_seconds()
        ...
        try:
            ...
            with self._lock:
                if partial_info.order_id in self._partial_fills:
                    ...
                    self._partial_fills[partial_info.order_id].last_update = (
                        datetime.now(UTC)
                    )
    ...
    def _calculate_health_metrics(self) -> OrderHealthMetrics:
        ...
        avg_fill_rate = 1.0
        if partial_fills:
            avg_fill_rate = sum(pf.fill_rate for pf in partial_fills) / len(
                partial_fills
            )
    ...
    def _record_partial_fill(
        self, order_id: str, filled_qty: int, total_qty: int
    ) -> None:
    ...
    def get_health_summary(self) -> dict:
        ...
        with self._lock:
            recent_metrics = (
                list(self._order_metrics)[-10:] if self._order_metrics else []
            )
