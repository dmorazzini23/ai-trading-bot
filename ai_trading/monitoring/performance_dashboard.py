...
from ..core.constants import PERFORMANCE_THRESHOLDS, DATA_PARAMETERS
...
class RealTimePnLTracker:
    ...
    def update_position(self, symbol: str, quantity: int, avg_price: float, 
                       current_price: float, commission: float = 0.0):
    ...
    def record_trade(self, symbol: str, quantity: int, price: float, 
                    commission: float, trade_type: str = "unknown"):
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, current_return: float, current_pnl: float, 
                        current_volatility: float) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(self.returns_history) >= 30:
                ...
                if abs(current_return - mean_return) > self.sensitivity * std_return:
                    anomalies.append({
                        "type": "unusual_return",
                        "severity": "high" if abs(current_return) > 0.05 else "medium",
            ...
            if len(self.pnl_history) >= 30:
                ...
                if abs(current_pnl - mean_pnl) > self.sensitivity * std_pnl:
                    anomalies.append({
                        "type": "unusual_pnl",
                        "severity": "high" if abs(current_pnl) > 10000 else "medium",
                        "value": current_pnl,
                        "baseline": mean_pnl,
...
class PerformanceDashboard:
    ...
    def update_performance(self, equity_value: float, daily_return: float = None,
                ...
                if anomalies and self.alert_manager:
                    for anomaly in anomalies:
                        ...
                        self.alert_manager.send_performance_alert(
                            anomaly["type"], anomaly["value"], anomaly["threshold"], severity
                        )
    ...
    def _check_performance_thresholds(self):
        ...
        try:
            ...
            if sharpe < self.alert_thresholds["MIN_SHARPE_RATIO"]:
                self.alert_manager.send_performance_alert(
                    "Sharpe Ratio", sharpe, self.alert_thresholds["MIN_SHARPE_RATIO"],
            ...
            if max_dd > self.alert_thresholds["MAX_DRAWDOWN"]:
                self.alert_manager.send_performance_alert(
                    "Maximum Drawdown", f"{max_dd:.2%}", f"{self.alert_thresholds['MAX_DRAWDOWN']:.2%}",
            ...
            if win_rate < self.alert_thresholds["MIN_WIN_RATE"]:
                self.alert_manager.send_performance_alert(
                    "Win Rate", f"{win_rate:.2%}", f"{self.alert_thresholds['MIN_WIN_RATE']:.2%}",
                    AlertSeverity.WARNING
                )
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "anomaly_details": recent_anomalies[-5:] if recent_anomalies else [],  # Last 5
                "system_status": {
    ...
    def add_trade(self, symbol: str, entry_time: datetime, exit_time: datetime,
        ...
        try:
            # Add to metrics
            self.metrics.add_trade(symbol, entry_time, exit_time, entry_price, 
