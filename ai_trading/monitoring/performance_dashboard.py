...
class PerformanceMetrics:
    ...
    def __init__(self, lookback_days: int = 252):
        ...
        # AI-AGENT-REF: Real-time performance metrics calculation
        ...
        logger.info(
            f"PerformanceMetrics initialized with lookback_days={lookback_days}"
        )
    ...
    def add_trade(
        ...
        try:
            trade = {
                ...
                "return_pct": (
                    (exit_price - entry_price) / entry_price if entry_price > 0 else 0
                ),
    ...
    def calculate_max_drawdown(
        self, equity_curve: list[float] = None
    ) -> tuple[float, int]:
    ...
    def calculate_win_rate(self) -> dict[str, float]:
        ...
        try:
            ...
            avg_win = (
                statistics.mean([t["net_pnl"] for t in winning_trades])
                if winning_trades
                else 0.0
            )
            avg_loss = (
                statistics.mean([abs(t["net_pnl"]) for t in losing_trades])
                if losing_trades
                else 0.0
            )
    ...
    def _calculate_metrics(self):
        ...
        try:
            ...
            volatility = (
                statistics.stdev(self.returns) * (252**0.5)
...
class RealTimePnLTracker:
    ...
    def __init__(self):
        ...
        # AI-AGENT-REF: Real-time P&L tracking system
        self.positions = {}
        self.daily_pnl = 0.0
    ...
    def record_trade(
        ...
        try:
            with self._lock:
                # Calculate trade P&L if we have position data
                trade_pnl = 0.0
                if symbol in self.positions:
    ...
    def get_pnl_summary(self) -> dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                session_pnl = (
                    (self.session_high_equity - self.session_start_equity)
                    if self.session_start_equity > 0
                    else 0
                )
                session_drawdown = (
                    (self.session_high_equity - self.session_low_equity)
                ...
                return {
                    ...
                    "session_high": self.session_high_equity,
...
class AnomalyDetector:
    ...
    def __init__(self, sensitivity: float = 2.0):
        ...
        # AI-AGENT-REF: Performance anomaly detection system
        ...
        self.volatility_history = deque(maxlen=50)  # 50 days for volatility
    ...
    def detect_anomalies(
        self, current_return: float, current_pnl: float, current_volatility: float
    ) -> list[dict[str, Any]]:
                    ...
                    anomalies.append(
                        {
                            "type": "unusual_return",
                            "severity": (
                                "high" if abs(current_return) > 0.05 else "medium"
                            ),
                            "value": current_return,
                            "baseline": mean_return,
                            "threshold": self.sensitivity * std_return,
                            "description": f"Daily return {current_return:.2%} is {abs(current_return - mean_return)/std_return:.1f} std devs from mean",
                        }
                    )
                    ...
                    anomalies.append(
                        {
                            "type": "unusual_pnl",
                            "severity": (
                                "high" if abs(current_pnl) > 10000 else "medium"
                            ),
                            "value": current_pnl,
                            "baseline": mean_pnl,
                            "threshold": self.sensitivity * std_pnl,
                            "description": f"Daily P&L ${current_pnl:,.2f} is unusual compared to recent performance",
                        }
                    )
                    ...
                    anomalies.append(
                        {
                            ...
                            "severity": (
                                "high" if current_volatility > 0.5 else "medium"
                            ),
                            "value": current_volatility,
                            "baseline": mean_vol,
                            "threshold": self.sensitivity * std_vol,
                            "description": f"Volatility {current_volatility:.1%} is significantly different from recent levels",
                        }
                    )
        ...
        try:
            if len(self.returns_history) >= 30:
                self.return_threshold = (
                    statistics.stdev(self.returns_history) * self.sensitivity
                )

            if len(self.pnl_history) >= 30:
                self.pnl_threshold = (
                    statistics.stdev(self.pnl_history) * self.sensitivity
                )
...
class PerformanceDashboard:
    ...
    def __init__(self, alert_manager: AlertManager = None):
        ...
        # AI-AGENT-REF: Comprehensive performance monitoring dashboard
    ...
    def update_performance(
        self, equity_value: float, daily_return: float = None, volatility: float = None
    ):
        ...
        try:
            ...
            if daily_return is not None and volatility is not None:
                ...
                anomalies = self.anomaly_detector.detect_anomalies(
                    daily_return, daily_pnl, volatility
                )
        ...
        try:
            ...
            if win_rate < self.alert_thresholds["MIN_WIN_RATE"]:
                self.alert_manager.send_performance_alert(
                    "Win Rate",
                    f"{win_rate:.2%}",
                    f"{self.alert_thresholds['MIN_WIN_RATE']:.2%}",
                    AlertSeverity.WARNING,
                )
    ...
    def get_dashboard_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "anomaly_details": (
                    recent_anomalies[-5:] if recent_anomalies else []
                ),  # Last 5
                "system_status": {
                    "last_update": self.last_update,
                    "update_interval": self.update_interval,
                    "alerts_configured": self.alert_manager is not None,
                },
            }
