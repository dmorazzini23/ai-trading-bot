...
class BuildFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(
                f"Generated {features.shape[1]} features from {X.shape[1]} inputs"
            )
            return features
    ...
    def _add_volatility_features(
        ...
        try:
            ...
            # EWMA volatility
            features[f"vol_ewma_{self.vol_span}"] = returns.ewm(
                span=self.vol_span
            ).std()
            ...
            features["vol_rank"] = (
                features["vol_20d"].rolling(vol_rank_window).rank(pct=True)
            )
    ...
    def _add_volume_features(
        self, features: pd.DataFrame, volume: pd.Series
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if "close" in features.columns:
                ...
                if "high" in features.columns and "low" in features.columns:
                    ...
                    vwap = (typical_price * volume).rolling(20).sum() / volume.rolling(
                        20
                    ).sum()
    ...
    def _add_regime_features(
        ...
        try:
            ...
            # Volatility regime
            vol_low = self.feature_params_.get(
                "regime_vol_low", rolling_vol.quantile(0.33)
            )
            vol_high = self.feature_params_.get(
                "regime_vol_high", rolling_vol.quantile(0.67)
            )
            ...
            if "high" in features.columns and "low" in features.columns:
                ...
                if "volume" in features.columns:
                    # Simplified VWAP
                    vwap_approx = (
                        (features["high"] + features["low"] + prices)
                        / 3
                        * features["volume"]
                    ).rolling(20).sum() / features["volume"].rolling(20).sum()
...
def create_feature_pipeline(
    scaler_type: str = "standard", build_features_params: dict[str, Any] | None = None
) -> Pipeline:
    """
    ...
    try:
        ...
        if hasattr(X_train_transformed, "mean"):
            ...
            if mean_diff < tolerance:
                logger.warning(
                    "Train and test statistics are very similar - possible leakage"
                )
                return False
