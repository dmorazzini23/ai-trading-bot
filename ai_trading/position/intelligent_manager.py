...
class IntelligentPositionManager:
    ...
    def __init__(self, ctx=None):
        ...
        # Caching
        self.last_portfolio_analysis: Optional[PortfolioAnalysis] = None
        self.last_regime: Optional[RegimeMetrics] = None
        
    def analyze_position(self, symbol: str, position_data: Any, 
                        current_positions: List[Any]) -> PositionRecommendation:
        """
        ...
        try:
            ...
            recommendation = self._generate_integrated_recommendation(
                ...
                regime_analysis, technical_analysis, profit_analysis,
                stop_analysis, correlation_analysis
            )
            
            self.logger.info(
                "POSITION_ANALYSIS | %s action=%s confidence=%.2f urgency=%.2f reason=%s",
                symbol, recommendation.action.value, recommendation.confidence,
                recommendation.urgency, recommendation.primary_reason
            )
    ...
    def should_hold_position(self, symbol: str, current_position, unrealized_pnl_pct: float, 
                           days_held: int, current_positions: List[Any] = None) -> bool:
        """
    ...
    def _analyze_profit_opportunities(self, symbol: str, position_data: Any, 
    ...
    def _analyze_trailing_stops(self, symbol: str, position_data: Any, 
                               current_price: float) -> Dict[str, Any]:
    ...
    def _analyze_portfolio_context(self, symbol: str, current_positions: List[Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (self.last_portfolio_analysis and 
                (datetime.now(timezone.utc) - self.last_portfolio_analysis.timestamp).seconds < 300):
    ...
    def _generate_integrated_recommendation(self, symbol: str, position_data: Any, current_price: float,
                                          regime_analysis: Dict, technical_analysis: Dict,
                                          profit_analysis: Dict, stop_analysis: Dict,
                                          correlation_analysis: Dict) -> PositionRecommendation:
        ...
        try:
            ...
            quantity_to_sell, percentage_to_sell, target_price, stop_price = self._calculate_action_details(
                action, position_data, current_price, triggered_targets, stop_analysis
            )
            
            # Determine primary reason
            primary_reason = self._determine_primary_reason(
                action, regime_analysis, technical_analysis, profit_analysis,
                stop_analysis, correlation_analysis
            )
            ...
            recommendation = PositionRecommendation(
                ...
                technical_influence=max(hold_score, sell_score) * self.analysis_weights['technical'],
                profit_taking_influence=partial_sell_score * self.analysis_weights['profit_taking'],
    ...
    def _determine_action_from_scores(self, hold_score: float, sell_score: float, 
                                    partial_sell_score: float) -> Tuple[PositionAction, float, float]:
        ...
        urgency = max(0.0, min(1.0, (max_score - 0.3) / 0.4))  # Urgency starts at 0.3 score
    ...
    def _calculate_action_details(self, action: PositionAction, position_data: Any, 
                                current_price: float, triggered_targets: List,
                                stop_analysis: Dict) -> Tuple[Optional[int], Optional[float], 
    ...
    def _determine_primary_reason(self, action: PositionAction, regime_analysis: Dict,
                                technical_analysis: Dict, profit_analysis: Dict,
                                stop_analysis: Dict, correlation_analysis: Dict) -> str:
