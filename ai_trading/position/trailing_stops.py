...
class TrailingStopManager:
    ...
    def update_trailing_stop(self, symbol: str, position_data: Any, 
                           current_price: float) -> Optional[TrailingStopLevel]:
        """
        ...
        try:
            ...
            stop_level.last_updated = datetime.now(timezone.utc)
            ...
            self.logger.info(
                ...
                symbol, current_price, stop_level.stop_price, 
                stop_level.trail_distance, unrealized_gain_pct
            )
    ...
    def get_stop_level(self, symbol: str) -> Optional[TrailingStopLevel]:
    ...
    def _initialize_stop_level(self, symbol: str, entry_price: float, 
                              current_price: float, position_data: Any) -> TrailingStopLevel:
        ...
        self.logger.info(
            ...
            symbol, entry_price, current_price, initial_stop, initial_distance
        )
    ...
    def _calculate_adaptive_stop(self, symbol: str, stop_level: TrailingStopLevel) -> float:
        ...
        try:
            ...
            if hasattr(stop_level, 'qty') or current_price > entry_price:  # Assume long for now
    ...
    def _calculate_atr_stop_distance(self, data: pd.DataFrame) -> float:
        ...
        try:
            if ('high' not in data.columns or 'low' not in data.columns or
                'close' not in data.columns or len(data) < self.atr_period):
                return self.base_trail_percent
    ...
    def _combine_stop_distances(self, distances: Dict[str, float], 
                               stop_level: TrailingStopLevel) -> float:
        ...
        try:
            ...
            if stop_level.is_triggered:
                self.logger.warning(
                    ...
                    stop_level.symbol, stop_level.trigger_reason
                )
