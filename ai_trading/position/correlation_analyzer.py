...
class PortfolioCorrelationAnalyzer:
    ...
    def analyze_portfolio(self, positions: list[Any]) -> PortfolioAnalysis:
        ...
        try:
            ...
            analysis = PortfolioAnalysis(
                ...
                concentration_level=portfolio_metrics.get(
                    "concentration_level", ConcentrationLevel.LOW
                ),
                diversification_ratio=portfolio_metrics.get(
                    "diversification_ratio", 1.0
                ),
                effective_positions=portfolio_metrics.get(
                    "effective_positions", len(position_data)
                ),
            ...
            self.logger.info(
                ...
                len(position_data),
                analysis.avg_portfolio_correlation,
                analysis.max_correlation,
                analysis.concentration_level.value,
            )
    ...
    def get_position_correlation(
        self, symbol1: str, symbol2: str
    ) -> PositionCorrelation | None:
    ...
    def should_reduce_position(
        ...
        try:
            ...
            for sector_exp in self.last_analysis.sector_exposures:
                if (
                    sector_exp.sector == symbol_sector
                    and sector_exp.concentration_level
                    in [ConcentrationLevel.HIGH, ConcentrationLevel.EXTREME]
                ):
                    return (
                        True,
                        f"Sector concentration: {sector_exp.portfolio_percentage:.1f}%",
                    )
            ...
            if self.last_analysis.largest_position_pct > 40.0:
                ...
                if symbol in position_data:
                    symbol_pct = (
                        ...
                        / self.last_analysis.total_value
                    ) * 100
                    if symbol_pct > 30.0:
    ...
    def get_correlation_adjustment_factor(self, symbol: str) -> float:
        ...
        try:
            ...
            symbol_correlations = [
                corr
                for corr in self.last_analysis.position_correlations
            ...
            avg_correlation = sum(
                abs(corr.correlation) for corr in symbol_correlations
    ...
    def _classify_correlation_strength(
        ...
        if abs_correlation >= self.correlation_thresholds[CorrelationStrength.HIGH]:
            ...
            abs_correlation >= self.correlation_thresholds[CorrelationStrength.MODERATE]
        ):
            return CorrelationStrength.HIGH
        elif abs_correlation >= self.correlation_thresholds[CorrelationStrength.LOW]:
            return CorrelationStrength.MODERATE
        elif (
            abs_correlation >= self.correlation_thresholds[CorrelationStrength.VERY_LOW]
        ):
    ...
    def _analyze_sector_exposures(
        ...
        try:
            ...
            for sector, positions in sector_groups.items():
                ...
                sector_pct = (
                    (sector_value / total_value) * 100 if total_value > 0 else 0
                )
    ...
    def _classify_sector_concentration(self, sector_pct: float) -> ConcentrationLevel:
        ...
        if sector_pct >= self.sector_concentration_thresholds[ConcentrationLevel.HIGH]:
            ...
            sector_pct
            >= self.sector_concentration_thresholds[ConcentrationLevel.MODERATE]
        ):
    ...
    def _calculate_portfolio_metrics(
        ...
        try:
            ...
            if correlations:
                avg_correlation = sum(
                    abs(corr.correlation) for corr in correlations
                ) / len(correlations)
            ...
            concentration_level = self._classify_position_concentration(
    ...
    def _classify_position_concentration(
        self, largest_position_pct: float
    ) -> ConcentrationLevel:
        """Classify portfolio concentration based on largest position."""
        if (
            largest_position_pct
            >= self.position_concentration_thresholds[ConcentrationLevel.HIGH]
        ):
            return ConcentrationLevel.EXTREME
        elif (
            largest_position_pct
            >= self.position_concentration_thresholds[ConcentrationLevel.MODERATE]
        ):
            return ConcentrationLevel.HIGH
        elif (
            largest_position_pct
            >= self.position_concentration_thresholds[ConcentrationLevel.LOW]
        ):
            return ConcentrationLevel.MODERATE
        else:
            return ConcentrationLevel.LOW
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            for sector_exp in sector_exposures:
                if sector_exp.concentration_level in [
                    ConcentrationLevel.HIGH,
                    ConcentrationLevel.EXTREME,
                ]:
                    # Recommend reducing largest positions in concentrated sectors
                    sector_positions = [
                        (symbol, position_data[symbol]["market_value"])
                        for symbol in sector_exp.symbols
                        if symbol in position_data
                    ]
            ...
            if (
                len(sector_exposures) > 0
                and sector_exposures[0].portfolio_percentage > 50.0
            ):
                rebalance.append(
                    f"High sector concentration in {sector_exposures[0].sector}"
                )
