...
class ProfitTakingEngine:
    ...
    def __init__(self, ctx=None):
        ...
        self.default_targets = [
            {'level': 2.0, 'quantity_pct': 25.0, 'strategy': ProfitTakingStrategy.RISK_MULTIPLE},
            {'level': 3.0, 'quantity_pct': 25.0, 'strategy': ProfitTakingStrategy.RISK_MULTIPLE},
            {'level': 5.0, 'quantity_pct': 25.0, 'strategy': ProfitTakingStrategy.RISK_MULTIPLE},
    ...
    def create_profit_plan(self, symbol: str, position_data: Any, 
                          entry_price: float, risk_amount: float) -> ProfitTakingPlan:
        """
        ...
        try:
            ...
            targets.extend(self._create_risk_multiple_targets(
            ...
            self.logger.info(
                ...
                symbol, entry_price, position_size, len(targets)
            )
    ...
    def update_profit_plan(self, symbol: str, current_price: float, 
                          position_data: Any = None) -> List[ProfitTarget]:
        """
        ...
        try:
            ...
            for target in plan.targets:
                if not target.triggered and self._is_target_triggered(target, plan):
                    ...
                    target.trigger_time = datetime.now(timezone.utc)
                    triggered_targets.append(target)
                    
                    self.logger.info(
                        "PROFIT_TARGET_TRIGGERED | %s level=%.2f qty_pct=%.1f%% strategy=%s",
                        symbol, target.level, target.quantity_pct, target.strategy.value
                    )
    ...
    def get_profit_plan(self, symbol: str) -> Optional[ProfitTakingPlan]:
    ...
    def calculate_profit_velocity(self, symbol: str) -> float:
        ...
        try:
            ...
            plan = self.profit_plans[symbol]
            days_held = (datetime.now(timezone.utc) - plan.created_time).days
    ...
    def _create_risk_multiple_targets(self, entry_price: float, risk_amount: float, 
                                    position_size: int) -> List[ProfitTarget]:
    ...
    def _create_technical_targets(self, symbol: str, current_price: float, 
                                position_size: int) -> List[ProfitTarget]:
    ...
    def _create_time_based_targets(self, entry_price: float, current_price: float, 
                                 position_size: int) -> List[ProfitTarget]:
    ...
    def _is_target_triggered(self, target: ProfitTarget, plan: ProfitTakingPlan) -> bool:
        ...
        try:
            ...
            if target.strategy == ProfitTakingStrategy.RISK_MULTIPLE:
                return current_price >= target.level
            
            elif target.strategy == ProfitTakingStrategy.TECHNICAL_LEVELS:
                return current_price >= target.level
            
            elif target.strategy == ProfitTakingStrategy.PERCENTAGE_BASED:
                return current_price >= target.level
                ...
                if days_held >= self.time_decay_days:
                    ...
                    time_factor = min(1.0, days_held / 30.0)  # Full factor after 30 days
                    adjusted_level = target.level * (1 - time_factor * 0.02)  # Up to 2% reduction
        ...
        try:
            ...
            if current_gain_pct > 10.0:  # Significant gain
                ...
                recent_correlation_targets = [
                    ...
                    if (t.strategy == ProfitTakingStrategy.CORRELATION_BASED and 
                        t.triggered and t.trigger_time and
                        (datetime.now(timezone.utc) - t.trigger_time).days < 5)
                ]
                
                if not recent_correlation_targets:
                    correlation_target.triggered = True
                    correlation_target.trigger_time = datetime.now(timezone.utc)
                    plan.targets.append(correlation_target)
                    triggered_targets.append(correlation_target)
                    
                    self.logger.info(
                        "CORRELATION_TARGET_TRIGGERED | %s gain=%.2f%%",
                        symbol, current_gain_pct
                    )
    ...
    def _create_rsi_overbought_target(self, symbol: str, data: pd.DataFrame, 
                                    position_size: int) -> Optional[ProfitTarget]:
