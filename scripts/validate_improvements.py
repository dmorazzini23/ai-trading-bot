...
def run_basic_pytest():
    ...
    try:
        result = subprocess.run([sys.executable, '-m', 'pytest', '--collect-only', '-q'], cwd='/home/runner/work/ai-trading-bot/ai-trading-bot', capture_output=True, text=True, timeout=30, check=True)
...
def main():
    ...
    os.environ['TESTING'] = 'true'
    tests = [test_package_imports, test_timezone_usage, test_idempotency_and_reconciliation, check_shebang_removal, run_basic_pytest]
    results = []
    for test in tests:
        try:
            result = test()
