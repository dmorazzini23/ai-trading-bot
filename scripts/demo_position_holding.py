...
def demo_position_holding_logic():
    ...
    scenarios = [('UBER', None, 28.3, 5, 'High profit winner from logs'), ('SHOP', None, 20.0, 3, 'Good profit winner from logs'), ('PG', None, 16.6, 2, 'Decent profit, new position'), ('JNJ', None, -2.5, 1, 'Small loss, very new position'), ('NVDA', None, 3.2, 7, 'Small gain, older position'), ('GOOGL', None, -8.1, 10, 'Loss, old position')]
...
def demo_signal_enhancement():
    ...
    logging.info(str('=' * 60))
    original_signals = [{'symbol': 'UBER', 'side': 'sell', 'confidence': 0.6, 'reason': 'take_profit'}, {'symbol': 'SHOP', 'side': 'sell', 'confidence': 0.7, 'reason': 'momentum_fade'}, {'symbol': 'PG', 'side': 'sell', 'confidence': 0.5, 'reason': 'rebalance'}, {'symbol': 'JNJ', 'side': 'buy', 'confidence': 0.8, 'reason': 'new_entry'}, {'symbol': 'NVDA', 'side': 'buy', 'confidence': 0.9, 'reason': 'breakout'}, {'symbol': 'GOOGL', 'side': 'sell', 'confidence': 0.9, 'reason': 'stop_loss'}]
    hold_signals = {'UBER': 'hold', 'SHOP': 'hold', 'PG': 'hold', 'JNJ': 'neutral', 'NVDA': 'neutral', 'GOOGL': 'sell'}
...
def demo_meta_learning_trigger():
    ...
    logging.info(str('=' * 60))
    trade_executions = [{'symbol': 'META', 'qty': 50, 'side': 'buy', 'price': 485.2, 'order_id': 'meta-001'}, {'symbol': 'AMZN', 'qty': 25, 'side': 'buy', 'price': 186.5, 'order_id': 'amzn-001'}, {'symbol': 'CRM', 'qty': 100, 'side': 'sell', 'price': 315.8, 'order_id': 'crm-001'}, {'symbol': 'NVDA', 'qty': 75, 'side': 'buy', 'price': 875.3, 'order_id': 'nvda-001'}]
    ...
    for trade in trade_executions:
        ...
        {**trade, 'timestamp': datetime.now(UTC).isoformat(), 'status': 'filled'}
    ...
    logging.info("• No more 'METALEARN_EMPTY_TRADE_LOG' errors")
