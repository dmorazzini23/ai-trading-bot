...
def evaluator(params):
    ...
    base_sharpe = 1.2
    kelly_impact = (kelly - 0.5) * 0.8
    atr_impact = (2.0 - atr_mult) * 0.2
    lookback_impact = (100 - lookback) * 0.001
    sharpe = base_sharpe + kelly_impact + atr_impact + lookback_impact
    return {'sharpe': round(sharpe, 3), 'calmar': round(sharpe * 0.7, 3), 'mdd': round(0.15 - sharpe * 0.02, 3), 'total_return': round(sharpe * 0.12, 3), 'volatility': round(0.18 - sharpe * 0.01, 3)}

def main():
    """Run example grid search for parameter optimization."""
    logging.info('=== AI Trading Bot - Grid Search Example ===\n')
    grid = {'kelly': [0.3, 0.6, 0.9], 'atr_mult': [1.5, 2.0, 2.5], 'lookback': [50, 100, 150]}
