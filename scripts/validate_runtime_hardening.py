...
def test_http_timeouts():
    ...
    for p in root.rglob('*.py'):
        ...
        for m in re.finditer('requests\\.(get|post|put|delete|patch)\\s*\\(', txt):
            start = m.start()
            i = start + len(m.group(0)) - 1
            paren_count = 1
            j = i + 1
...
def main():
    ...
    tests = [test_utc_helper, test_http_timeouts, test_package_imports, test_lazy_loading, test_version_warning]
    passed = 0
    failed = 0
    for test in tests:
        try:
