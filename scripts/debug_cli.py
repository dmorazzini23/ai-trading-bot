...
def cmd_positions():
    ...
    logging.info(str('=' * 40))
    try:
        from ai_trading.execution import force_position_reconciliation, get_position_reconciler
...
def cmd_pnl(symbol=None):
    ...
    logging.info(str('=' * 40))
    try:
        from ai_trading.execution import explain_recent_pnl_changes, get_pnl_attributor, get_portfolio_pnl_summary, get_symbol_pnl_breakdown
        get_pnl_attributor()
        if symbol:
...
def cmd_health():
    ...
    logging.info(str('=' * 40))
    try:
        from ai_trading.execution import force_position_reconciliation, get_execution_statistics, get_pnl_attribution_stats
        issues = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Trading Execution Debugging CLI Tool', formatter_class=argparse.RawDescriptionHelpFormatter)
