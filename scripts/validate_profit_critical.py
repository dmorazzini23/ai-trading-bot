...
def test_money_math():
    ...
    logging.info(f"Money('1.005').quantize(Decimal('0.01')) = {result_str}")
...
def test_symbol_specs():
    ...
    from decimal import Decimal
    from symbol_specs import get_lot_size, get_symbol_spec, get_tick_size
...
def main():
    ...
    tests = [test_file_structure, test_money_math, test_symbol_specs]
    results = []
    for test in tests:
        try:
            result = test()
