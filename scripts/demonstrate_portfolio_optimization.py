...
def demonstrate_portfolio_optimization():
    ...
    current_positions = {'AAPL': 100.0, 'MSFT': 80.0, 'GOOGL': 60.0, 'TSLA': 40.0}
    market_data = {'prices': {'AAPL': 150.0, 'MSFT': 300.0, 'GOOGL': 2500.0, 'TSLA': 200.0, 'SPY': 400.0}, 'returns': {'AAPL': [0.01, -0.02, 0.015, 0.005, -0.01, 0.02, -0.005] * 15, 'MSFT': [0.005, 0.01, -0.01, 0.02, 0.005, -0.015, 0.01] * 15, 'GOOGL': [0.02, -0.015, 0.01, -0.005, 0.01, 0.005, -0.02] * 15, 'TSLA': [0.03, -0.04, 0.02, -0.01, 0.015, -0.025, 0.01] * 15, 'SPY': [0.008, -0.012, 0.01, 0.005, -0.008, 0.015, -0.003] * 15}, 'correlations': {'AAPL': {'MSFT': 0.6, 'GOOGL': 0.4, 'TSLA': 0.3}, 'MSFT': {'AAPL': 0.6, 'GOOGL': 0.5, 'TSLA': 0.2}, 'GOOGL': {'AAPL': 0.4, 'MSFT': 0.5, 'TSLA': 0.25}, 'TSLA': {'AAPL': 0.3, 'MSFT': 0.2, 'GOOGL': 0.25}}, 'volumes': {'AAPL': 50000000, 'MSFT': 30000000, 'GOOGL': 20000000, 'TSLA': 25000000, 'SPY': 100000000}}
    ...
    kelly_efficiency = portfolio_optimizer.calculate_portfolio_kelly_efficiency(current_positions, market_data['returns'], market_data['prices'])
    ...
    trade_proposals = [('AAPL', 'Small Increase', 110.0), ('MSFT', 'Medium Increase', 100.0), ('GOOGL', 'Large Increase', 120.0), ('TSLA', 'Sell Half', 20.0), ('NVDA', 'New Position', 50.0)]
    approved_trades = 0
    rejected_trades = 0
    deferred_trades = 0
    for symbol, description, proposed_position in trade_proposals:
        logging.info(f'   📈 Analyzing: {symbol} - {description}')
        decision, reasoning = portfolio_optimizer.make_portfolio_decision(symbol, proposed_position, current_positions, market_data)
        if decision == PortfolioDecision.APPROVE:
            ...
            expected_profit = position_change * market_data['prices'].get(symbol, 100.0) * 0.02
            profitability = transaction_calculator.validate_trade_profitability(symbol, position_change, expected_profit, market_data)
    ...
    target_weights = {'AAPL': 0.3, 'MSFT': 0.3, 'GOOGL': 0.25, 'TSLA': 0.15}
    should_rebalance, reason = portfolio_optimizer.should_trigger_rebalance(current_positions, target_weights, market_data['prices'])
...
if __name__ == '__main__':
    ...
    except Exception as e:  # noqa: BLE001 - demo script guard
