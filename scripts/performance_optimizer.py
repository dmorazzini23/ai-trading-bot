...
class PerformanceOptimizer:
    ...
    def __init__(self, enable_memory_profiling: bool=True):
        ...
        self.memory_thresholds = {'warning': 1024, 'critical': 2048, 'gc_trigger': 512}
        self.performance_targets = {'order_execution_ms': 10.0, 'data_processing_ms': 5.0, 'indicator_calculation_ms': 2.0, 'risk_check_ms': 1.0}
    ...
    def _record_performance(self, func_name: str, execution_time_ms: float, memory_delta: float):
        ...
        if func_name in self.function_profiles:
            ...
            self.function_profiles[func_name] = PerformanceProfile(function_name=func_name, call_count=1, total_time=execution_time_ms, avg_time=execution_time_ms, max_time=execution_time_ms, min_time=execution_time_ms, memory_usage=memory_delta, timestamp=datetime.now(UTC))
    ...
    def _check_performance_thresholds(self, func_name: str, execution_time_ms: float):
        ...
        if target_key and target_key in self.performance_targets:
            ...
            if execution_time_ms > threshold * 2:
                self.logger.error(f'CRITICAL: {func_name} execution time {execution_time_ms:.2f}ms exceeds critical threshold {threshold * 2:.2f}ms')
            elif execution_time_ms > threshold:
                self.logger.warning(f'WARNING: {func_name} execution time {execution_time_ms:.2f}ms exceeds target {threshold:.2f}ms')
    ...
    def take_memory_snapshot(self) -> MemorySnapshot:
        ...
        gc_stats = {'objects': len(gc.get_objects()), 'collections': gc.get_stats(), 'thresholds': gc.get_threshold()}
        snapshot = MemorySnapshot(timestamp=datetime.now(UTC), rss_mb=memory_info.rss / 1024 / 1024, vms_mb=memory_info.vms / 1024 / 1024, percent=system_memory.percent, available_mb=system_memory.available / 1024 / 1024, gc_objects=gc_stats['objects'], gc_stats=gc_stats)
    ...
    def cached_function(self, cache_key_func: Callable | None=None, ttl_seconds: int=300):
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        return {'cache_size': len(self.cache), 'cache_hits': self.cache_hits, 'cache_misses': self.cache_misses, 'hit_rate_percent': hit_rate, 'max_size': self.cache_max_size}

    def get_performance_report(self) -> dict[str, Any]:
        """Generate comprehensive performance report."""
        sorted_profiles = sorted(self.function_profiles.values(), key=lambda p: p.avg_time, reverse=True)
        recent_snapshots = list(self.memory_snapshots)[-10:] if self.memory_snapshots else []
        memory_trend = {'current_mb': recent_snapshots[-1].rss_mb if recent_snapshots else 0, 'avg_mb': statistics.mean([s.rss_mb for s in recent_snapshots]) if recent_snapshots else 0, 'max_mb': max([s.rss_mb for s in recent_snapshots]) if recent_snapshots else 0, 'snapshots_count': len(recent_snapshots)}
        violations = []
            ...
            for target_key, threshold in self.performance_targets.items():
                if target_key in profile.function_name.lower():
                    if profile.avg_time > threshold:
                        violations.append({'function': profile.function_name, 'avg_time_ms': profile.avg_time, 'threshold_ms': threshold, 'violation_ratio': profile.avg_time / threshold})
        return {'timestamp': datetime.now(UTC).isoformat(), 'performance_summary': {'functions_profiled': len(self.function_profiles), 'total_function_calls': sum((p.call_count for p in self.function_profiles.values())), 'top_slow_functions': [{'name': p.function_name, 'avg_time_ms': p.avg_time, 'call_count': p.call_count, 'total_time_ms': p.total_time} for p in sorted_profiles[:5]]}, 'memory_usage': memory_trend, 'cache_performance': self.get_cache_stats(), 'performance_violations': violations, 'optimization_settings': {'gc_optimization_enabled': self._gc_optimization_enabled, 'query_optimization_enabled': self._query_optimization_enabled, 'caching_enabled': self._caching_enabled, 'memory_profiling_enabled': self.enable_memory_profiling}}
    ...
    def optimize_memory_usage(self):
        ...
        self.logger.info(f'Memory optimization completed. Memory freed: {memory_freed:.1f}MB ({before_snapshot.rss_mb:.1f}MB -> {after_snapshot.rss_mb:.1f}MB)')
        return memory_freed
...
def cached(cache_key_func: Callable | None=None, ttl_seconds: int=300):
    """Decorator for function result caching."""
    optimizer = get_performance_optimizer()
