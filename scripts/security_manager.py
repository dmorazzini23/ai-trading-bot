...
class RateLimiter:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self.lock:
            return {'active_clients': len(self.requests), 'blocked_clients': len(self.blocked_clients), 'total_requests': sum((len(req_queue) for req_queue in self.requests.values()))}
...
class AuditLogger:
    ...
    def __init__(self, log_file: str='logs/audit.log'):
        ...
        formatter = logging.Formatter('%(asctime)s|%(levelname)s|%(message)s', datefmt='%Y-%m-%dT%H:%M:%S.%fZ')
        handler.setFormatter(formatter)
        self.logger.addHandler(handler)
        self.logger.setLevel(logging.INFO)
        os.makedirs(os.path.dirname(log_file), exist_ok=True)

    def log_event(self, event_type: str, details: dict[str, Any], user_id: str | None=None, source_ip: str | None=None):
        """Log audit event."""
        audit_record = {'timestamp': datetime.now(UTC).isoformat(), 'event_type': event_type, 'user_id': user_id, 'source_ip': source_ip, 'details': details}
    ...
    def log_risk_violation(self, violation_type: str, details: dict[str, Any]):
        ...
        self.log_event(event_type='RISK_VIOLATION', details={'violation_type': violation_type, **details})

    def log_security_event(self, event: SecurityEvent):
        """Log security events."""
        self.log_event(event_type='SECURITY_EVENT', details={'security_event_type': event.event_type, 'severity': event.severity.value, 'description': event.description, 'action_taken': event.action_taken, **event.details}, source_ip=event.source_ip, user_id=event.user_id)
...
class AnomalyDetector:
    ...
    def __init__(self):
        ...
        self.thresholds = {'unusual_trade_size': 5.0, 'rapid_fire_orders': 10, 'abnormal_pnl_swing': 0.1, 'position_concentration': 0.3}
        self.anomalies_detected: list[dict[str, Any]] = []

    def analyze_trade_pattern(self, symbol: str, side: str, quantity: float, price: float) -> dict[str, Any] | None:
        """Analyze trade for anomalous patterns."""
        current_time = time.time()
        trade_record = {'timestamp': current_time, 'symbol': symbol, 'side': side, 'quantity': quantity, 'price': price, 'value': quantity * price}
        self.trade_history.append(trade_record)
        anomalies = []
        recent_trades = [t for t in self.trade_history if t['symbol'] == symbol and current_time - t['timestamp'] < 3600]
        if len(recent_trades) > 1:
            ...
            if quantity > avg_quantity * self.thresholds['unusual_trade_size']:
                anomalies.append({'type': 'unusual_trade_size', 'severity': SecurityLevel.MEDIUM, 'details': {'current_quantity': quantity, 'average_quantity': avg_quantity, 'ratio': quantity / avg_quantity}})
        recent_orders = [t for t in self.trade_history if current_time - t['timestamp'] < 60]
        if len(recent_orders) >= self.thresholds['rapid_fire_orders']:
            anomalies.append({'type': 'rapid_fire_orders', 'severity': SecurityLevel.HIGH, 'details': {'orders_count': len(recent_orders), 'time_window': 60, 'threshold': self.thresholds['rapid_fire_orders']}})
        if anomalies:
            anomaly_report = {'timestamp': datetime.now(UTC), 'trade': trade_record, 'anomalies': anomalies}
    ...
    def analyze_pnl_pattern(self, current_pnl: float) -> dict[str, Any] | None:
        ...
        if pnl_change > self.thresholds['abnormal_pnl_swing']:
            return {'type': 'abnormal_pnl_swing', 'severity': SecurityLevel.HIGH, 'details': {'current_pnl': current_pnl, 'previous_pnl': previous_pnl, 'change_percent': pnl_change * 100, 'threshold_percent': self.thresholds['abnormal_pnl_swing'] * 100}}
        return None

    def get_anomaly_report(self) -> dict[str, Any]:
        """Get comprehensive anomaly report."""
        recent_anomalies = [a for a in self.anomalies_detected if (datetime.now(UTC) - a['timestamp']).total_seconds() < 3600]
        return {'timestamp': datetime.now(UTC).isoformat(), 'total_anomalies': len(self.anomalies_detected), 'recent_anomalies': len(recent_anomalies), 'anomaly_types': list(set((anomaly['type'] for report in recent_anomalies for anomaly in report['anomalies']))), 'recent_reports': recent_anomalies[-10:]}
...
class SecurityManager:
    ...
    def __init__(self, enable_encryption: bool=True):
        ...
        self.security_settings = {'enable_rate_limiting': True, 'enable_encryption': enable_encryption, 'enable_audit_logging': True, 'enable_anomaly_detection': True, 'require_api_authentication': True, 'max_failed_attempts': 5, 'lockout_duration': 300}
        self.failed_attempts: dict[str, list[float]] = defaultdict(list)
        self.locked_clients: dict[str, float] = {}
        self.logger.info('Security manager initialized')

    def authenticate_api_request(self, api_key: str, signature: str, timestamp: str, body: str, client_ip: str=None) -> bool:
        ...
        try:
            if self.security_settings['enable_rate_limiting'] and client_ip:
                ...
                if not allowed:
                    self._log_security_event('RATE_LIMIT_EXCEEDED', SecurityLevel.MEDIUM, f'Rate limit exceeded for {client_ip}', {'client_ip': client_ip, 'remaining_requests': remaining})
                    return False
            if client_ip and client_ip in self.locked_clients:
                if time.time() < self.locked_clients[client_ip]:
                    self._log_security_event('LOCKED_CLIENT_ACCESS_ATTEMPT', SecurityLevel.HIGH, f'Locked client attempted access: {client_ip}', {'client_ip': client_ip})
                    return False
                else:
            ...
            try:
                ...
                return False
            expected_signature = hmac.new(expected_secret.encode(), f'{timestamp}{body}'.encode(), hashlib.sha256).hexdigest()
            if not hmac.compare_digest(signature, expected_signature):
                self._handle_failed_auth(client_ip, 'Invalid signature')
                return False
            self._log_security_event('SUCCESSFUL_AUTHENTICATION', SecurityLevel.LOW, f'Successful API authentication for {api_key}', {'api_key': api_key, 'client_ip': client_ip})
            return True
        except (ValueError, TypeError) as e:
    ...
    def _handle_failed_auth(self, client_ip: str | None, reason: str):
        ...
        if client_ip:
            ...
            self.failed_attempts[client_ip] = [t for t in self.failed_attempts[client_ip] if t > cutoff_time]
            if len(self.failed_attempts[client_ip]) >= self.security_settings['max_failed_attempts']:
                ...
                self._log_security_event('CLIENT_LOCKED', SecurityLevel.HIGH, f'Client locked due to repeated failed attempts: {client_ip}', {'client_ip': client_ip, 'failed_attempts': len(self.failed_attempts[client_ip]), 'lockout_duration': self.security_settings['lockout_duration']})
        self._log_security_event('FAILED_AUTHENTICATION', SecurityLevel.MEDIUM, f'Failed authentication: {reason}', {'client_ip': client_ip, 'reason': reason})

    def _log_security_event(self, event_type: str, severity: SecurityLevel, description: str, details: dict[str, Any]):
        """Log security event."""
        event = SecurityEvent(timestamp=datetime.now(UTC), event_type=event_type, severity=severity, source_ip=details.get('client_ip'), user_id=details.get('user_id'), description=description, details=details)
    ...
    def analyze_trade_for_anomalies(self, symbol: str, side: str, quantity: float, price: float) -> dict[str, Any] | None:
        """Analyze trade for security anomalies."""
        if not self.security_settings['enable_anomaly_detection']:
            return None
        anomaly_report = self.anomaly_detector.analyze_trade_pattern(symbol, side, quantity, price)
        if anomaly_report:
            self._log_security_event('TRADING_ANOMALY_DETECTED', SecurityLevel.MEDIUM, f'Anomalous trading pattern detected for {symbol}', {'symbol': symbol, 'anomaly_types': [a['type'] for a in anomaly_report['anomalies']], 'trade_details': anomaly_report['trade']})
        return anomaly_report

    def get_security_report(self) -> dict[str, Any]:
        """Generate comprehensive security report."""
        recent_events = [event for event in self.security_events if (datetime.now(UTC) - event.timestamp).total_seconds() < 3600]
        event_summary = defaultdict(lambda: defaultdict(int))
        for event in recent_events:
            event_summary[event.event_type][event.severity.value] += 1
        return {'timestamp': datetime.now(UTC).isoformat(), 'security_summary': {'total_events': len(self.security_events), 'recent_events': len(recent_events), 'active_rate_limits': self.rate_limiter.get_stats(), 'locked_clients': len(self.locked_clients), 'failed_auth_attempts': sum((len(attempts) for attempts in self.failed_attempts.values()))}, 'event_breakdown': dict(event_summary), 'anomaly_report': self.anomaly_detector.get_anomaly_report(), 'security_settings': self.security_settings}

    def run_security_audit(self) -> dict[str, Any]:
        """Run comprehensive security audit."""
        audit_results = {'timestamp': datetime.now(UTC).isoformat(), 'audit_checks': {}, 'recommendations': [], 'security_score': 0}
        score = 100
        ...
        if missing_vars:
            ...
            audit_results['recommendations'].append(f"Missing critical environment variables: {', '.join(missing_vars)}")
            score -= 20
        else:
        ...
        if self.security_settings['enable_audit_logging']:
            ...
            score -= 15
        recent_critical_events = [event for event in self.security_events if event.severity == SecurityLevel.CRITICAL and (datetime.now(UTC) - event.timestamp).total_seconds() < 86400]
        if recent_critical_events:
            audit_results['audit_checks']['recent_incidents'] = 'FAIL'
            audit_results['recommendations'].append(f'Address {len(recent_critical_events)} critical security incidents from last 24 hours')
            score -= 25
        else:
