...
def test_monitoring_api():
    ...
    try:
        ...
        performance_monitor.record_trade({'symbol': 'AAPL', 'side': 'buy', 'quantity': 100, 'price': 150.0, 'latency_ms': 45.2, 'success': True, 'pnl': 500.0, 'timestamp': '2024-01-01T10:00:00Z'})
...
def test_cost_aware_signals():
    ...
    try:
        ...
        test_cases = [('AAPL', 0.003, 'ACCEPT'), ('MSFT', 0.0008, 'REJECT'), ('GOOGL', -0.001, 'REJECT'), ('TSLA', 0.002, 'ACCEPT')]
        results = []
...
def test_performance_allocator():
    ...
    try:
        ...
        allocator = PerformanceBasedAllocator({'performance_window_days': 20, 'min_allocation_pct': 0.05, 'max_allocation_pct': 0.4, 'min_trades_threshold': 3})
        strategies = ['momentum', 'mean_reversion', 'breakout']
        trade_data = [('momentum', {'symbol': 'AAPL', 'entry_price': 100, 'exit_price': 105, 'pnl': 500, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('momentum', {'symbol': 'MSFT', 'entry_price': 200, 'exit_price': 208, 'pnl': 800, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('momentum', {'symbol': 'GOOGL', 'entry_price': 150, 'exit_price': 155, 'pnl': 500, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('mean_reversion', {'symbol': 'AAPL', 'entry_price': 100, 'exit_price': 98, 'pnl': -200, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('mean_reversion', {'symbol': 'TSLA', 'entry_price': 300, 'exit_price': 310, 'pnl': 1000, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('mean_reversion', {'symbol': 'AMD', 'entry_price': 80, 'exit_price': 82, 'pnl': 200, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('breakout', {'symbol': 'NVDA', 'entry_price': 400, 'exit_price': 390, 'pnl': -1000, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('breakout', {'symbol': 'META', 'entry_price': 250, 'exit_price': 245, 'pnl': -500, 'quantity': 100, 'timestamp': datetime.now(UTC)}), ('breakout', {'symbol': 'NFLX', 'entry_price': 350, 'exit_price': 348, 'pnl': -200, 'quantity': 100, 'timestamp': datetime.now(UTC)})]
...
def test_exception_handling():
    ...
    try:
        test_cases = [('AttributeError', 'Missing attribute test'), ('KeyError', 'Missing key test'), ('ValueError', 'Invalid value test'), ('TypeError', 'Type conversion test')]
...
def main():
    ...
    tests = [('Monitoring API Unification', test_monitoring_api), ('Cost-Aware Signal Pipeline', test_cost_aware_signals), ('Performance-Based Allocation', test_performance_allocator), ('Exception Handling', test_exception_handling)]
    results = []
