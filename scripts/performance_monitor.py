...
class ResourceMonitor:
    ...
    def _default_thresholds(self) -> dict:
        ...
        return {'memory_usage_percent': 80, 'swap_usage_mb': 500, 'cpu_usage_percent': 90, 'disk_usage_percent': 85, 'file_descriptors': 500, 'thread_count': 50, 'response_time_ms': 5000}

    def get_system_metrics(self) -> dict:
        """Collect comprehensive system metrics."""
        metrics = {'timestamp': datetime.now(UTC).isoformat(), 'collection_time_ms': 0}
    ...
    def _get_memory_metrics(self) -> dict:
        ...
        try:
            with open('/proc/meminfo') as f:
                ...
                memory_metrics = {'total_mb': total_mb, 'available_mb': available_mb, 'free_mb': free_mb, 'used_mb': total_mb - available_mb, 'usage_percent': (total_mb - available_mb) / total_mb * 100, 'cached_mb': cached_mb, 'buffers_mb': buffers_mb, 'swap_total_mb': swap_total_mb, 'swap_used_mb': swap_used_mb, 'swap_usage_percent': swap_used_mb / max(swap_total_mb, 1) * 100}
        except (ValueError, TypeError) as e:
    ...
    def _get_disk_metrics(self) -> dict:
        ...
        try:
            ...
            disk_metrics = {'total_gb': total / 1024 ** 3, 'used_gb': used / 1024 ** 3, 'free_gb': free / 1024 ** 3, 'usage_percent': used / total * 100}
            large_files = []
                ...
                for file in files[:20]:
                    try:
                        ...
                        if size > 50 * 1024 * 1024:
                            large_files.append({'path': filepath, 'size_mb': size / 1024 ** 2})
                    except OSError:
                        continue
    ...
    def _count_trading_bot_processes(self) -> int:
        ...
        trading_bot_count = 0
        try:
            result = subprocess.run(['ps', 'aux'], capture_output=True, text=True, timeout=10, check=True)
            ...
            for line in lines:
                ...
                trading_indicators = ['bot_engine.py', 'runner.py', 'run.py', 'trade_execution.py', 'ai-trading-bot', 'trading_bot', 'alpaca', 'predict.py', 'performance_monitor.py', 'retrain.py']
                for indicator in trading_indicators:
                    if indicator in command:
                        is_trading_bot = True
                        break
                temp_indicators = ['pgrep', 'ps aux', 'grep', '/tmp/', 'diagnostic', 'test_', 'pytest', 'coverage', 'pip install']
    ...
    def _count_python_processes_fallback(self) -> int:
        ...
        try:
            result = subprocess.run(['pgrep', '-f', 'python'], capture_output=True, text=True, timeout=5, check=True)
    ...
    def _get_network_metrics(self) -> dict:
        ...
        network_metrics = {}
        try:
            result = subprocess.run(['netstat', '-tn'], timeout=30, capture_output=True, text=True, check=True)
            if result.returncode == 0:
    ...
    def _get_python_metrics(self) -> dict:
        ...
        try:
            ...
            trading_modules = [name for name in sys.modules.keys() if any((keyword in name.lower() for keyword in ['trading', 'alpaca', 'ai_trading']))]
    ...
    def check_alert_conditions(self, metrics: dict) -> list[dict]:
        ...
        if 'memory' in metrics and isinstance(metrics['memory'], dict):
            ...
            if 'usage_percent' in mem and mem['usage_percent'] > self.alert_thresholds['memory_usage_percent']:
                alerts.append({'type': 'memory_high', 'severity': 'high', 'message': f"Memory usage at {mem['usage_percent']:.1f}%", 'value': mem['usage_percent'], 'threshold': self.alert_thresholds['memory_usage_percent']})
            if 'swap_used_mb' in mem and mem['swap_used_mb'] > self.alert_thresholds['swap_usage_mb']:
                alerts.append({'type': 'swap_high', 'severity': 'critical', 'message': f"Swap usage at {mem['swap_used_mb']:.1f}MB", 'value': mem['swap_used_mb'], 'threshold': self.alert_thresholds['swap_usage_mb']})
        if 'cpu' in metrics and isinstance(metrics['cpu'], dict):
            ...
            if 'usage_percent' in cpu and cpu['usage_percent'] > self.alert_thresholds['cpu_usage_percent']:
                alerts.append({'type': 'cpu_high', 'severity': 'medium', 'message': f"CPU usage at {cpu['usage_percent']:.1f}%", 'value': cpu['usage_percent'], 'threshold': self.alert_thresholds['cpu_usage_percent']})
        if 'process' in metrics and isinstance(metrics['process'], dict):
            ...
            if 'file_descriptors' in proc and proc['file_descriptors'] > self.alert_thresholds['file_descriptors']:
                alerts.append({'type': 'file_descriptors_high', 'severity': 'medium', 'message': f"File descriptors: {proc['file_descriptors']}", 'value': proc['file_descriptors'], 'threshold': self.alert_thresholds['file_descriptors']})
            if 'thread_count' in proc and proc['thread_count'] > self.alert_thresholds['thread_count']:
                alerts.append({'type': 'thread_count_high', 'severity': 'low', 'message': f"Thread count: {proc['thread_count']}", 'value': proc['thread_count'], 'threshold': self.alert_thresholds['thread_count']})
            if 'python_processes' in proc and proc['python_processes'] > 2:
                alerts.append({'type': 'multiple_trading_bot_processes', 'severity': 'medium', 'message': f"Multiple trading bot processes detected: {proc['python_processes']} (filtered for trading-specific processes only)", 'value': proc['python_processes'], 'threshold': 2})
        for alert in alerts:
    ...
    def start_monitoring(self):
        ...
        if self.monitoring_thread is None or not self.monitoring_thread.is_alive():
            ...
            self.monitoring_thread = threading.Thread(target=self.monitoring_loop, name='ResourceMonitor', daemon=True)
    ...
    def get_performance_report(self) -> dict:
        ...
        report = {'timestamp': datetime.now(UTC).isoformat(), 'current_metrics': current_metrics, 'trends': trends, 'recent_alerts': list(self.alerts_history)[-20:], 'monitoring_status': {'active': self.monitoring_thread and self.monitoring_thread.is_alive(), 'metrics_collected': len(self.metrics_history), 'alerts_generated': len(self.alerts_history)}, 'alert_thresholds': self.alert_thresholds}
        return report
...
class TradingPerformanceMonitor:
    ...
    def record_trade_execution_time(self, execution_time_ms: float, trade_type: str):
        ...
        self.trade_metrics.append({'timestamp': datetime.now(UTC).isoformat(), 'execution_time_ms': execution_time_ms, 'trade_type': trade_type})

    def record_api_response_time(self, endpoint: str, response_time_ms: float):
        """Record API response time."""
        self.api_response_times.append({'timestamp': datetime.now(UTC).isoformat(), 'endpoint': endpoint, 'response_time_ms': response_time_ms})

    def get_trading_performance_report(self) -> dict:
        """Get trading performance report."""
        report = {'trade_execution': {'total_trades': len(self.trade_metrics), 'average_execution_time_ms': 0, 'max_execution_time_ms': 0}, 'api_performance': {'total_calls': len(self.api_response_times), 'average_response_time_ms': 0, 'max_response_time_ms': 0}}
