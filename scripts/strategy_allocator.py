...
class StrategyAllocator:
    ...
    def _ensure_config_attributes(self):
        ...
        required_attrs = {'signal_confirmation_bars': 2, 'delta_threshold': 0.02, 'min_confidence': 0.6}
    ...
    def allocate(self, signals_by_strategy: dict[str, list[Any]]) -> list[Any]:
        ...
        logger.debug(f'Current config: signal_confirmation_bars={self.config.signal_confirmation_bars}, min_confidence={self.config.min_confidence}, delta_threshold={self.config.delta_threshold}')
        ...
        for strat, sigs in (confirmed_signals or {}).items():
            ...
            for s in sigs or []:
                ...
                if c >= threshold:
                    ...
                    logger.info('CONFIDENCE_DROP', extra={'strategy': strat, 'symbol': getattr(s, 'symbol', '?'), 'confidence': c, 'threshold': threshold})
            if kept:
                gated[strat] = kept
        ...
        for strategy, signals in signals_by_strategy.items():
            ...
            for s in signals:
                ...
                if len(self.signal_history[key]) >= confirmation_bars:
                    ...
                    logger.debug(f'Signal confirmation check: {s.symbol}, history={history_values}, avg_conf={avg_conf:.4f}, threshold={min_conf_threshold:.4f}, bars_required={confirmation_bars}, bars_available={len(history_values)}')
        ...
        for s in sorted(all_signals, key=lambda x: (x.symbol, -x.confidence)):
            ...
            if last_dir and last_dir != s.side:
                if s.side == 'sell' and self.hold_protect.get(s.symbol, 0) > 0:
                    ...
                    logger.info('HOLD_PROTECT_ACTIVE', extra={'symbol': s.symbol, 'remaining_cycles': remaining})
            ...
            if delta < self.config.delta_threshold and last_dir == s.side:
                logger.info('SIGNAL_SKIPPED_DELTA', extra={'symbol': s.symbol, 'delta': delta, 'threshold': self.config.delta_threshold})
                continue
    ...
    def _assign_portfolio_weights(self, signals: list[Any]) -> None:
        ...
        if buy_signals:
            ...
            if total_confidence > 0:
                ...
                equal_weight = min(max_total_exposure / len(buy_signals), 0.15)
                for signal in buy_signals:
                    signal.weight = equal_weight
                    logger.debug(f'Assigned equal weight {signal.weight:.3f} to {signal.symbol}')
        for signal in sell_signals:
            max_exit_weight = min(0.25, max_total_exposure / max(len(sell_signals), 1))
