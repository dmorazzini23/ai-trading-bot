...
def with_error_handling(error_type: type[Exception]=Exception, default_return: Any=None, log_level: str='error', reraise: bool=False):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                error_details = {'function': func.__name__, 'error_type': type(e).__name__, 'error_message': str(e), 'timestamp': datetime.now(UTC).isoformat(), 'traceback': traceback.format_exc()}
                if log_level == 'error':
...
def graceful_shutdown(reason: str='Unknown'):
    ...
    """
    logger.critical('TRADING_BOT_SHUTDOWN', extra={'reason': reason, 'timestamp': datetime.now(UTC).isoformat()})
    try:
