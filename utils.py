...
# Lazy-load optional Alpaca SDK to avoid import-time failures
REST = None
def _get_alpaca_rest():
    """Import and cache alpaca_trade_api.REST on first use."""
    global REST
    if REST is None:
        try:
            from alpaca_trade_api.rest import REST as _REST  # type: ignore
            REST = _REST
        except Exception as e:  # pragma: no cover - optional dependency
