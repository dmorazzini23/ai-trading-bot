...
PATS = {'MODULE_CONST': re.compile('^\\s*[A-Z0-9_]+\\s*=\\s*get_settings\\(\\)\\.\\w+', re.M), 'MODULE_CFG': re.compile('^\\s*CFG\\s*=\\s*get_settings\\(\\)', re.M), 'DIRECT_ATTR': re.compile('\\bget_settings\\(\\)\\.\\w+'), 'BARE_CALL': re.compile('\\bget_settings\\(')}
