...
def fix_metrics_logger(text: str) -> str:
    text = re.sub('(^|\\n)\\s*import\\s+metrics_logger\\s*(\\n|$)', '\\1from ai_trading.telemetry import metrics_logger\\2', text)
    text = re.sub('(^|\\n)\\s*from\\s+metrics_logger\\s+import\\s+', '\\1from ai_trading.telemetry.metrics_logger import ', text)
    return text

def strip_mock_classes(text: str) -> str:
    rx_bases = re.compile('(?ms)^(\\s*)class\\s+Mock[A-Za-z0-9_]+\\s*\\([^)]*\\)\\s*:\\s*\\n(?:\\1    |\\t).+?(?=^\\1\\S|\\Z)')
    rx_plain = re.compile('(?ms)^(\\s*)class\\s+Mock[A-Za-z0-9_]+\\s*:\\s*\\n(?:\\1    |\\t).+?(?=^\\1\\S|\\Z)')
    text = rx_bases.sub('', text)
    text = rx_plain.sub('', text)
    return text

def strip_dunder_getattr(text: str) -> str:
    rx = re.compile('(?ms)^def\\s+__getattr__\\s*\\([^)]*\\)\\s*:\\s*\\n(?:\\s{4}|\\t).+?(?=^\\S|\\Z)')
    return rx.sub('', text)
...
def ensure_helpers(text: str) -> str:
    ...
    return ''.join(lines)
UPPER = re.compile('\\bconfig\\.([A-Z][A-Z0-9_]+)\\b')
...
def migrate_config_callsites(text: str):
    ...
    if re.search('^\\s*S\\s*=\\s*get_settings\\(\\)\\s*$', new, flags=re.MULTILINE) is None:
...
def ensure_settings_fields(settings_py: Path, needed: set):
    ...
    if 'BaseSettings' not in txt:
        ...
        txt = 'from pydantic import BaseSettings, Field\n\nclass Settings(BaseSettings):\n' + fields + "\ndef get_settings() -> 'Settings':\n    global _S\n    try:\n        return _S\n    except NameError:\n        _S = Settings()  # type: ignore\n        return _S\n"
    else:
        existing = set(re.findall('^\\s*([a-z_][a-z0-9_]*)\\s*:\\s*', txt, flags=re.MULTILINE))
        ...
        if additions and 'class Settings' in txt:
            txt = re.sub('(class\\s+Settings\\s*\\(.*?\\)\\s*:\\s*\\n)', '\\1' + ''.join(additions), txt, count=1, flags=re.DOTALL)
