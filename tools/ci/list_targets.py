...
patterns = {
    ...
    "requests_no_timeout": re.compile(r"requests\.(get|post|put|delete|patch)\s*\((?![^)]*timeout\s*=)"),
    "subprocess_unsafe": re.compile(r"subprocess\.(run|Popen|call|check_call|check_output)\s*\((?![^)]*timeout\s*=)"),
    ...
    "empty_except": re.compile(r"^\s*except\s+(?:[A-Za-z_][\w\.]*\s+as\s+\w+|[A-Za-z_][\w\.]*)?\s*:\s*(?:\n\s+(pass|return\s*(?:None)?|#.*\n))", re.MULTILINE),
    "mutable_default": re.compile(r"def\s+\w+\s*\([^)]*(\[\]|\{\}|set\(\))", re.DOTALL),
    "time_sleep_in_async": re.compile(r"^\s*async\s+def[\s\S]*?^\s*time\.sleep\(", re.MULTILINE),
}
rows = []
