...
PATTERNS = {'import_guard_block': re.compile('(?ms)try:\\s*(?:\\n\\s*(?:from\\s+[.\\w]+\\s+import\\s+[^\\n]+|import\\s+[^\\n]+))+?\\n\\s*except\\s+ImportError\\s*:\\s*'), 'mock_class': re.compile('(^|\\n)\\s*class\\s+Mock[A-Za-z0-9_]+\\s*:', re.MULTILINE), 'config_dunder_getattr': re.compile('def\\s+__getattr__\\s*\\('), 'uppercase_alias_property': re.compile('@property\\s+def\\s+[A-Z0-9_]+\\s*\\('), 'raw_exec': re.compile('\\bexec\\s*\\('), 'raw_eval': re.compile('(^|[^.])\\beval\\s*\\(', re.MULTILINE), 'root_metrics_logger': re.compile('\\b(from|import)\\s+metrics_logger\\b')}
