    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.Call:
        ...
        if m.matches(updated_node, m.Call(func=m.Attribute(value=m.Name("subprocess"), attr=m.OneOf(m.Name("run"), m.Name("Popen"), m.Name("call"), m.Name("check_call"), m.Name("check_output"))))):
            ...
            if m.matches(updated_node.func, m.Attribute(attr=m.Name("run"))):
                if "check" not in kws: args.append(cst.Arg(keyword=cst.Name("check"), value=cst.Name("True")))
                if "capture_output" not in kws: args.append(cst.Arg(keyword=cst.Name("capture_output"), value=cst.Name("True")))
                if "text" not in kws: args.append(cst.Arg(keyword=cst.Name("text"), value=cst.Name("True")))
