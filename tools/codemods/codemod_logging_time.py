...
def replace_print_with_logging(content: str, filepath: str) -> str:
    ...
    has_logging_import = bool(re.search(r'^(import logging|from .*logging.* import)', content, re.MULTILINE))
    ...
    content = re.sub(r'\bprint\s*\(\s*(["\'][^"\']*["\'])\s*\)', r'logging.info(\1)', content)
    
    # print(f"...") -> logger.info(f"...")
    content = re.sub(r'\bprint\s*\(\s*(f["\'][^"\']*["\'])\s*\)', r'logging.info(\1)', content)
    
    # print(variable) -> logger.info(str(variable))
    content = re.sub(r'\bprint\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)', r'logging.info(str(\1))', content)
...
def replace_datetime_now(content: str) -> str:
    ...
    if 'datetime.now(datetime.timezone.utc)' in content:
        if not re.search(r'^(import datetime|from datetime import)', content, re.MULTILINE):
            # Add datetime import
            import_lines = []
            other_lines = []
            in_imports = True
