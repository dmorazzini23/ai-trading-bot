...
# AI-AGENT-REF: narrow bare exceptions to reduce BLE001 noise
    ...
    def leave_ExceptHandler(
        ...
        if original.type is None or m.matches(original.type, m.Name("Exception")):
            ...
            if self.has_datetime:
                new_type = cst.Tuple(
                    [
                        cst.Element(cst.Name("ValueError")),
                        cst.Element(cst.Name("TypeError")),
                    ]
                )
            ...
            if self.has_requests:
                new_type = cst.Tuple(
                    [
                        cst.Element(
                            cst.Attribute(cst.Name("requests"), cst.Name("RequestException"))
                        ),
                        cst.Element(cst.Name("TimeoutError")),
                    ]
                )
                return updated.with_changes(type=new_type)
