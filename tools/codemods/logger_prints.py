...
# AI-AGENT-REF: replace print statements with centralized logger
        ...
        if node.module and get_full_name_for_node(node.module) == "ai_trading.logging":
            for alias in node.names:
                if (
                    ...
                    and alias.asname
                    and alias.evaluated_name == "logger"
                    and alias.asname.name.value == "log"
                ):
    ...
    def leave_Call(self, original: cst.Call, updated: cst.Call) -> cst.BaseExpression:
        if m.matches(original.func, m.Name("print")):
            ...
            return updated.with_changes(
                func=cst.Attribute(value=cst.Name("log"), attr=cst.Name("info")),
                args=args,
            )
        return updated
