...
KNOWN_GETTERS = {'rebalance_interval_min': 'get_rebalance_interval_min', 'news_api_key': 'get_news_api_key', 'disaster_dd_limit': 'get_disaster_dd_limit', 'capital_cap': 'get_capital_cap', 'dollar_risk_limit': 'get_dollar_risk_limit', 'buy_threshold': 'get_buy_threshold', 'conf_threshold': 'get_conf_threshold', 'trade_cooldown_min': 'get_trade_cooldown_min', 'portfolio_drift_threshold': 'get_portfolio_drift_threshold', 'max_drawdown_threshold': 'get_max_drawdown_threshold', 'daily_loss_limit': 'get_daily_loss_limit', 'max_portfolio_positions': 'get_max_portfolio_positions'}
RE_MODULE_CONST = re.compile('^(\\s*)([A-Z0-9_]+)\\s*=\\s*get_settings\\(\\)\\.(\\w+)\\s*$', re.M)
RE_MODULE_CFG = re.compile('^(\\s*)CFG\\s*=\\s*get_settings\\(\\)\\s*$', re.M)
RE_DIRECT_ATTR = re.compile('\\bget_settings\\(\\)\\.(\\w+)')
...
def add_function_local_runtime_import(lines):
    ...
    if 'get_runtime_settings()' not in j:
        return False
    func = re.compile('^(\\s*)def\\s+\\w+\\s*\\(.*?\\):', re.M)
    pos = 0
    while True:
        m = func.search(j, pos)
        if not m:
...
def patch_text(text):
    original = text
    text = RE_MODULE_CFG.sub('\\1# REMOVED: module-scope CFG = get_settings()', text)
    const_map = {}
    ...
    for const, repl in const_map.items():
        text = re.sub(f'\\b{re.escape(const)}\\b', repl, text)
...
def main():
    ...
    for py in ROOT.rglob('*.py'):
        if any((part in EXCLUDE for part in py.parts)):
            continue
        if py.parts[0] not in {'ai_trading', 'tests'}:
            continue
        if py.name == 'settings.py' and py.parts[-2] == 'ai_trading':
            continue
        try:
            if patch_file(py):
                changed += 1
