...
> - Real `requests`, `torch`, and other libraries are required. The old `third_party_stubs/` directory has been removed, but the repo still carries a test-only `sitecustomize.py` hook for deterministic test behavior. Tests skip automatically when optional packages are missing.
