...
def test_execute_order_returns_execution_result(monkeypatch):
    ...
    assert result_obj.has_fill
    assert result_obj.filled_quantity == 5
    assert result_obj.fill_ratio == pytest.approx(1.0)
...
def test_async_fill_triggers_risk_engine(monkeypatch):
    ...
    assert isinstance(result, ExecutionResult)
    assert cast(Any, result).filled_quantity == 0
    assert risk.fills == []

    order = engine.order_manager.orders[str(result)]
    order.add_fill(4, Money(100.0))
    ...
    order.add_fill(remaining, Money(100.0))
    engine._handle_execution_event(order, "completed")
...
def test_order_add_fill_tracks_partial_then_full():
    ...
    order.add_fill(2, Money(101.0))
    assert order.status == OrderStatus.PARTIALLY_FILLED
    assert order.filled_quantity == 2

    order.add_fill(3, Money(102.0))
