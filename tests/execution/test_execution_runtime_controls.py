...
def _engine_stub() -> Any:
    ...
    engine._symbol_submit_no_result_last_fingerprint = {}
    engine._submit_no_result_fingerprint_failure_mono = {}
    engine._submit_no_result_fingerprint_backoff_until = {}
    engine._submit_no_result_fingerprint_last_cluster_count = {}
    engine._submit_no_result_recent_events = deque(maxlen=512)
    engine._submit_no_result_broker_health_backoff_until_mono = 0.0
    engine._submit_no_result_broker_health_context = {}
    engine._submit_no_result_broker_health_alert_last_mono = 0.0
...
def _exploration_contract(**overrides: Any) -> dict[str, Any]:
...
def test_broker_sync_unknown_blocks_openings_but_allows_reductions(monkeypatch):
    engine = _engine_stub()
    engine.ctx = SimpleNamespace(state={"service_phase": "active"})
    engine._broker_open_orders_unknown = True
    engine._broker_positions_unknown = False
    monkeypatch.setenv("AI_TRADING_EXECUTION_PHASE_GATE_ENABLED", "1")
    ...
    assert opening_detail == "broker_sync_unknown components=open_orders"
...
def test_runtime_gonogo_soft_derisks_in_paper_when_enabled(monkeypatch, tmp_path):
    engine = _engine_stub()
    engine.execution_mode = "paper"
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            "gate_passed": False,
            "failed_checks": ["win_rate", "slippage_drag_bps"],
            "thresholds": {
                "min_win_rate": 0.46,
                "max_slippage_drag_bps": 9.0,
            },
            "observed": {
                "win_rate": 0.435,
                "slippage_drag_bps": 11.5,
            },
        },
    )
    ...
    assert context["execution_capture_guard"]["active"] is True
    assert context["execution_capture_guard"]["order_qty_scale"] == pytest.approx(0.3)
...
def test_runtime_gonogo_soft_derisk_does_not_bypass_integrity_failures(monkeypatch, tmp_path):
    ...
    assert allowed is False
    assert context["gate_passed"] is False
    assert context.get("soft_enforced") is not True
    assert "open_position_reconciliation_consistent" in context["failed_checks"]
...
def test_symbol_intraday_slippage_budget_capture_ratio_requires_min_notional(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    engine = _engine_stub()
                ...
                json.dumps(
                    {
                        ...
                        "entry_time": f"{today}T14:00:00+00:00",
                    }
                ),
                json.dumps(
                    {
    ...
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_CAPTURE_RATIO_GUARD_ENABLED", "1")
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_CAPTURE_RATIO_MIN_FILLS", "2")
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_CAPTURE_RATIO_MIN_NOTIONAL", "5000")
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_CAPTURE_RATIO_SOFT_FLOOR", "0.20")
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_CAPTURE_RATIO_HARD_FLOOR", "0.10")
    monkeypatch.setenv("AI_TRADING_EXECUTION_SYMBOL_INTRADAY_SLIPPAGE_BUDGET_TZ", "UTC")
    ...
    assert allowed is True
    assert context["capture_ratio_guard_enabled"] is True
    assert context["capture_ratio_min_notional"] == pytest.approx(5000.0)
    assert context["today_symbol_notional"] == pytest.approx(2050.0)
    assert context["capture_ratio_state"] == "insufficient_samples"
...
def test_submit_failure_records_execution_quality_event(monkeypatch):
    ...
    assert payload["submit_no_result_failure"] is False
...
def test_execution_quality_event_enriches_phase_trace_and_error_code(monkeypatch):
    ...
    assert payload["submit_no_result_failure"] is True
...
def test_submit_no_result_fingerprint_backoff_triggers_without_symbol_backoff(monkeypatch):
    ...
    monkeypatch.setenv("AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_SYMBOL_BACKOFF_ENABLED", "1")
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_SYMBOL_BACKOFF_TRIGGER_COUNT", "99"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_FINGERPRINT_BACKOFF_TRIGGER_COUNT", "2"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_FINGERPRINT_BACKOFF_BASE_COOLDOWN_SEC", "90"
    )
    now = {"value": 40_000.0}
    monkeypatch.setattr(lt, "monotonic_time", lambda: float(now["value"]))

    for idx in range(2):
        now["value"] = 40_000.0 + float(idx)
        engine._record_submit_failure(
            symbol="AAPL",
            side="buy",
            reason="submit_no_result",
            client_order_id=f"cid-fp-{idx}",
            order_type="limit",
            status_code=504,
            detail="no broker response",
        )
    ...
    assert context["cluster_count"] >= 2


def test_submit_no_result_broker_health_backoff_blocks_new_openings(monkeypatch):
    engine = _engine_stub()
    monkeypatch.setenv("AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_SYMBOL_BACKOFF_ENABLED", "1")
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_SYMBOL_BACKOFF_TRIGGER_COUNT", "99"
    ...
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_BROKER_HEALTH_GUARD_ENABLED", "1"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_BROKER_HEALTH_TRIGGER_COUNT", "2"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_BROKER_HEALTH_MIN_SYMBOLS", "2"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_BROKER_HEALTH_COOLDOWN_SEC", "60"
    )
    now = {"value": 50_000.0}
    monkeypatch.setattr(lt, "monotonic_time", lambda: float(now["value"]))
    ...
    engine._record_submit_failure(
        ...
        side="buy",
        reason="submit_no_result",
        client_order_id="cid-bh-2",
        order_type="limit",
        status_code=504,
        detail="no broker response",
    )

    allowed, context = engine._submit_no_result_broker_health_allows_opening(
        symbol="NVDA"
    )
    assert allowed is False
    assert context["reason"] == "submit_no_result_broker_health_backoff"

    now["value"] += 120.0
    allowed_after, context_after = engine._submit_no_result_broker_health_allows_opening(
        symbol="NVDA"
    )
    assert allowed_after is True
    assert context_after["reason"] == "backoff_inactive"
...
def test_adaptive_submit_no_result_recovery_wait_scales_with_backoff_pressure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = _engine_stub()
    now = {"value": 70_000.0}
    monkeypatch.setattr(lt, "monotonic_time", lambda: float(now["value"]))
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SUBMIT_NO_RESULT_RECOVERY_WAIT_MAX_SECONDS", "5.0"
    )
    monkeypatch.setenv(
    ...
    engine._symbol_submit_no_result_last_fingerprint["AAPL"] = "504:no_broker_response"
    engine._submit_no_result_fingerprint_backoff_until[
        "504:no_broker_response"
    ] = float(now["value"]) + 120.0
    engine._submit_no_result_broker_health_backoff_until_mono = (
        float(now["value"]) + 90.0
    )

    adapted_wait, context = engine._adaptive_submit_no_result_recovery_wait_seconds(
        base_wait_seconds=2.0,
        symbol="AAPL",
    )

    assert adapted_wait == pytest.approx(5.0)
    assert context["applied"] is True
    assert context["multiplier"] == pytest.approx(2.5)
    assert context["error_fingerprint"] == "504:no_broker_response"
    assert context["fingerprint_backoff_remaining_s"] > 0.0
    assert context["broker_health_backoff_remaining_s"] > 0.0


def test_sample_formation_override_applies_when_execution_quality_starved(monkeypatch):
    engine = _engine_stub()
    ...
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SAMPLE_FORMATION_SYMBOLS", "AAPL,MSFT"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SAMPLE_FORMATION_MAX_EVENTS_PER_WINDOW", "2"
    )
    monkeypatch.setenv(
        "AI_TRADING_EXECUTION_SAMPLE_FORMATION_MAX_EVENTS_PER_SYMBOL_PER_WINDOW", "1"
    )
    monkeypatch.setattr(
        engine,
    ...
    assert action_first == "ioc"
    assert context_first["applied"] is True

    now["value"] += 1.0
    action_second, context_second = (
        engine._maybe_override_passive_fill_low_prob_action_for_sample_formation(
            symbol="AAPL",
            action_token="skip",
...
def test_execution_kpi_snapshot_tracks_precheck_detail_reason(caplog) -> None:
    engine = _engine_stub()
    engine._runtime_exec_event_persistence_enabled = lambda: False
    engine._skip_submit(
        symbol="AAPL",
        side="buy",
        reason="pre_execution_order_checks_failed",
        order_type="limit",
        detail="runtime_gonogo_gate",
        context={"reason": "runtime_gonogo_gate"},
    )
    engine._list_open_orders_snapshot = lambda: []
    ...
    record = next(rec for rec in caplog.records if rec.message == "EXECUTION_KPI_SNAPSHOT")
    assert record.precheck_detail_counts == {"runtime_gonogo_gate": 1}


def test_execution_kpi_snapshot_includes_submit_no_result_fingerprint_and_rolling_sum(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = _engine_stub()
    monkeypatch.setenv("AI_TRADING_EXECUTION_REALIZED_BPS_ROLLING_WINDOW_SEC", "1800")
    now = {"value": 10_000.0}
    monkeypatch.setattr(lt, "monotonic_time", lambda: float(now["value"]))
    engine._record_cycle_order_outcome(
        symbol="AAPL",
    ...
    engine._cycle_order_outcomes = [
        {
            ...
            "submit_no_result_error_fingerprint": "504:no_broker_response",
            "symbol": "AAPL",
            "duration_s": 0.1,
        },
        {
    ...
    context = getattr(engine, "_execution_skip_pressure_context", {})
    assert context["submit_no_result_failure_count"] == 1
    assert context["submit_no_result_error_fingerprint_counts"] == {
        "504:no_broker_response": 1
    }
    assert context["submit_no_result_symbol_fingerprint_counts"] == {
        "AAPL|504:no_broker_response": 1
    }
    assert context["sum_realized_bps_30m"] == pytest.approx(3.5)
    assert context["sum_realized_bps_30m_samples"] == 1
...
def test_metrics_improvement_control_allows_budgeted_paper_side_recovery_sample(
    ...
    payload = {
        ...
        "exploration_contract": _exploration_contract(),
    }
    monkeypatch.setattr(engine, "_load_metrics_improvement_control", lambda: payload)
    ...
    assert order["_metrics_improvement_exploration_pending"]["symbol"] == "MSFT"
...
def test_metrics_improvement_control_caps_paper_side_recovery_budget(monkeypatch) -> None:
    ...
    assert second_context["exploration_contract"]["reason"] == "applied"
...
def test_metrics_improvement_control_treats_memory_budget_as_advisory(monkeypatch) -> None:
    engine = _engine_stub()
    engine.execution_mode = "paper"
    payload = {
        "generated_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
    ...
    assert record_context["reason"] == "recorded"
    assert second_allowed is True
    assert second_context["action"] == "explore"
    assert second_context["exploration_contract"]["reason"] == "applied"
...
def test_metrics_improvement_control_relaxes_exploration_edge_in_paper(monkeypatch) -> None:
    ...
    order = {
        ...
        "quote_age_ms": 100.0,
        "spread_bps": 2.0,
        "correlation_id": "decision-quote-order-fill-exit-1",
    }
    allowed, context = engine._metrics_improvement_control_allows_opening(order=order)
    ...
    assert order["paper_sampling_policy"] == "passive_only"
    assert order["correlation_id"] == "decision-quote-order-fill-exit-1"


@pytest.mark.parametrize(
    ("order_update", "reason"),
    [
        ({"expected_edge_bps": -0.01}, "metrics_control_exploration_expected_edge_floor"),
        ({"expected_edge_bps": float("nan")}, "metrics_control_exploration_expected_edge_missing"),
        ({"quote_age_ms": 1_751.0}, "metrics_control_exploration_quote_age_block"),
        ({"quote_age_ms": None}, "metrics_control_exploration_quote_age_block"),
        ({"spread_bps": 12.51}, "metrics_control_exploration_spread_block"),
        ({"spread_bps": None}, "metrics_control_exploration_spread_block"),
    ],
)
def test_metrics_improvement_exploration_contract_blocks_invalid_probe(
    monkeypatch: pytest.MonkeyPatch,
...
def test_metrics_improvement_exploration_contract_blocks_non_governed_symbol(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = _engine_stub()
    engine.execution_mode = "paper"
    payload = {
        ...
        "authority_increase_allowed": False,
        "by_symbol": {
            "GOOGL": {"action": "explore", "qty_scale": 0.5, "required_edge_bps": 0.0}
        },
        "exploration_budget": {},
        "exploration_contract": _exploration_contract(
            governed_universe=["AAPL", "AMZN", "MSFT", "GOOGL"],
            actionable_symbols=["AAPL", "AMZN", "MSFT", "GOOGL"],
        ),
    }
    monkeypatch.setattr(engine, "_load_metrics_improvement_control", lambda: payload)
    ...
    assert allowed is False
    assert context["reason"] == "metrics_control_exploration_symbol_block"
...
def test_metrics_improvement_control_blocks_live_exploration_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = _engine_stub()
    engine.execution_mode = "live"
    ...
    assert allowed is False
    assert context["reason"] == "metrics_control_exploration_paper_only"
