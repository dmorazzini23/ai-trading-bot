...
def test_balanced_mode_max_position_alias_rejected(monkeypatch):
...
def test_mode_precedence_env_wins(monkeypatch):
    monkeypatch.setenv("MAX_DRAWDOWN_THRESHOLD", "0.2")
    monkeypatch.setenv("AI_TRADING_TRADING_MODE", "aggressive")
    monkeypatch.setenv("AI_TRADING_TRADING_MODE_PRECEDENCE", "env_wins")
...
def test_mode_effective_snapshot_has_sources(monkeypatch):
    monkeypatch.setenv("MAX_DRAWDOWN_THRESHOLD", "0.2")
    monkeypatch.delenv("AI_TRADING_TRADING_MODE_PRECEDENCE", raising=False)
    ...
    cfg = TradingConfig.from_env(
        {
            ...
            "AI_TRADING_KELLY_FRACTION": "0.44",
            "MAX_DRAWDOWN_THRESHOLD": "0.2",
        }
    )
    ...
    assert snapshot["managed_fields"]["kelly_fraction"]["source"] == "AI_TRADING_KELLY_FRACTION"
