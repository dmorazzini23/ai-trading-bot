...
def test_retry_fixed_and_linear_modes():
...
def test_retry_returns_fallback_without_retryerror():
    calls = {"fail": 0}

    @retry(retries=2, delay=0.001, fallback="ok", exceptions=(RuntimeError,))
    def always_fail():
        calls["fail"] += 1
        raise RuntimeError
