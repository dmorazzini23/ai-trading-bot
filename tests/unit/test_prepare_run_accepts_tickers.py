...
def test_prepare_run_preserves_canonical_universe(monkeypatch):
    runtime = types.SimpleNamespace(params={}, cfg=None)
    state = eng.BotState()
    ...
    _, _, symbols = eng._prepare_run(runtime, state, universe)

    assert symbols == ["AAPL", "MSFT"]
    assert runtime.tickers == ["AAPL", "MSFT"]
    assert runtime.universe_tickers == universe
