...
def _imported_alpaca_modules(mod: str) -> list[str]:
    code = (
        ...
        f"import {mod}\n"
        "print('ALPACA_MODULES=' + json.dumps([m for m in sys.modules if m.startswith('alpaca')]))\n"
    )
    env = os.environ.copy()
    ...
    for line in reversed(result.stdout.strip().splitlines()):
        if line.startswith("ALPACA_MODULES="):
            return cast(list[str], json.loads(line.removeprefix("ALPACA_MODULES=")))
