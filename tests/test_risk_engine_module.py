...
def test_position_size_zero_raw_qty_defaults_to_min(caplog):
    ...
    assert qty == 0
    assert any("returning 0" in rec.message for rec in caplog.records)
...
def test_position_size_floor_is_capped_by_cash_and_exposure():
    ...
    assert eng.position_size(sig, cash=50, price=10) == 0

    eng.asset_limits["equity"] = 0.05
    eng.strategy_limits["s"] = 0.05
    eng.exposure["equity"] = 0.04
    assert eng.position_size(sig, cash=1_000, price=10) == 0
