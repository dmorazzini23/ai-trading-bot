...
class TradeExecutionMonitor:
    ...
    def __init__(self):
        self.execution_stats = {
            'buy_orders_attempted': 0,
            'buy_orders_executed': 0,
            'buy_orders_skipped_insufficient_funds': 0,
            'buy_orders_skipped_other': 0,
            'sell_orders_attempted': 0,
            'sell_orders_executed': 0,
            'sell_orders_skipped_no_position': 0,
            'sell_orders_skipped_other': 0,
            'api_errors': 0,
    ...
    def log_buy_attempt(self, symbol: str, qty: int, reason: str = None):
        ...
        logger.info("BUY_ORDER_ATTEMPT", extra={
            "symbol": symbol,
            "qty": qty,
    ...
    def log_buy_executed(self, symbol: str, qty: int):
        ...
        logger.info("BUY_ORDER_EXECUTED", extra={
            "symbol": symbol,
            "qty": qty
        })
    ...
    def log_buy_skipped(self, symbol: str, qty: int, reason: str):
        ...
        logger.warning("BUY_ORDER_SKIPPED", extra={
            "symbol": symbol,
            "qty": qty,
            "reason": reason
        })
    ...
    def log_sell_attempt(self, symbol: str, qty: int):
        ...
        logger.info("SELL_ORDER_ATTEMPT", extra={
            "symbol": symbol,
            "qty": qty
        })
    ...
    def log_sell_executed(self, symbol: str, qty: int):
        ...
        logger.info("SELL_ORDER_EXECUTED", extra={
            "symbol": symbol,
            "qty": qty
        })
    ...
    def log_sell_skipped(self, symbol: str, qty: int, reason: str):
        ...
        logger.warning("SELL_ORDER_SKIPPED", extra={
            "symbol": symbol,
            "qty": qty,
            "reason": reason
        })
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'period_start': self.execution_stats['last_reset'].isoformat(),
            'current_time': datetime.now(timezone.utc).isoformat(),
            'buy_execution_rate': (
                self.execution_stats['buy_orders_executed'] / total_buy_attempts * 100
                if total_buy_attempts > 0 else 0
            ),
            'sell_execution_rate': (
                self.execution_stats['sell_orders_executed'] / total_sell_attempts * 100
                if total_sell_attempts > 0 else 0
            ),
    ...
    def log_periodic_summary(self):
        ...
        # Alert if execution rates are too low
        if summary['buy_execution_rate'] < 50 and self.execution_stats['buy_orders_attempted'] >= 5:
            logger.critical("LOW_BUY_EXECUTION_RATE", extra={
                "rate": summary['buy_execution_rate'],
                "attempted": self.execution_stats['buy_orders_attempted'],
                "executed": self.execution_stats['buy_orders_executed']
            })
            
    def reset_stats(self):
        """Reset execution statistics."""
        self.execution_stats = {k: 0 if isinstance(v, int) else datetime.now(timezone.utc) 
                               for k, v in self.execution_stats.items()}
        self.execution_stats['last_reset'] = datetime.now(timezone.utc)
