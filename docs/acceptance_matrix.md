...
| AC-25 | Phase 2 execution-edge gate blocks deploy when performance regresses and emits calibration diagnostics | `ai_trading/analytics/execution_report.py`, `ci/scripts/institutional_gates.sh`, `ai_trading/tools/update_phase2_execution_baseline.py` | `tests/test_execution_report_daily_rollup.py`, `tests/unit/test_update_phase2_execution_baseline_tool.py` | `AI_TRADING_INSTITUTIONAL_REQUIRE_PHASE2_GATE=1` with fresh execution report and baseline calibration block |
| AC-26 | Phase 2 adaptive routing remains guarded and opt-in until calibrated from paper/live TCA evidence | `ai_trading/execution/order_policy.py`, `ai_trading/execution/live_trading.py`, `ai_trading/analytics/execution_report.py` | `tests/execution/test_order_policy.py`, `tests/execution/test_execution_runtime_controls.py`, `tests/test_execution_report_daily_rollup.py` | Keep `AI_TRADING_PHASE2_EXECUTION_EDGE_ROUTING_ENABLED=0` unless calibration diagnostics are sufficient and release sign-off approves threshold changes |
| AC-27 | Pre-open operator drill surfaces flat-start and OMS blockers before opening trades | `scripts/pre_open_acceptance_gate.py`, `ai_trading/execution/live_trading.py`, `ai_trading/health_payload.py` | `tests/scripts/test_pre_open_acceptance_gate.py`, `tests/execution/test_execution_runtime_controls.py`, `tests/test_health_endpoints.py` | Run `./venv/bin/python scripts/pre_open_acceptance_gate.py --json`; require clean state before enabling `AI_TRADING_EXECUTION_PREOPEN_REQUIRE_FLAT_START=1` |
| AC-28 | Live KPI breach handling persists consecutive breaches and keeps rollback explicitly gated | `ai_trading/governance/promotion.py`, `ai_trading/main.py` | `tests/test_model_promotion_institutional.py`, `tests/main/test_runtime_governance_hooks.py` | Verify `AI_TRADING_PROMOTION_LIVE_KPI_BREACH_CONSECUTIVE_REQUIRED`; keep `AI_TRADING_PROMOTION_AUTO_ROLLBACK_ON_CONTROL_BAND=0` until rollback drill passes |
| AC-29 | Strict OMS readiness gates can fail readiness while observe-only mode remains visible | `ai_trading/health_payload.py`, `scripts/pre_open_acceptance_gate.py` | `tests/test_health_endpoints.py`, `tests/scripts/test_pre_open_acceptance_gate.py` | Paper canary validates `AI_TRADING_HEALTH_REQUIRE_OMS_INVARIANTS=1` and `AI_TRADING_HEALTH_REQUIRE_OMS_LIFECYCLE_PARITY=1` against `/healthz` readiness gates |
...
8. Pre-open acceptance gate passes with no flat-start or required OMS blockers.
9. Phase 2 routing recommendations reviewed; adaptive routing remains disabled unless explicitly approved.
